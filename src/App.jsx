...
const db = {
  ...
  async getCheckpointStatus(seekerId) {
    const { data } = await supabase.from("intersection_progress")
      .select("*").eq("seeker_id", seekerId).single();
    return data;
  },
  async upsertCheckpointStatus(seekerId, checkpointId, status, notes) {
    const { error } = await supabase.from("intersection_progress")
      .upsert({ seeker_id: seekerId, current_checkpoint: checkpointId, status, notes, updated_at: new Date().toISOString() }, { onConflict: "seeker_id" });
    if (error) throw error;
  },
  async getAllCheckpoints() {
    const { data } = await supabase.from("intersection_progress")
      .select("*, job_seekers(full_name, governorate, sector, profile_score)");
    return data || [];
  },
  async updateCheckpointStatus(id, updates) {
    const { error } = await supabase.from("intersection_progress")
...
function MyCheckpoints() {
  ...
  useEffect(() => {
    if (profile?.id) {
      db.getCheckpointStatus(profile.id).then(setProgress).catch(() => {}).finally(() => setLoading(false));
    } else setLoading(false);
  }, [profile?.id]);

  const currentIdx = INTERSECTION_CHECKPOINTS.findIndex(c => c.id === (progress?.current_checkpoint || "foundation"));
  const pct = Math.round((currentIdx / (INTERSECTION_CHECKPOINTS.length - 1)) * 100);

  return (
    <div>
      <div style={{ marginBottom: 32 }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>◉ Intersection Mentorship</div>
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 26, fontWeight: 700, color: "#F0EBE0", marginBottom: 6 }}>My Program Journey</h1>
        <p style={{ color: "#8A9BB5", fontSize: 14 }}>Your progress through the Intersection life cycle</p>
      </div>

      {/* Timeline bar */}
      <div className="card fade-up" style={{ padding: 32, marginBottom: 24, position: "relative", overflow: "hidden" }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 24 }}>✦ Life Cycle Timeline</div>

        {/* Timeline track */}
        <div style={{ position: "relative", marginBottom: 32 }}>
          <div style={{ height: 3, background: "rgba(255,255,255,0.08)", borderRadius: 99, position: "relative" }}>
            <div style={{ width: `${pct}%`, height: "100%", background: "linear-gradient(90deg,#C9A84C,#E8C96A)", borderRadius: 99, transition: "width 1s ease", boxShadow: "0 0 10px rgba(201,168,76,0.5)" }} />
          </div>
          {/* Checkpoint dots on timeline */}
          <div style={{ display: "flex", justifyContent: "space-between", position: "absolute", top: "50%", transform: "translateY(-50%)", width: "100%", left: 0 }}>
            {INTERSECTION_CHECKPOINTS.map((cp, i) => {
              const done = i < currentIdx;
              const active = i === currentIdx;
              return (
                <div key={cp.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 18, height: 18, borderRadius: "50%", background: done || active ? cp.color : "rgba(255,255,255,0.1)", border: `2px solid ${done || active ? cp.color : "rgba(255,255,255,0.15)"}`, boxShadow: active ? `0 0 16px ${cp.color}66` : done ? `0 0 8px ${cp.color}44` : "none", transition: "all .4s", animation: active ? "goldPulse 2.5s infinite" : "none" }} />
                </div>
              );
            })}
          </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          {INTERSECTION_CHECKPOINTS.map((cp, i) => (
            <div key={cp.id} style={{ textAlign: "center", flex: 1 }}>
              <div style={{ fontSize: 10, color: "#4A5A72", fontWeight: 600, letterSpacing: 1, marginBottom: 4 }}>{cp.period}</div>
              <div style={{ fontSize: 11, color: i === currentIdx ? cp.color : i < currentIdx ? "#8A9BB5" : "#4A5A72", fontWeight: i === currentIdx ? 700 : 400 }}>{cp.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {!loading && (
        <div className="fade-up-1" style={{ marginBottom: 20 }}>
          {INTERSECTION_CHECKPOINTS.map((cp, i) => {
            ...
            return (
              <div key={cp.id} className="card" style={{ padding: 28, marginBottom: 12, border: active ? `1px solid ${cp.color}44` : "1px solid rgba(201,168,76,0.08)", position: "relative", overflow: "hidden" }}>
                {active && <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg,${cp.color},${cp.color}88)` }} />}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                      <span style={{ fontSize: 18, color: cp.color }}>{cp.icon}</span>
                      <span style={{ fontSize: 11, background: cp.colorLight, color: cp.color, border: `1px solid ${cp.color}44`, borderRadius: 20, padding: "3px 10px", fontWeight: 600 }}>{cp.period}</span>
                      {active && <span style={{ fontSize: 11, background: "rgba(201,168,76,0.12)", color: "#C9A84C", border: "1px solid rgba(201,168,76,0.3)", borderRadius: 20, padding: "3px 10px", fontWeight: 700 }}>CURRENT STAGE</span>}
                      {done && <span style={{ fontSize: 11, background: "rgba(39,174,96,0.12)", color: "#4AE08A", border: "1px solid rgba(39,174,96,0.3)", borderRadius: 20, padding: "3px 10px" }}>✓ Completed</span>}
                    </div>
                    <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 18, fontWeight: 700, color: "#F0EBE0", marginBottom: 12 }}>{cp.label}</div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                      {cp.tasks.map(task => (
                        <div key={task} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 14px", background: cp.colorLight, border: `1px solid ${cp.color}33`, borderRadius: 20, fontSize: 13, color: cp.color }}>
                          {done ? "✓" : "→"} {task}
                        </div>
                      ))}
                    </div>
                    {active && progress?.notes && (
                      <div style={{ marginTop: 16, padding: 14, background: "rgba(255,255,255,0.03)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.06)" }}>
                        <div style={{ fontSize: 11, color: "#C9A84C", marginBottom: 6, letterSpacing: 1, textTransform: "uppercase" }}>Trainer Notes</div>
                        <div style={{ fontSize: 13, color: "#8A9BB5", lineHeight: 1.7 }}>{progress.notes}</div>
                      </div>
                    )}
                  </div>
                </div>
      ...
      {/* Upcoming stages */}
      <div className="card fade-up-2" style={{ padding: 28 }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>◐ Coming Up</div>
        {INTERSECTION_CHECKPOINTS.filter((_, i) => i > currentIdx).map(cp => (
          <div key={cp.id} style={{ display: "flex", alignItems: "center", gap: 16, padding: "12px 0", borderBottom: "1px solid rgba(255,255,255,0.05)", opacity: .6 }}>
            <span style={{ fontSize: 16, color: "#4A5A72" }}>{cp.icon}</span>
            <div>
              <div style={{ fontSize: 13, color: "#8A9BB5", fontWeight: 500 }}>{cp.period} — {cp.label}</div>
              <div style={{ fontSize: 12, color: "#4A5A72" }}>{cp.tasks.join(" · ")}</div>
            </div>
          </div>
        ))}
        {currentIdx >= INTERSECTION_CHECKPOINTS.length - 1 && (
          <div style={{ textAlign: "center", padding: 24, color: "#C9A84C", fontSize: 15, fontWeight: 600 }}>
            ★ You have completed the full Intersection life cycle!
          </div>
        )}
      </div>
    </div>
...
// ── Trainer/INJAZ Team Checkpoint Management ──────────────────
function CheckpointManager({ isInjazTeam = false }) {
  const [allProgress, setAllProgress] = useState([]);
  const [seekers, setSeekers] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selected, setSelected] = useState(null);
  const [editForm, setEditForm] = useState({ checkpoint: "", status: "in_progress", notes: "" });
  const [saving, setSaving] = useState(false);
  ...
  useEffect(() => {
    (async () => {
      const [prog, users] = await Promise.all([db.getAllCheckpoints(), db.getAllUsers()]);
      setAllProgress(prog);
      setSeekers(users.seekers);
      setLoading(false);
    })();
  }, []);

  // Merge seekers with their progress
  const merged = seekers.map(s => {
    const prog = allProgress.find(p => p.seeker_id === s.id);
    return { ...s, progress: prog || null };
  }).filter(s => filterCheckpoint === "all" || (s.progress?.current_checkpoint || "foundation") === filterCheckpoint);

  const openEdit = (seeker) => {
    setSelected(seeker);
    setEditForm({
      checkpoint: seeker.progress?.current_checkpoint || "foundation",
      status: seeker.progress?.status || "in_progress",
      notes: seeker.progress?.notes || "",
    });
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await db.upsertCheckpointStatus(selected.id, editForm.checkpoint, editForm.status, editForm.notes);
      // Refresh
      const prog = await db.getAllCheckpoints();
      setAllProgress(prog);
      setSelected(null);
  ...
  const statusColors = {
    not_started: { bg: "rgba(255,255,255,0.05)", text: "#4A5A72", label: "Not Started" },
    in_progress: { bg: "rgba(41,128,185,0.12)", text: "#74B9FF", label: "In Progress" },
    completed: { bg: "rgba(39,174,96,0.12)", text: "#4AE08A", label: "Completed" },
    needs_support: { bg: "rgba(200,57,43,0.12)", text: "#FF8A80", label: "Needs Support" },
  };

  // Summary counts per checkpoint
  const counts = INTERSECTION_CHECKPOINTS.reduce((acc, cp) => {
    acc[cp.id] = seekers.filter(s => {
      const prog = allProgress.find(p => p.seeker_id === s.id);
      return (prog?.current_checkpoint || "foundation") === cp.id;
    }).length;
    return acc;
  }, {});
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        ...
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>Checkpoint Management</h1>
        <p style={{ color: "#8A9BB5", fontSize: 14 }}>Update and track participant progress across the life cycle</p>
      </div>

      {/* Checkpoint summary */}
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 24 }}>
        <div className="card" style={{ padding: "12px 16px", cursor: "pointer", border: filterCheckpoint === "all" ? "1px solid rgba(201,168,76,0.4)" : "1px solid rgba(201,168,76,0.12)", textAlign: "center", minWidth: 80 }} onClick={() => setFilterCheckpoint("all")}>
          <div style={{ fontSize: 20, color: "#C9A84C" }}>◈</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: "#F0EBE0" }}>{seekers.length}</div>
          <div style={{ fontSize: 10, color: "#8A9BB5" }}>All</div>
        </div>
        {INTERSECTION_CHECKPOINTS.map(cp => (
          <div key={cp.id} className="card" style={{ padding: "12px 16px", cursor: "pointer", border: filterCheckpoint === cp.id ? `1px solid ${cp.color}66` : "1px solid rgba(201,168,76,0.12)", textAlign: "center", minWidth: 110, transition: "all .2s" }} onClick={() => setFilterCheckpoint(filterCheckpoint === cp.id ? "all" : cp.id)}>
            <div style={{ fontSize: 18, color: cp.color, marginBottom: 4 }}>{cp.icon}</div>
            <div style={{ fontSize: 22, fontWeight: 700, color: "#F0EBE0" }}>{counts[cp.id] || 0}</div>
            <div style={{ fontSize: 9, color: "#8A9BB5", marginTop: 4, lineHeight: 1.3 }}>{cp.period}</div>
            <div style={{ fontSize: 9, color: cp.color, marginTop: 2 }}>{cp.label}</div>
          </div>
        ))}
      </div>
        ...
        : <div className="card" style={{ padding: 0, overflow: "hidden" }}>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(201,168,76,0.12)", background: "rgba(201,168,76,0.04)" }}>
                  {["Participant", "Region", "Current Checkpoint", "Status", "Last Updated", "Notes", "Action"].map(h => (
                    <th key={h} style={{ padding: "12px 16px", textAlign: "left", color: "#C9A84C", fontWeight: 600, fontSize: 10, textTransform: "uppercase", letterSpacing: .8, whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {merged.map(s => {
                  ...
                  const cp = INTERSECTION_CHECKPOINTS.find(c => c.id === (prog?.current_checkpoint || "foundation")) || INTERSECTION_CHECKPOINTS[0];
                  const statusInfo = statusColors[prog?.status || "not_started"];
                  return (
                    <tr key={s.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}
                      onMouseEnter={e => e.currentTarget.style.background = "rgba(201,168,76,0.03)"}
                      onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <div style={{ width: 32, height: 32, borderRadius: "50%", background: "rgba(201,168,76,0.12)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: "#C9A84C", fontWeight: 700, flexShrink: 0 }}>
                            {(s.full_name || "?")[0].toUpperCase()}
                          </div>
                          <span style={{ fontWeight: 600, color: "#F0EBE0" }}>{s.full_name || "—"}</span>
                        </div>
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <span style={{ color: cp.color }}>{cp.icon}</span>
                          <div>
                            <div style={{ fontSize: 12, fontWeight: 600, color: "#F0EBE0" }}>{cp.label}</div>
                            <div style={{ fontSize: 10, color: "#4A5A72" }}>{cp.period}</div>
                          </div>
                        </div>
                      </td>
                      <td style={{ padding: "12px 16px" }}>
                        <span style={{ fontSize: 11, background: statusInfo.bg, color: statusInfo.text, border: `1px solid ${statusInfo.text}44`, borderRadius: 20, padding: "3px 10px", whiteSpace: "nowrap" }}>
                          {statusInfo.label}
                        </span>
                      </td>
                      <td style={{ padding: "12px 16px", color: "#4A5A72", fontSize: 11 }}>
                        {prog?.updated_at ? new Date(prog.updated_at).toLocaleDateString("en-GB", { day: "numeric", month: "short" }) : "—"}
                      </td>
                      <td style={{ padding: "12px 16px", color: "#8A9BB5", fontSize: 12, maxWidth: 200 }}>
                        {prog?.notes ? prog.notes.slice(0, 50) + (prog.notes.length > 50 ? "..." : "") : "—"}
                      </td>
                      <td style={{ padding: "12px 16px" }}>
                        <button className="btn-primary" style={{ fontSize: 12, padding: "6px 14px" }} onClick={() => openEdit(s)}>
                          Update
                        </button>
                      </td>
                    </tr>
      ...
      {/* Edit modal */}
      {selected && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 20, backdropFilter: "blur(8px)" }} onClick={() => setSelected(null)}>
          <div className="card" style={{ maxWidth: 520, width: "100%", padding: 36, border: "1px solid rgba(201,168,76,0.3)" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
              <div>
                <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 6 }}>Update Checkpoint</div>
                <h2 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 20, fontWeight: 700, color: "#F0EBE0" }}>{selected.full_name}</h2>
              </div>
              <button onClick={() => setSelected(null)} style={{ background: "none", border: "none", color: "#4A5A72", fontSize: 24, cursor: "pointer" }}>×</button>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {INTERSECTION_CHECKPOINTS.map(cp => (
                  <label key={cp.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", borderRadius: 10, border: `1px solid ${editForm.checkpoint === cp.id ? cp.color + "66" : "rgba(255,255,255,0.08)"}`, background: editForm.checkpoint === cp.id ? cp.colorLight : "rgba(255,255,255,0.03)", cursor: "pointer", transition: "all .2s" }}>
                    <input type="radio" value={cp.id} checked={editForm.checkpoint === cp.id} onChange={() => setEditForm(f => ({ ...f, checkpoint: cp.id }))} style={{ display: "none" }} />
                    <span style={{ fontSize: 16, color: cp.color }}>{cp.icon}</span>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: editForm.checkpoint === cp.id ? cp.color : "#F0EBE0" }}>{cp.label}</div>
                      <div style={{ fontSize: 11, color: "#4A5A72" }}>{cp.period}</div>
                    </div>
                  </label>
                ))}
              </div>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {Object.entries(statusColors).map(([k, v]) => (
                  <button key={k} onClick={() => setEditForm(f => ({ ...f, status: k }))}
                    style={{ padding: "8px 14px", borderRadius: 20, border: `1px solid ${editForm.status === k ? v.text + "66" : "rgba(255,255,255,0.08)"}`, background: editForm.status === k ? v.bg : "transparent", color: editForm.status === k ? v.text : "#8A9BB5", fontSize: 12, fontWeight: editForm.status === k ? 700 : 400, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", transition: "all .2s" }}>
                    {v.label}
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <textarea className="input-field" rows={3} style={{ resize: "vertical" }}
                placeholder="Add notes about this participant's progress..."
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button className="btn-primary" style={{ flex: 1, justifyContent: "center" }} onClick={handleSave} disabled={saving}>
                {saving ? <Spinner size={16} color="#080F1E" /> : "✦ Save Checkpoint"}
              </button>

...
// ── Intersection Life Cycle Checkpoints ──────────────────────
const INTERSECTION_CHECKPOINTS = [
  {
    id: "foundation",
    period: "APR – MAY",
    ...
    tasks: ["Build trust", "Set goals", "Explore interests"],
    icon: "◆",
  },
  {
    id: "exploration",
    ...
    tasks: ["Narrow direction", "Test paths"],
    icon: "◈",
  },
  {
    id: "building",
    period: "AUG – SEP",
    label: "Building & Doing",
    color: "#2980B9",
    colorLight: "rgba(41,128,185,0.12)",
    tasks: ["Develop skills", "Create outputs"],
    icon: "◉",
  },
  {
    id: "exposure",
    period: "OCT – NOV",
    label: "Exposure & Positioning",
    color: "#8E44AD",
    colorLight: "rgba(142,68,173,0.12)",
    tasks: ["Build confidence", "Engage opportunities"],
    icon: "◐",
  },
  {
    id: "reflection",
    ...
    color: "#C8392B",
    colorLight: "rgba(200,57,43,0.12)",
    tasks: ["Reflect on growth", "Plan ahead"],
    icon: "★",
  },
];
...
const globalStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  /* Fix select dropdown options - dark theme */
  select option {
...
const db = {
  ...
  async getPlatformStats() {
    ...
    } catch(e) { return { seekers: 0, postings: 0, applications: 0 }; }
  },
  // Admin
  async getAllUsers() {
    try {
    ...
    } catch(e) { return { seekers: [], employers: [] }; }
  },
  ...
  async getMyAttendance(seekerId) {
    try {
      const { data } = await supabase.from("attendance").select("*").eq("seeker_id", seekerId).order("date", { ascending: false });
    return data || [];
    } catch(e) { return []; }
  },
  async addAttendanceSession(session) {
    const { data, error } = await supabase.from("attendance").insert(session).select().single();
    if (error) throw error;
    return data;
  },
  async updateAttendanceSession(id, updates) {
...
function GoldDivider() {
  return <div style={{ height: 1, background: "linear-gradient(90deg,transparent,rgba(201,168,76,0.25),transparent)", margin: "20px 0" }} />;
}
...
function AuthScreen({ onAuth }) {
  ...
  const [form, setForm] = useState({ email: "", password: "", full_name: "", org_name: "", governorate: "", sector: "", trainer_type: "trainer", injaz_role: "program_officer" });
    ...
    try {
      if (mode === "login") {
        ...
        await db.signUp(form.email, form.password, {
          ...
          governorate: form.governorate, sector: form.sector,
          trainer_type: form.trainer_type || null,
          injaz_role: form.injaz_role || null,
        });
  ...
  const roles = [["seeker", "◈ Participant"], ["employer", "◆ Employer"], ["trainer", "◉ Trainer / Mentor"], ["injaz_team", "✦ INJAZ Team"]];
      ...
      <div style={{ flex: "0 0 45%", background: "linear-gradient(160deg,#0A1628 0%,#050B14 100%)", display: "flex", flexDirection: "column", justifyContent: "space-between", padding: "56px 52px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          <div style={{ marginBottom: 56 }}>
            <div style={{ fontSize: 38, fontWeight: 300, color: "#F0EBE0", letterSpacing: 6, marginBottom: 4 }}>INJAZ</div>
            <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 4, textTransform: "uppercase" }}>Lebanon Career Platform</div>
          </div>
          <div style={{ fontSize: 42, fontWeight: 300, color: "#F0EBE0", lineHeight: 1.25, marginBottom: 40 }}>
            Your career<br /><span style={{ color: "#C9A84C", fontStyle: "italic" }}>journey</span><br />starts here.
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {[["◆", "AI-powered job matching"], ["◈", "CV review & optimization"], ["◉", "Interview coaching"], ["◐", "Career journey tracking"], ["★", "Mentorship program"]].map(([icon, text]) => (
              <div key={text} style={{ display: "flex", alignItems: "center", gap: 14, color: "#8A9BB5", fontSize: 14 }}>
                <span style={{ color: "#C9A84C", fontSize: 10 }}>{icon}</span>{text}
              </div>
            ))}
          </div>
        </div>
        <div style={{ position: "relative", zIndex: 1, fontSize: 11, color: "#4A5A72" }}>© 2025 INJAZ Lebanon · Intersection Mentorship Platform</div>
      </div>
        ...
        <div style={{ width: "100%", maxWidth: 420 }}>
          <div style={{ marginBottom: 32 }}>
            <h2 style={{ fontSize: 28, fontWeight: 700, color: "#F0EBE0", marginBottom: 6 }}>
              {mode === "login" ? "Welcome back" : step === 1 ? "Create Account" : "Your Details"}
            </h2>
            <p style={{ fontSize: 14, color: "#8A9BB5" }}>
              {mode === "login" ? "Sign in to your INJAZ account" : step === 1 ? "Register with your email" : "Tell us about yourself"}
            </p>
          </div>
          ...
          {success && <div style={{ background: "rgba(39,174,96,0.12)", border: "1px solid rgba(39,174,96,0.3)", borderRadius: 10, padding: "12px 16px", fontSize: 13, color: "#4AE08A", marginBottom: 20 }}>{success}</div>}

          {/* Role selector - register step 1 */}
          {mode === "register" && step === 1 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 20, background: "rgba(255,255,255,0.03)", padding: 6, borderRadius: 12, border: "1px solid rgba(255,255,255,0.06)" }}>
              {roles.map(([r, l]) => (
                <button key={r} onClick={() => setRole(r)}
                  style={{ flex: "1 1 40%", padding: "9px 8px", borderRadius: 8, border: `1px solid ${role === r ? "rgba(201,168,76,0.4)" : "transparent"}`, fontWeight: 600, fontSize: 12, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", background: role === r ? "rgba(201,168,76,0.12)" : "transparent", color: role === r ? "#C9A84C" : "#8A9BB5", transition: "all .2s" }}>{l}
                </button>
              ))}
            </div>
          )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {/* Step 1 fields */}
            {mode === "register" && step === 1 && (
              <div><label className="lbl">Full Name</label><input className="input-field" placeholder="Your full name" value={form.full_name} onChange={e => set("full_name", e.target.value)} /></div>
            )}
            {(mode === "login" || step === 1) && <>
              <div><label className="lbl">Email Address</label><input className="input-field" type="email" placeholder="your@email.com" value={form.email} onChange={e => set("email", e.target.value)} /></div>
              <div><label className="lbl">Password</label><input className="input-field" type="password" placeholder="••••••••" value={form.password} onChange={e => set("password", e.target.value)} onKeyDown={e => e.key === "Enter" && mode === "login" && handleSubmit()} /></div>
            </>}

            {/* Step 2 fields */}
            {mode === "register" && step === 2 && <>
              <div><label className="lbl">Governorate</label>
                <select className="input-field" value={form.governorate} onChange={e => set("governorate", e.target.value)}>
                  <option value="">Select...</option>{govs.map(g => <option key={g}>{g}</option>)}
                </select>
              </div>
              <div><label className="lbl">Primary Sector</label>
              ...
              {role === "employer" && <div><label className="lbl">Organization Name</label><input className="input-field" placeholder="Company or NGO name" value={form.org_name} onChange={e => set("org_name", e.target.value)} /></div>}
              {role === "trainer" && <>
                <div><label className="lbl">I am a</label>
                  <div style={{ display: "flex", gap: 8 }}>
                    {[["trainer","Trainer"],["mentor","Mentor"],["both","Both"]].map(([v,l]) => (
                      <button key={v} onClick={() => set("trainer_type", v)} style={{ flex: 1, padding: "9px", borderRadius: 10, border: `1px solid ${form.trainer_type===v?"rgba(201,168,76,0.4)":"rgba(255,255,255,0.1)"}`, background: form.trainer_type===v?"rgba(201,168,76,0.12)":"rgba(255,255,255,0.03)", color: form.trainer_type===v?"#C9A84C":"#8A9BB5", fontSize: 13, fontWeight: form.trainer_type===v?700:400, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", transition:"all .2s" }}>{l}</button>
                    ))}
                  </div>
                </div>
                <div><label className="lbl">Organization</label><input className="input-field" placeholder="e.g. INJAZ Lebanon, AUB" value={form.org_name} onChange={e => set("org_name", e.target.value)} /></div>
              </>}
              {role === "injaz_team" && <>
                <div><label className="lbl">Your Role at INJAZ</label>
                  <select className="input-field" value={form.injaz_role} onChange={e => set("injaz_role", e.target.value)}>
                    {[["program_officer","Program Officer"],["regional_manager","Regional Manager"],["country_director","Country Director"],["monitoring_evaluation","M&E"],["partnerships","Partnerships"],["other","Other"]].map(([v,l]) => <option key={v} value={v}>{l}</option>)}
                  </select>
                </div>
              </>}
            </>}
            ...
            <button className="btn-primary" style={{ width: "100%", justifyContent: "center", padding: "13px", fontSize: 15 }}
              onClick={mode === "register" && step === 1 ? () => { if (!form.full_name || !form.email || !form.password) { setError("Please fill all fields."); return; } setError(""); setStep(2); } : handleSubmit}
              disabled={loading}>
              {loading ? <Spinner size={18} color="#080F1E" /> : mode === "login" ? "Sign In ✦" : step === 1 ? "Continue →" : "Create Account ✦"}
            </button>

            {mode === "login" && <button className="btn-ghost" style={{ width: "100%", justifyContent: "center", fontSize: 13 }} onClick={handleReset}>Forgot password?</button>}
            {mode === "register" && step === 2 && <button className="btn-ghost" style={{ width: "100%", justifyContent: "center", fontSize: 13 }} onClick={() => setStep(1)}>← Back</button>}
          </div>

          <div style={{ height: 1, background: "linear-gradient(90deg,transparent,rgba(201,168,76,0.2),transparent)", margin: "24px 0" }} />
          <div style={{ textAlign: "center", fontSize: 14, color: "#8A9BB5" }}>
            {mode === "login" ? "New to INJAZ? " : "Have an account? "}
            <button onClick={() => { setMode(mode === "login" ? "register" : "login"); setStep(1); setError(""); setSuccess(""); }}
              style={{ background: "none", border: "none", color: "#C9A84C", fontWeight: 700, cursor: "pointer", fontSize: 14, fontFamily: "'DM Sans',sans-serif" }}>
...
//  SHELL LAYOUT
// ══════════════════════════════════════════════════════════════
function Shell({ navItems, userLabel, userSub, children, activePage, setActivePage, unreadCount = 0 }) {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: C.bg }}>
      ...
      {/* Mobile overlay */}
      {sidebarOpen && <div style={{ display: "block", position: "fixed", inset: 0, background: "rgba(0,0,0,.6)", zIndex: 99, backdropFilter: "blur(4px)" }} onClick={() => setSidebarOpen(false)} />}

      {/* Hamburger */}
      <button className="btn-ghost ham-btn" onClick={() => setSidebarOpen(true)} style={{ padding: "10px 12px" }}>☰</button>

      {/* Sidebar */}
      <div className={`sidebar${sidebarOpen ? " open" : ""}`} style={{ width: 240, background: C.bgSidebar, borderRight: "1px solid rgba(201,168,76,0.15)", display: "flex", flexDirection: "column", padding: "24px 14px", position: "sticky", top: 0, height: "100vh", overflowY: "auto", flexShrink: 0, zIndex: 100 }}>
        {/* Logo */}
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 24, paddingLeft: 4 }}>
          <div style={{ width: 34, height: 34, borderRadius: 9, background: "linear-gradient(135deg,#C9A84C,#E8C96A)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 16, color: "#080F1E", boxShadow: "0 4px 12px rgba(201,168,76,0.3)" }}>I</div>
          <div>
            <div style={{ fontSize: 16, fontWeight: 600, color: "#F0EBE0", letterSpacing: 2 }}>INJAZ</div>
            <div style={{ fontSize: 9, color: "#4A5A72", letterSpacing: 1.5, textTransform: "uppercase" }}>Career Platform</div>
          </div>
        </div>

        {/* User card */}
        <div style={{ background: "rgba(201,168,76,0.08)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 12, padding: "12px 14px", marginBottom: 20 }}>
          ...
          {navItems.map(item => (
            <button key={item.id}
              ...
              {item.id === "announcements" && unreadCount > 0 && (
                <span style={{ fontSize: 10, background: "#C9A84C", color: "#080F1E", borderRadius: 99, padding: "2px 7px", fontWeight: 700 }}>{unreadCount}</span>
              )}
            </button>
          ))}
        </nav>
        ...
        {/* Divider */}
        <div style={{ height: 1, background: "linear-gradient(90deg,transparent,rgba(201,168,76,0.2),transparent)", margin: "16px 0" }} />
        ...
        <button className="nav-item" onClick={async () => {
          ...
          style={{ color: "#FF8A80", marginTop: 4 }}>
      ...
      <div style={{ flex: 1, overflowX: "hidden" }}>
        <div style={{ maxWidth: 1140, margin: "0 auto", padding: "36px 28px" }}>
          <ErrorBoundary key={children?.type?.name}>
            {children}
          </ErrorBoundary>
        </div>
      </div>
...
function SeekerProfile() {
  ...
  const handleCVUpload = async (file) => {
    ...
    try {
      ...
      let review;
      try {
        review = await ai.reviewCV(text, profile.sector || "General");
      } catch (aiErr) {
        // Fallback review if AI unavailable
        review = {
          ats_score: 65,
          overall_verdict: "CV uploaded successfully. AI review temporarily unavailable — try again later.",
          top_suggestions: ["Add quantified achievements", "Include relevant keywords", "Keep to 1-2 pages"],
          missing_sections: [],
          strongest_section: "Uploaded successfully"
        };
      }
      setCvReview(review);
...
function CoverLetterAI() {
  const { profile, showError } = useAuth();
...
function JourneyTracker() {
  ...
  useEffect(() => {
    const _t = setTimeout(() => setLoading(false), 7000);
    if (profile) {
  ...
  return (
    <div>
      ...
      <div className="grid-3" style={{ marginBottom: 24 }}>
        <StatCard label="Platform Participants" value={stats.seekers} icon="◈" color={C.accent} delay={0} />
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20, background: "rgba(255,255,255,0.05)", padding: 4, borderRadius: C.radiusSm, width: "fit-content" }}>
        {[["overview", "Overview"], ["seekers", "Participants"], ["employers", "Employers"], ["postings", "Postings"]].map(([t, l]) => (
      ...
      {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 60 }}><Spinner /></div> : (
        <>
          ...
          {tab === "seekers" && (
            <div className="card">
              <h3 style={{ fontWeight: 700, color: "#F0EBE0", marginBottom: 16 }}>All Participants ({users.seekers.length})</h3>
...
function AdminJourneyOverview() {
  ...
  useEffect(() => {
    const _t = setTimeout(() => setLoading(false), 7000);
    db.getAllUsers().then(u => { setSeekers(u.seekers || []); }).catch(()=>{}).finally(()=>setLoading(false));
  }, []);
...
// ══════════════════════════════════════════════════════════════
//  INJAZ TEAM PAGES — Comprehensive Platform Management
// ══════════════════════════════════════════════════════════════


// ── Chart Components (pure SVG/CSS, no external libs) ─────────
      ...
      {data.map((d, i) => (
        <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4, height: "100%" }}>
          <div style={{ flex: 1, display: "flex", alignItems: "flex-end", width: "100%" }}>
            <div style={{ width: "100%", height: `${(d.value / max) * 100}%`, minHeight: d.value > 0 ? 4 : 0, background: `linear-gradient(180deg, ${color}, ${color}88)`, borderRadius: "4px 4px 0 0", transition: "height .8s ease", position: "relative", cursor: "default" }}
              title={`${d.label}: ${d.value}`}>
              {d.value > 0 && <div style={{ position: "absolute", top: -20, left: "50%", transform: "translateX(-50%)", fontSize: 10, color, fontWeight: 700, whiteSpace: "nowrap" }}>{d.value}</div>}
            </div>
          </div>
          <div style={{ fontSize: 9, color: "#4A5A72", textAlign: "center", lineHeight: 1.2 }}>{d.label}</div>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
      <svg width={size} height={size} viewBox="0 0 100 100">
        ...
        <text x="50" y="54" textAnchor="middle" fill="#F0EBE0" fontSize="14" fontWeight="700">{total}</text>
      </svg>
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {segments.map((seg, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12 }}>
            <div style={{ width: 8, height: 8, borderRadius: "50%", background: seg.color, flexShrink: 0 }} />
            <span style={{ color: "#8A9BB5" }}>{seg.label}</span>
...
function LineSparkline({ data, color = "#C9A84C", height = 50 }) {
...
// Lebanon interactive map (governorate heat map)
function LebanonMap({ data }) {
  // Simplified Lebanon governorate regions as labeled boxes
  const govData = {
    "Beirut": { x: 42, y: 58, w: 12, h: 10 },
    "Mount Lebanon": { x: 30, y: 45, w: 22, h: 30 },
    "North Lebanon": { x: 22, y: 10, w: 35, h: 32 },
    "Akkar": { x: 15, y: 5, w: 20, h: 22 },
    "South Lebanon": { x: 28, y: 72, w: 28, h: 18 },
    "Nabatieh": { x: 35, y: 68, w: 20, h: 16 },
    "Bekaa": { x: 55, y: 20, w: 28, h: 55 },
    "Baalbek-Hermel": { x: 58, y: 5, w: 25, h: 30 },
  };
  ...
  return (
    <div style={{ position: "relative" }}>
      <svg viewBox="0 0 100 100" style={{ width: "100%", height: 220 }}>
        ...
        {Object.entries(govData).map(([gov, pos]) => {
          const val = data[gov] || 0;
          const intensity = val / maxVal;
          const isHovered = hovered === gov;
          return (
            <g key={gov} onMouseEnter={() => setHovered(gov)} onMouseLeave={() => setHovered(null)} style={{ cursor: "pointer" }}>
              <rect x={pos.x} y={pos.y} width={pos.w} height={pos.h}
                fill={`rgba(201,168,76,${0.08 + intensity * 0.6})`}
                stroke={isHovered ? "#C9A84C" : "rgba(201,168,76,0.3)"}
                strokeWidth={isHovered ? .8 : .4} rx="1.5" />
              <text x={pos.x + pos.w/2} y={pos.y + pos.h/2 - 2} textAnchor="middle" fill="#F0EBE0" fontSize="3.5" fontWeight="600">{gov.split(" ")[0]}</text>
              <text x={pos.x + pos.w/2} y={pos.y + pos.h/2 + 4} textAnchor="middle" fill="#C9A84C" fontSize="4.5" fontWeight="700">{val}</text>
            </g>
          );
        })}
      </svg>
      {hovered && (
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, textAlign: "center", padding: "8px", background: "rgba(201,168,76,0.1)", borderRadius: 8, border: "1px solid rgba(201,168,76,0.2)" }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: "#C9A84C" }}>{hovered}</span>
          <span style={{ fontSize: 13, color: "#8A9BB5", marginLeft: 8 }}>{data[hovered] || 0} seekers</span>
        </div>
      )}
      {/* Legend */}
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8 }}>
        <span style={{ fontSize: 11, color: "#4A5A72" }}>Low</span>
        <div style={{ flex: 1, height: 6, borderRadius: 99, background: "linear-gradient(90deg, rgba(201,168,76,0.1), rgba(201,168,76,0.8))" }} />
        <span style={{ fontSize: 11, color: "#4A5A72" }}>High</span>
      </div>
    </div>
  );
}

function RatingBar({ label, value, max = 5, color = "#C9A84C" }) {
  const pct = (value / max) * 100;
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
        <span style={{ fontSize: 13, color: "#8A9BB5" }}>{label}</span>
        <span style={{ fontSize: 13, fontWeight: 700, color }}>{value.toFixed(1)}/5</span>
      </div>
      <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 99, height: 6 }}>
        <div style={{ width: `${pct}%`, height: "100%", background: `linear-gradient(90deg,${color},${color}CC)`, borderRadius: 99, boxShadow: `0 0 6px ${color}44` }} />
      </div>
    </div>
  );
}

// ── INJAZ Team Comprehensive Dashboard ───────────────────────
function InjazDashboard({ setActivePage }) {
  const { profile } = useAuth();
  const [stats, setStats] = useState({ seekers: 0, employers: 0, postings: 0, applications: 0 });
  const [users, setUsers] = useState({ seekers: [], employers: [] });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [s, u, att, refs] = await Promise.all([
          db.getPlatformStats(),
          db.getAllUsers(),
          db.getAllAttendance().catch(() => []),
          db.getAllReflections().catch(() => []),
        ]);
        setStats({ ...s, employers: u.employers.length });
        setUsers(u);
        setAttendance(att);
        setReflections(refs);
      } finally { setLoading(false); }
  ...
  const regionalData = seekers.reduce((acc, s) => {
    if (s.governorate) acc[s.governorate] = (acc[s.governorate] || 0) + 1;
    return acc;
  }, {});

  // Profile score distribution
  const scoreRanges = [
    { label: "80-100%", value: seekers.filter(s => (s.profile_score||20) >= 80).length, color: "#27AE60" },
    { label: "60-79%", value: seekers.filter(s => (s.profile_score||20) >= 60 && (s.profile_score||20) < 80).length, color: "#2980B9" },
    { label: "40-59%", value: seekers.filter(s => (s.profile_score||20) >= 40 && (s.profile_score||20) < 60).length, color: "#C9A84C" },
    { label: "<40%", value: seekers.filter(s => (s.profile_score||20) < 40).length, color: "#C8392B" },
  ];

  // Sector distribution
  const sectors = seekers.reduce((acc, s) => { if (s.sector) acc[s.sector] = (acc[s.sector] || 0) + 1; return acc; }, {});
  const topSectors = Object.entries(sectors).sort((a,b) => b[1]-a[1]).slice(0,6).map(([label,value]) => ({ label: label.slice(0,8), value }));

  // Education breakdown
  const edu = seekers.reduce((acc, s) => { const e = s.education_level || "unknown"; acc[e] = (acc[e]||0)+1; return acc; }, {});
  const eduSegments = [
    { label: "Bachelor's", value: edu.bachelor || 0, color: "#2980B9" },
    { label: "Master's", value: edu.master || 0, color: "#C9A84C" },
    { label: "PhD", value: edu.phd || 0, color: "#8E44AD" },
    { label: "Vocational", value: edu.vocational || 0, color: "#27AE60" },
    { label: "High School", value: edu.high_school || 0, color: "#E67E22" },
  ].filter(s => s.value > 0);

  // Attendance insights
  const totalSessions = attendance.length;
  const verifiedSessions = attendance.filter(s => s.verified).length;
  const mentorshipSessions = attendance.filter(s => s.type === "mentorship").length;
  const trainingSessions = attendance.filter(s => s.type === "training").length;
  const oneOnOne = attendance.filter(s => s.mentorship_format === "one_on_one").length;
  const avgRating = reflections.length > 0 ? (reflections.reduce((s, r) => s + (r.rating||0), 0) / reflections.length) : 0;

  // Experience distribution
  const expData = [
    { label: "0y", value: seekers.filter(s => (s.years_experience||0) === 0).length },
    { label: "1-2y", value: seekers.filter(s => (s.years_experience||0) >= 1 && (s.years_experience||0) <= 2).length },
    { label: "3-5y", value: seekers.filter(s => (s.years_experience||0) >= 3 && (s.years_experience||0) <= 5).length },
    { label: "6-10y", value: seekers.filter(s => (s.years_experience||0) >= 6 && (s.years_experience||0) <= 10).length },
    { label: "10y+", value: seekers.filter(s => (s.years_experience||0) > 10).length },
  ];

  // Employment status
  const empStatus = seekers.reduce((acc, s) => { const e = s.employment_status||"seeking"; acc[e]=(acc[e]||0)+1; return acc; }, {});

  return (
    <div>
      {/* Header */}
      <div className="fade-up" style={{ marginBottom: 28, position: "relative" }}>
        <div style={{ position: "absolute", top: -10, right: 0, width: 80, height: 80, border: "1px solid rgba(201,168,76,0.15)", borderRadius: "50%", animation: "rotateSlow 20s linear infinite" }} />
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 6 }}>✦ INJAZ Lebanon Platform Intelligence</div>
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 26, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>Good {new Date().getHours()<12?"morning":new Date().getHours()<17?"afternoon":"evening"}, <span style={{ color: "#C9A84C" }}>{name}</span></h1>
        <p style={{ color: "#8A9BB5", fontSize: 13 }}>{profile?.injaz_role?.replace(/_/g," ")} · {new Date().toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</p>
      </div>

      {/* KPI Row */}
      <div className="grid-4" style={{ marginBottom: 20 }}>
        {[
          { label: "Participants", value: stats.seekers, icon: "◈", color: "#2980B9", sub: `${seekers.filter(s=>(s.profile_score||20)>=80).length} complete`, page: "seekers" },
          { label: "Employers", value: stats.employers, icon: "◆", color: "#27AE60", sub: `${stats.postings} active postings`, page: "employers" },
          { label: "Sessions Logged", value: totalSessions, icon: "◉", color: "#C9A84C", sub: `${verifiedSessions} verified`, page: "attendance" },
          { label: "Reflections", value: reflections.length, icon: "★", color: "#8E44AD", sub: avgRating > 0 ? `Avg rating: ${avgRating.toFixed(1)}/5` : "No ratings yet", page: "attendance" },
        ].map((s, i) => (
          <div key={s.label} className="card stat-hover fade-up" style={{ padding: 20, cursor: "pointer", animationDelay: `${i*.06}s` }} onClick={() => setActivePage(s.page)}>
            ...
            <div style={{ fontSize: 30, fontWeight: 700, color: "#F0EBE0", marginBottom: 3 }}>{loading ? "—" : s.value}</div>
            <div style={{ fontSize: 12, color: "#8A9BB5", marginBottom: 4 }}>{s.label}</div>
            <div style={{ fontSize: 11, color: s.color }}>{s.sub}</div>
          </div>
        ))}
      </div>

      {/* Row 2: Map + Journey Funnel */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        ...
        <div className="card fade-up-1" style={{ padding: 24 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <div>
              <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 4 }}>◈ Geographic Distribution</div>
              <div style={{ fontSize: 14, fontWeight: 600, color: "#F0EBE0" }}>Seekers by Region</div>
            </div>
            <div style={{ fontSize: 11, color: "#4A5A72" }}>Hover to explore</div>
          </div>
          <LebanonMap data={regionalData} />
        </div>

        {/* Journey Funnel */}
        <div className="card fade-up-1" style={{ padding: 24 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>✦ Journey Funnel</div>
          {JOURNEY_STAGES.map((stage, i) => {
            const count = stageCounts[stage.id] || 0;
            const pct = stats.seekers > 0 ? Math.round((count / stats.seekers) * 100) : 0;
            const width = 100 - i * 10;
            return (
              <div key={stage.id} style={{ marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                  <span style={{ fontSize: 12, color: "#F0EBE0" }}>{stage.icon} {stage.label}</span>
                  <span style={{ fontSize: 12, fontWeight: 700, color: "#C9A84C" }}>{count} <span style={{ color: "#4A5A72", fontWeight: 400 }}>({pct}%)</span></span>
                </div>
                <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 99, height: 6 }}>
                  <div style={{ width: `${pct}%`, height: "100%", background: `linear-gradient(90deg,#C9A84C,#E8C96A)`, borderRadius: 99, boxShadow: "0 0 6px rgba(201,168,76,0.3)", transition: "width .8s ease" }} />
                </div>
              </div>
            );
          })}
      ...
      {/* Row 3: Sector bar + Profile scores donut */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        
        {/* Sector distribution */}
        <div className="card fade-up-2" style={{ padding: 24 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>◆ Seekers by Sector</div>
          <BarChart data={topSectors} color="#2980B9" height={130} />
        </div>

        {/* Profile completion donut */}
        <div className="card fade-up-2" style={{ padding: 24 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>◈ Profile Completion</div>
          <DonutChart segments={scoreRanges} size={130} />
        </div>
      </div>

      {/* Row 4: Experience + Education + Employment */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
        
        {/* Experience */}
        <div className="card fade-up-3" style={{ padding: 24 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>◉ Experience Levels</div>
          <BarChart data={expData} color="#C9A84C" height={110} />
        </div>

        {/* Education */}
        <div className="card fade-up-3" style={{ padding: 24 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>◈ Education</div>
          <DonutChart segments={eduSegments} size={110} />
        </div>

        {/* Employment status */}
        <div className="card fade-up-3" style={{ padding: 24 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>◐ Employment Status</div>
          <DonutChart segments={[
            { label: "Seeking", value: empStatus.seeking||0, color: "#C8392B" },
            { label: "Employed", value: empStatus.employed||0, color: "#27AE60" },
            { label: "Student", value: empStatus.student||0, color: "#2980B9" },
            { label: "Freelance", value: empStatus.freelance||0, color: "#C9A84C" },
          ].filter(s=>s.value>0)} size={110} />
        </div>
      </div>

      {/* Row 5: Attendance insights + Mentor ratings */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        
        {/* Attendance breakdown */}
        <div className="card fade-up-4" style={{ padding: 24 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>≡ Attendance Insights</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
            {[
              { label: "Total Sessions", value: totalSessions, color: "#C9A84C" },
              { label: "Verified", value: verifiedSessions, color: "#27AE60" },
              { label: "Training", value: trainingSessions, color: "#2980B9" },
              { label: "Mentorship", value: mentorshipSessions, color: "#8E44AD" },
              { label: "1-on-1", value: oneOnOne, color: "#E67E22" },
              { label: "Group", value: mentorshipSessions - oneOnOne, color: "#1ABC9C" },
            ].map(s => (
              <div key={s.label} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 10, padding: "12px 14px", border: `1px solid ${s.color}22` }}>
          ...
          <div style={{ fontSize: 12, color: "#8A9BB5", display: "flex", justifyContent: "space-between" }}>
            <span>Verification rate:</span>
            <span style={{ color: "#27AE60", fontWeight: 700 }}>{totalSessions > 0 ? Math.round(verifiedSessions/totalSessions*100) : 0}%</span>
          </div>
        </div>

        {/* Reflection ratings */}
        <div className="card fade-up-4" style={{ padding: 24 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>★ Mentorship Quality</div>
          {reflections.length === 0 ? (
            <div style={{ textAlign: "center", padding: 32, color: "#4A5A72" }}>
              <div style={{ fontSize: 32, marginBottom: 12, opacity: .3 }}>★</div>
              <div>No reflection cards submitted yet</div>
            </div>
          ) : (
            <>
              <div style={{ textAlign: "center", marginBottom: 20 }}>
                <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 42, fontWeight: 700, color: "#C9A84C" }}>{avgRating.toFixed(1)}</div>
                <div style={{ display: "flex", justifyContent: "center", gap: 4, marginBottom: 6 }}>
                  {[1,2,3,4,5].map(s => <span key={s} style={{ fontSize: 18, color: s <= Math.round(avgRating) ? "#C9A84C" : "rgba(255,255,255,0.1)" }}>★</span>)}
                </div>
                <div style={{ fontSize: 12, color: "#8A9BB5" }}>Average session rating · {reflections.length} total</div>
              </div>
              <RatingBar label="5 stars" value={reflections.filter(r=>r.rating===5).length} max={reflections.length} color="#27AE60" />
              <RatingBar label="4 stars" value={reflections.filter(r=>r.rating===4).length} max={reflections.length} color="#2980B9" />
              <RatingBar label="3 stars" value={reflections.filter(r=>r.rating===3).length} max={reflections.length} color="#C9A84C" />
              <RatingBar label="1-2 stars" value={reflections.filter(r=>r.rating<=2).length} max={reflections.length} color="#C8392B" />
            </>
          )}
        </div>
      </div>
      ...
      <div className="card fade-up" style={{ padding: 20 }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 14 }}>✦ Quick Access</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 8 }}>
          {[["◈","Seekers","seekers"],["◆","Employers","employers"],["◐","Journey","journey"],["◉","Checkpoints","checkpoints"],["≡","Attendance","attendance"],["◈","Trainers","trainers-overview"],["★","Reports","reports"],["⊕","Assessments","assessments"]].map(([icon,label,page])=>(
            <button key={page} onClick={()=>setActivePage(page)} style={{ padding:"12px 8px", borderRadius:10, border:"1px solid rgba(201,168,76,0.12)", background:"rgba(255,255,255,0.02)", cursor:"pointer", textAlign:"center", transition:"all .2s", fontFamily:"'DM Sans',sans-serif" }}
              ...
              onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(201,168,76,0.12)";e.currentTarget.style.background="rgba(255,255,255,0.02)"}}>
              <div style={{fontSize:18,color:"#C9A84C",marginBottom:6}}>{icon}</div>
              <div style={{fontSize:11,color:"#F0EBE0"}}>{label}</div>
            </button>
          ))}
        </div>
      </div>
...
function TrainerDashboard({ setActivePage }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [s, u, att, refs] = await Promise.all([
          db.getPlatformStats(),
          db.getAssignedSeekers(profile?.id).catch(() => []),
          db.getAllAttendance().catch(() => []),
          db.getAllReflections().catch(() => []),
        ]);
        setStats(s);
  ...
  // Journey distribution of all seekers (visible to trainer)
  const stageCounts = JOURNEY_STAGES.reduce((acc, s) => { acc[s.id] = 0; return acc; }, {});
  participants.forEach(a => {
    const stage = a.job_seekers?.journey_stage || "registered";
    if (stageCounts[stage] !== undefined) stageCounts[stage]++;
  });

  // Session type breakdown
  const sessionData = [
    { label: "Training", value: attendance.filter(s=>s.type==="training").length },
    { label: "Group", value: attendance.filter(s=>s.mentorship_format==="group").length },
    { label: "1-on-1", value: oneOnOne },
  ];

  return (
    <div>
      <div className="fade-up" style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 6 }}>◉ {typeLabel} Dashboard</div>
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>Welcome, <span style={{ color: "#C9A84C" }}>{name}</span></h1>
        <p style={{ color: "#8A9BB5", fontSize: 13 }}>{profile?.org_name || "INJAZ Lebanon"} · {typeLabel}</p>
      </div>

      {/* KPIs */}
      <div className="grid-4" style={{ marginBottom: 20 }}>
        {[
          { label: "Participants", value: participants.length, icon: "◈", color: "#2980B9" },
          { label: "Total Sessions", value: totalSessions, icon: "◉", color: "#C9A84C" },
          { label: "Verified", value: verifiedSessions, icon: "✓", color: "#27AE60" },
          { label: "Avg Rating", value: avgRating > 0 ? avgRating.toFixed(1)+"★" : "—", icon: "★", color: "#8E44AD" },
        ].map((s,i) => (
          <div key={s.label} className="card stat-hover fade-up" style={{ padding: 20, animationDelay: `${i*.06}s` }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
              <span style={{ fontSize: 20, color: s.color }}>{s.icon}</span>
              <div style={{ width: 6, height: 6, borderRadius: "50%", background: s.color, boxShadow: `0 0 8px ${s.color}` }} />
            </div>
            <div style={{ fontSize: 28, fontWeight: 700, color: "#F0EBE0", marginBottom: 3 }}>{loading ? "—" : s.value}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        {/* Session breakdown */}
        <div className="card fade-up-1" style={{ padding: 24 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>◆ Session Breakdown</div>
          <BarChart data={sessionData} color="#C9A84C" height={120} />
          <div style={{ height: 1, background: "rgba(255,255,255,0.06)", margin: "16px 0" }} />
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
            <span style={{ color: "#8A9BB5" }}>Verification rate</span>
            <span style={{ color: "#27AE60", fontWeight: 700 }}>{totalSessions > 0 ? Math.round(verifiedSessions/totalSessions*100) : 0}%</span>
          </div>
        </div>
        ...
        {/* Mentorship quality */}
        <div className="card fade-up-1" style={{ padding: 24 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>★ Session Quality</div>
          {reflections.length === 0 ? (
            <div style={{ textAlign: "center", padding: 32, color: "#4A5A72" }}>
              <div style={{ fontSize: 32, marginBottom: 8, opacity: .3 }}>★</div>
              <div style={{ fontSize: 13 }}>No reflections yet</div>
            </div>
          ) : (
            <>
              <div style={{ textAlign: "center", marginBottom: 20 }}>
                <div style={{ fontSize: 40, fontWeight: 700, color: "#C9A84C" }}>{avgRating.toFixed(1)}</div>
                <div style={{ display: "flex", justifyContent: "center", gap: 3, marginBottom: 6 }}>
                  {[1,2,3,4,5].map(s=><span key={s} style={{ fontSize: 16, color: s<=Math.round(avgRating)?"#C9A84C":"rgba(255,255,255,0.1)" }}>★</span>)}
                </div>
                <div style={{ fontSize: 12, color: "#8A9BB5" }}>{reflections.length} reflection cards</div>
              </div>
              {[5,4,3,2,1].map(star => (
                <div key={star} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  <span style={{ fontSize: 11, color: "#C9A84C", width: 16 }}>{star}★</span>
                  <div style={{ flex: 1, background: "rgba(255,255,255,0.06)", borderRadius: 99, height: 5 }}>
                    <div style={{ width: reflections.length > 0 ? `${(reflections.filter(r=>r.rating===star).length/reflections.length)*100}%` : "0%", height: "100%", background: "#C9A84C", borderRadius: 99, transition: "width .8s ease" }} />
                  </div>
                  <span style={{ fontSize: 11, color: "#4A5A72", width: 16 }}>{reflections.filter(r=>r.rating===star).length}</span>
                </div>
              ))}
            </>
          )}
      ...
      <div className="card fade-up-2" style={{ padding: 28 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase" }}>◈ Assigned Participants</div>
          <button className="btn-ghost" style={{ fontSize: 12 }} onClick={() => setActivePage("participants")}>View all →</button>
        </div>
        {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 40 }}><Spinner size={28} /></div>
          : participants.length === 0 ? (
            ...
            if (!s) return null;
            const stage = JOURNEY_STAGES.find(st => st.id === (s.journey_stage || "registered"));
            const score = s.profile_score || 20;
            return (
              <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 16, padding: "12px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: "rgba(201,168,76,0.12)", border: "1px solid rgba(201,168,76,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, color: "#C9A84C", fontWeight: 700, flexShrink: 0 }}>
                  {(s.full_name||"?")[0].toUpperCase()}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 600, fontSize: 13, color: "#F0EBE0", marginBottom: 2 }}>{s.full_name||"—"}</div>
                  <div style={{ fontSize: 11, color: "#8A9BB5" }}>{s.governorate} · {s.sector}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <span style={{ fontSize: 11, background: "rgba(201,168,76,0.1)", color: "#C9A84C", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 20, padding: "2px 8px" }}>{stage?.icon} {stage?.label}</span>
                  <div style={{ fontSize: 11, color: "#4A5A72", marginTop: 4 }}>{score}% profile</div>
                </div>
              </div>
            );
          })}
...
function InjazSeekers() {
  ...
  const filtered = seekers
    .filter(s => {
      const matchSearch = !search || (s.full_name || "").toLowerCase().includes(search.toLowerCase()) || (s.sector || "").toLowerCase().includes(search.toLowerCase());
      const matchGov = govFilter === "all" || s.governorate === govFilter;
      const matchStage = stageFilter === "all" || (s.journey_stage || "registered") === stageFilter;
      return matchSearch && matchGov && matchStage;
    })
    .sort((a, b) => {
      if (sortBy === "score") return (b.profile_score || 20) - (a.profile_score || 20);
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
        <div>
          <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>Participants</h1>
          <p style={{ fontSize: 13, color: "#8A9BB5" }}>{filtered.length} of {seekers.length} participants</p>
        </div>
        <div style={{ background: "rgba(201,168,76,0.1)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 10, padding: "8px 18px", fontSize: 14, fontWeight: 700, color: "#C9A84C" }}>{seekers.length} total</div>
      </div>

      {/* Filters */}
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        <input className="input-field" style={{ flex: 1, minWidth: 200 }} placeholder="Search name or sector..." value={search} onChange={e => setSearch(e.target.value)} />
        <select className="select-field" style={{ width: 170 }} value={govFilter} onChange={e => setGovFilter(e.target.value)}>
          <option value="all">All Regions</option>
          {govs.map(g => <option key={g}>{g}</option>)}
        </select>
        <select className="select-field" style={{ width: 170 }} value={stageFilter} onChange={e => setStageFilter(e.target.value)}>
          <option value="all">All Stages</option>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(201,168,76,0.12)", background: "rgba(201,168,76,0.04)" }}>
                  {["Name", "Region", "Sector", "Experience", "Education", "Profile", "Journey", "Registered"].map(h => (
                    <th key={h} style={{ padding: "12px 16px", textAlign: "left", color: "#C9A84C", fontWeight: 600, fontSize: 10, textTransform: "uppercase", letterSpacing: .8, whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                    ...
                    <tr key={s.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)", transition: "background .15s" }}
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <div style={{ width: 32, height: 32, borderRadius: "50%", background: "rgba(201,168,76,0.15)", border: "1px solid rgba(201,168,76,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "#C9A84C", fontWeight: 700, flexShrink: 0 }}>
                            {(s.full_name || "?")[0].toUpperCase()}
                          </div>
                          <div>
                            <div style={{ fontWeight: 600, color: "#F0EBE0" }}>{s.full_name || "—"}</div>
                      ...
                      <td style={{ padding: "12px 16px", color: "#8A9BB5" }}>{s.governorate || "—"}</td>
                      <td style={{ padding: "12px 16px", color: "#8A9BB5" }}>{s.sector || "—"}</td>
                      <td style={{ padding: "12px 16px", color: "#8A9BB5" }}>{s.years_experience || 0}y</td>
                      <td style={{ padding: "12px 16px", color: "#8A9BB5" }}>{s.education_level?.replace("_", " ") || "—"}</td>
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{ width: 60, background: "rgba(255,255,255,0.06)", borderRadius: 99, height: 4 }}>
                            <div style={{ width: `${score}%`, height: "100%", background: score >= 80 ? "#27AE60" : score >= 50 ? "#C9A84C" : "#E74C3C", borderRadius: 99 }} />
                          </div>
                          <span style={{ fontSize: 12, fontWeight: 700, color: score >= 80 ? "#4AE08A" : score >= 50 ? "#C9A84C" : "#FF8A80" }}>{score}%</span>
                        </div>
                      </td>
                      <td style={{ padding: "12px 16px" }}>
                        <span style={{ fontSize: 11, background: "rgba(201,168,76,0.1)", color: "#C9A84C", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 20, padding: "3px 10px", whiteSpace: "nowrap" }}>
                          {stage?.icon} {stage?.label}
                        </span>
                      </td>
                      <td style={{ padding: "12px 16px", color: "#4A5A72", fontSize: 11 }}>
                        {s.created_at ? new Date(s.created_at).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "2-digit" }) : "—"}
                      </td>
                    </tr>
                  );
                })}
...
function InjazEmployers() {
  ...
  useEffect(() => {
    const _t = setTimeout(() => setLoading(false), 7000);
    (async () => {
      const [u, p] = await Promise.all([db.getAllUsers(), db.getAllPostings()]);
      setEmployers(u.employers);
      setPostings(p);
      setLoading(false);
    })();
  }, []);
  ...
  const togglePosting = async (p) => {
    const s = p.status === "active" ? "closed" : "active";
    await supabase.from("postings").update({ status: s }).eq("id", p.id);
    setPostings(ps => ps.map(x => x.id === p.id ? { ...x, status: s } : x));
    showSuccess(`Posting ${s}`);
  };

  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>Employers & Job Postings</h1>
        <p style={{ fontSize: 13, color: "#8A9BB5" }}>{employers.length} employers · {postings.length} total postings</p>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        {/* Employers */}
        <div className="card" style={{ padding: 28 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>◆ Registered Employers ({employers.length})</div>
          {loading ? <Spinner /> : employers.map(e => (
            <div key={e.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
              <div>
                <div style={{ fontWeight: 600, fontSize: 14, color: "#F0EBE0" }}>{e.org_name}</div>
                <div style={{ fontSize: 12, color: "#8A9BB5" }}>{e.contact_person} · {e.governorate} · {e.sector}</div>
                <div style={{ fontSize: 11, color: "#4A5A72", marginTop: 2 }}>
                  {postings.filter(p => p.employer_id === e.id).length} postings
                </div>
              </div>
              <span style={{ fontSize: 11, background: "rgba(39,174,96,0.12)", color: "#4AE08A", border: "1px solid rgba(39,174,96,0.25)", borderRadius: 20, padding: "3px 10px" }}>Active</span>
            </div>
          ))}
        </div>
        ...
        {/* Postings */}
        <div className="card" style={{ padding: 28 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>◉ All Job Postings ({postings.length})</div>
          {loading ? <Spinner /> : postings.map(p => (
            <div key={p.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, fontSize: 13, color: "#F0EBE0", marginBottom: 2 }}>{p.title}</div>
                <div style={{ fontSize: 11, color: "#8A9BB5" }}>{p.employers?.org_name} · {p.type} · {p.governorate}</div>
              </div>
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0, marginLeft: 10 }}>
                <span style={{ fontSize: 11, background: p.status === "active" ? "rgba(39,174,96,0.12)" : "rgba(255,255,255,0.05)", color: p.status === "active" ? "#4AE08A" : "#8A9BB5", border: `1px solid ${p.status === "active" ? "rgba(39,174,96,0.25)" : "rgba(255,255,255,0.08)"}`, borderRadius: 20, padding: "3px 10px" }}>{p.status}</span>
                <button onClick={() => togglePosting(p)} style={{ fontSize: 11, background: "transparent", border: "1px solid rgba(255,255,255,0.1)", color: "#8A9BB5", borderRadius: 8, padding: "3px 10px", cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}>{p.status === "active" ? "Close" : "Reopen"}</button>
              </div>
            </div>
          ))}
        </div>
...
function InjazJourneyTracking() {
  ...
  const filtered = seekers.filter(s => {
    const matchStage = stageFilter === "all" || (s.journey_stage || "registered") === stageFilter;
    const matchGov = govFilter === "all" || s.governorate === govFilter;
    return matchStage && matchGov;
  });

  const counts = JOURNEY_STAGES.reduce((a, s) => {
    a[s.id] = seekers.filter(u => (u.journey_stage || "registered") === s.id).length;
    return a;
  }, {});
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>Journey Tracking</h1>
        <p style={{ fontSize: 13, color: "#8A9BB5" }}>All participant progress across Lebanon</p>
      </div>

      {/* Stage cards */}
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 24 }}>
        <div className="card" style={{ padding: "12px 16px", cursor: "pointer", border: stageFilter === "all" ? "1px solid rgba(201,168,76,0.4)" : "1px solid rgba(201,168,76,0.12)", minWidth: 80, textAlign: "center" }} onClick={() => setStageFilter("all")}>
          <div style={{ fontSize: 20 }}>◈</div>
          <div style={{ fontSize: 22, fontWeight: 700, color: "#F0EBE0" }}>{seekers.length}</div>
          <div style={{ fontSize: 10, color: "#8A9BB5" }}>All</div>
        </div>
        {JOURNEY_STAGES.map(stage => (
          <div key={stage.id} className="card" style={{ padding: "12px 16px", cursor: "pointer", border: stageFilter === stage.id ? "1px solid rgba(201,168,76,0.4)" : "1px solid rgba(201,168,76,0.12)", minWidth: 90, textAlign: "center", transition: "all .2s" }} onClick={() => setStageFilter(stageFilter === stage.id ? "all" : stage.id)}>
            <div style={{ fontSize: 18, color: "#C9A84C" }}>{stage.icon}</div>
            <div style={{ fontSize: 20, fontWeight: 700, color: "#F0EBE0" }}>{counts[stage.id] || 0}</div>
            <div style={{ fontSize: 10, color: "#8A9BB5" }}>{stage.label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
        <select className="select-field" style={{ width: 200 }} value={govFilter} onChange={e => setGovFilter(e.target.value)}>
          <option value="all">All Regions</option>
          {govs.map(g => <option key={g}>{g}</option>)}
        </select>
        <div style={{ padding: "10px 16px", background: "rgba(201,168,76,0.1)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 10, fontSize: 13, color: "#C9A84C", fontWeight: 600 }}>
        ...
        {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 60 }}><Spinner size={36} /></div>
          : <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(201,168,76,0.12)", background: "rgba(201,168,76,0.04)" }}>
                  {["Participant", "Region", "Sector", "Profile", "Current Stage", "Journey Progress", "Last Updated"].map(h => (
                    <th key={h} style={{ padding: "12px 16px", textAlign: "left", color: "#C9A84C", fontWeight: 600, fontSize: 10, textTransform: "uppercase", letterSpacing: .8, whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
                ...
                {filtered.map(s => {
                  ...
                  return (
                    <tr key={s.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <div style={{ width: 30, height: 30, borderRadius: "50%", background: "rgba(201,168,76,0.12)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "#C9A84C", fontWeight: 700, flexShrink: 0 }}>
                            {(s.full_name || "?")[0].toUpperCase()}
                          </div>
                          <span style={{ fontWeight: 600, color: "#F0EBE0" }}>{s.full_name || "—"}</span>
                        </div>
                      </td>
                      <td style={{ padding: "12px 16px", color: "#8A9BB5" }}>{s.governorate || "—"}</td>
                      <td style={{ padding: "12px 16px", color: "#8A9BB5" }}>{s.sector || "—"}</td>
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <div style={{ width: 50, background: "rgba(255,255,255,0.06)", borderRadius: 99, height: 4 }}>
                            <div style={{ width: `${s.profile_score || 20}%`, height: "100%", background: "#C9A84C", borderRadius: 99 }} />
                          </div>
                          <span style={{ fontSize: 11, color: "#C9A84C", fontWeight: 700 }}>{s.profile_score || 20}%</span>
                        </div>
                      ...
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <div style={{ width: 80, background: "rgba(255,255,255,0.06)", borderRadius: 99, height: 5 }}>
                            <div style={{ width: `${progress}%`, height: "100%", background: "linear-gradient(90deg,#C9A84C,#E8C96A)", borderRadius: 99, boxShadow: "0 0 6px rgba(201,168,76,0.4)" }} />
                          </div>
                          <span style={{ fontSize: 11, color: "#4A5A72" }}>{progress}%</span>
                        </div>
                      </td>
                      <td style={{ padding: "12px 16px", color: "#4A5A72", fontSize: 11 }}>
                        {s.journey_updated_at ? new Date(s.journey_updated_at).toLocaleDateString("en-GB", { day: "numeric", month: "short" }) : "—"}
                      </td>
                    </tr>
                  );
                })}
...
function InjazTrainersOverview() {
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>Trainers & Mentors</h1>
        <p style={{ fontSize: 13, color: "#8A9BB5" }}>{trainers.length} registered trainers and mentors</p>
      </div>

      {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 80 }}><Spinner size={36} /></div>
        : trainers.length === 0 ? <div className="card" style={{ textAlign: "center", padding: 80, color: "#4A5A72" }}>No trainers registered yet.</div>
        : <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {trainers.map(t => (
            <div key={t.id} className="card" style={{ padding: "20px 24px", display: "flex", alignItems: "center", gap: 20 }}>
              <div style={{ width: 48, height: 48, borderRadius: "50%", background: "rgba(201,168,76,0.12)", border: "1px solid rgba(201,168,76,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, color: "#C9A84C", fontWeight: 700, flexShrink: 0 }}>
                ...
                {t.expertise_areas?.length > 0 && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {t.expertise_areas.slice(0, 4).map(e => (
                      <span key={e} style={{ fontSize: 11, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: "2px 8px", color: "#8A9BB5" }}>{e}</span>
                    ))}
                  </div>
                )}
              </div>
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                <ScoreRing score={t.profile_score || 20} size={48} />
              </div>
            </div>
          ))}
        </div>}
...
function InjazReports() {
  const [data, setData] = useState({ seekers: [], employers: [], postings: [] });
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    (async () => {
      const [u, p] = await Promise.all([db.getAllUsers(), db.getAllPostings()]);
      setData({ seekers: u.seekers, employers: u.employers, postings: p });
      setLoading(false);
    })();
  }, []);
  ...
  const exportCSV = (rows, filename) => {
    ...
    const csv = headers + newline + body;
  ...
  const seekerExport = data.seekers.map(s => ({
    name: s.full_name || "", governorate: s.governorate || "", sector: s.sector || "",
    education: s.education_level || "", experience: s.years_experience || 0,
    profile_score: s.profile_score || 20, journey_stage: s.journey_stage || "registered",
    registered: s.created_at ? new Date(s.created_at).toLocaleDateString() : ""
  }));

  const employerExport = data.employers.map(e => ({
    org_name: e.org_name || "", contact: e.contact_person || "",
    governorate: e.governorate || "", sector: e.sector || "",
    postings: data.postings.filter(p => p.employer_id === e.id).length
  }));

  const stats = {
    totalSeekers: data.seekers.length,
    profileComplete: data.seekers.filter(s => (s.profile_score || 20) >= 80).length,
    placed: data.seekers.filter(s => s.journey_stage === "placed").length,
    avgScore: data.seekers.length > 0 ? Math.round(data.seekers.reduce((s, u) => s + (u.profile_score || 20), 0) / data.seekers.length) : 0,
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>Reports & Export</h1>
        <p style={{ fontSize: 13, color: "#8A9BB5" }}>Program performance and data export</p>
      </div>

      {/* KPI Summary */}
      <div className="grid-3" style={{ marginBottom: 24 }}>
        {[
          { label: "Total Participants", value: stats.totalSeekers, color: "#2980B9" },
          { label: "Profile Complete (80%+)", value: stats.profileComplete, color: "#27AE60" },
          { label: "Successfully Placed", value: stats.placed, color: "#C9A84C" },
          { label: "Avg Profile Score", value: `${stats.avgScore}%`, color: "#9B59B6" },
          { label: "Active Job Postings", value: stats.activePostings, color: "#E74C3C" },
          { label: "Partner Employers", value: stats.totalEmployers, color: "#1ABC9C" },
        ].map((k, i) => (
          <div key={k.label} className="card fade-up" style={{ padding: 20, animationDelay: `${i*.05}s` }}>
            <div style={{ fontSize: 28, fontWeight: 700, color: k.color, marginBottom: 6 }}>{k.value}</div>
            <div style={{ fontSize: 12, color: "#8A9BB5" }}>{k.label}</div>
          </div>
        ))}
      </div>
      ...
      {/* Export buttons */}
      <div className="card" style={{ padding: 28 }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>✦ Export Data</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {[
            { label: "Export All Participants (CSV)", desc: "Names, regions, sectors, profile scores, journey stages", action: () => exportCSV(seekerExport, "injaz_participants.csv"), icon: "◈" },
            { label: "Export Employers (CSV)", desc: "Organizations, contacts, regions, number of postings", action: () => exportCSV(employerExport, "injaz_employers.csv"), icon: "◆" },
          ].map(item => (
            <div key={item.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 20px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(201,168,76,0.12)", borderRadius: 12 }}>
              <div>
                <div style={{ fontWeight: 600, fontSize: 14, color: "#F0EBE0", marginBottom: 4 }}>{item.icon} {item.label}</div>
                <div style={{ fontSize: 12, color: "#8A9BB5" }}>{item.desc}</div>
              </div>
              <button className="btn-primary" style={{ fontSize: 13, flexShrink: 0, marginLeft: 16 }} onClick={item.action} disabled={loading}>
                {loading ? <Spinner size={16} color="#080F1E" /> : "↓ Export"}
              </button>
            </div>
          ))}
        </div>
...
function InjazProfile() {
  const { profile, setProfile, showSuccess, showError } = useAuth();
  const [form, setForm] = useState({ full_name: "", phone: "", governorate: "", injaz_role: "program_officer" });
  const [saving, setSaving] = useState(false);
  const govs = ["Beirut", "Mount Lebanon", "North Lebanon", "South Lebanon", "Bekaa", "Nabatieh", "Akkar", "Baalbek-Hermel"];

  useEffect(() => {
    const _t = setTimeout(() => setLoading(false), 7000);
    if (profile) setForm({ full_name: profile.full_name || "", phone: profile.phone || "", governorate: profile.governorate || "", injaz_role: profile.injaz_role || "program_officer" });
  }, [profile?.id]);
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
        <div>
          <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>My Profile</h1>
          <p style={{ fontSize: 13, color: "#8A9BB5" }}>INJAZ Team Member</p>
        </div>
        <button className="btn-primary" onClick={handleSave} disabled={saving} style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {saving ? <Spinner size={16} color="#080F1E" /> : "✦"} Save
        </button>
      </div>
      <div className="card fade-up" style={{ padding: 28 }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>◆ Personal Information</div>
        <div className="grid-2" style={{ gap: 16 }}>
          <div><Label required>Full Name</Label><input className="input-field" value={form.full_name} onChange={e => setForm(f => ({ ...f, full_name: e.target.value }))} placeholder="Your full name" /></div>
          <div><Label>Phone</Label><input className="input-field" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} placeholder="+961 XX XXX XXX" /></div>
          <div><Label>Your Role at INJAZ</Label>
            <select className="select-field" value={form.injaz_role} onChange={e => setForm(f => ({ ...f, injaz_role: e.target.value }))}>
              ...
              <option value="partnerships">Partnerships Officer</option>
              <option value="other">Other</option>
            </select>
          </div>
          <div><Label>Region / Office</Label>
            <select className="select-field" value={form.governorate} onChange={e => setForm(f => ({ ...f, governorate: e.target.value }))}>
...
// ══════════════════════════════════════════════════════════════
//  INTERSECTION LIFE CYCLE PAGES
// ══════════════════════════════════════════════════════════════

// ── Participant & Trainer View (read-only, shows global program stage) ──
function MyCheckpoints() {
  const [checkpoint, setCheckpoint] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const _t = setTimeout(() => setLoading(false), 7000);
    db.getProgramCheckpoint()
      .then(setCheckpoint)
      .catch(() => setCheckpoint(null))
      .finally(() => setLoading(false));
  }, []);

  const currentId = checkpoint?.value || "foundation";
  const currentIdx = INTERSECTION_CHECKPOINTS.findIndex(c => c.id === currentId);
  const meta = checkpoint?.meta || {};
  const pct = Math.round((currentIdx / (INTERSECTION_CHECKPOINTS.length - 1)) * 100);
  ...
  return (
    <div>
      <div style={{ marginBottom: 32 }}>
        ...
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 26, fontWeight: 700, color: "#F0EBE0", marginBottom: 6 }}>Program Life Cycle</h1>
        <p style={{ color: "#8A9BB5", fontSize: 14 }}>Current stage for all participants — updated by the INJAZ team</p>
      </div>

      {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 80 }}><Spinner size={36} /></div> : (() => {
        const cp = INTERSECTION_CHECKPOINTS[currentIdx] || INTERSECTION_CHECKPOINTS[0];
        return (
          <>
            <div className="card fade-up" style={{ padding: 36, marginBottom: 20, border: `1px solid ${cp.color}44`, position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 4, background: `linear-gradient(90deg,${cp.color},${cp.color}88)` }} />
              <div style={{ display: "flex", alignItems: "flex-start", gap: 24 }}>
                <div style={{ width: 64, height: 64, borderRadius: "50%", background: cp.colorLight, border: `2px solid ${cp.color}66`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, color: cp.color, flexShrink: 0 }}>{cp.icon}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                    <span style={{ fontSize: 11, background: cp.colorLight, color: cp.color, border: `1px solid ${cp.color}44`, borderRadius: 20, padding: "3px 12px", fontWeight: 700 }}>{cp.period}</span>
                    <span style={{ fontSize: 11, background: "rgba(201,168,76,0.12)", color: "#C9A84C", border: "1px solid rgba(201,168,76,0.3)", borderRadius: 20, padding: "3px 12px", fontWeight: 700 }}>CURRENT STAGE</span>
                  </div>
                  <div style={{ fontSize: 22, fontWeight: 700, color: "#F0EBE0", marginBottom: 12 }}>{cp.label}</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: meta.note ? 16 : 0 }}>
                    {cp.tasks.map(task => (
                      <div key={task} style={{ padding: "6px 14px", background: cp.colorLight, border: `1px solid ${cp.color}33`, borderRadius: 20, fontSize: 13, color: cp.color }}>
                        {String.fromCharCode(8594)} {task}
                      </div>
                    ))}
                  </div>
                  {meta.note && (
                    <div style={{ padding: 14, background: "rgba(255,255,255,0.03)", borderRadius: 10, border: "1px solid rgba(255,255,255,0.06)" }}>
                      <div style={{ fontSize: 11, color: "#C9A84C", marginBottom: 6, letterSpacing: 1, textTransform: "uppercase" }}>Message from INJAZ Team</div>
                      <div style={{ fontSize: 14, color: "#8A9BB5", lineHeight: 1.7 }}>{meta.note}</div>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="card fade-up-1" style={{ padding: 28, marginBottom: 16 }}>
              <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 24 }}>Full Life Cycle</div>
              <div style={{ position: "relative", marginBottom: 28 }}>
                <div style={{ height: 3, background: "rgba(255,255,255,0.06)", borderRadius: 99 }}>
                  <div style={{ width: `${pct}%`, height: "100%", background: "linear-gradient(90deg,#C9A84C,#E8C96A)", borderRadius: 99, boxShadow: "0 0 10px rgba(201,168,76,0.5)" }} />
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", position: "absolute", top: "50%", transform: "translateY(-50%)", width: "100%" }}>
                  {INTERSECTION_CHECKPOINTS.map((c, i) => (
                    <div key={c.id} style={{ width: 18, height: 18, borderRadius: "50%", background: i <= currentIdx ? c.color : "rgba(255,255,255,0.08)", border: `2px solid ${i <= currentIdx ? c.color : "rgba(255,255,255,0.12)"}`, boxShadow: i === currentIdx ? `0 0 16px ${c.color}88` : "none" }} />
                  ))}
                </div>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                {INTERSECTION_CHECKPOINTS.map((c, i) => (
                  <div key={c.id} style={{ textAlign: "center", flex: 1 }}>
                    <div style={{ fontSize: 10, color: i === currentIdx ? c.color : "#4A5A72", fontWeight: 700, letterSpacing: 1, marginBottom: 4 }}>{c.period}</div>
                    <div style={{ fontSize: 11, color: i === currentIdx ? c.color : i < currentIdx ? "#8A9BB5" : "#4A5A72", fontWeight: i === currentIdx ? 700 : 400 }}>{c.label}</div>
                    {i < currentIdx && <div style={{ fontSize: 10, color: "#27AE60", marginTop: 4 }}>Done</div>}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="card fade-up-2" style={{ padding: 28 }}>
              <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>All Stages</div>
              {INTERSECTION_CHECKPOINTS.map((c, i) => (
                <div key={c.id} style={{ display: "flex", gap: 16, padding: "14px 0", borderBottom: i < 4 ? "1px solid rgba(255,255,255,0.05)" : "none", opacity: i > currentIdx ? 0.4 : 1 }}>
                  <div style={{ width: 36, height: 36, borderRadius: "50%", background: i < currentIdx ? c.color : i === currentIdx ? c.colorLight : "rgba(255,255,255,0.04)", border: `1px solid ${i <= currentIdx ? c.color : "rgba(255,255,255,0.08)"}44`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, color: i < currentIdx ? "#080F1E" : c.color, flexShrink: 0, fontWeight: 700 }}>
                    {i < currentIdx ? "✓" : c.icon}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                      <span style={{ fontSize: 14, fontWeight: 600, color: i === currentIdx ? c.color : i < currentIdx ? "#8A9BB5" : "#4A5A72" }}>{c.label}</span>
                      <span style={{ fontSize: 10, color: "#4A5A72" }}>{c.period}</span>
                      {i === currentIdx && <span style={{ fontSize: 10, background: c.colorLight, color: c.color, border: `1px solid ${c.color}44`, borderRadius: 20, padding: "2px 8px", fontWeight: 700 }}>NOW</span>}
                    </div>
                    <div style={{ fontSize: 12, color: "#4A5A72" }}>{c.tasks.join(" · ")}</div>
                  </div>
                </div>
              ))}
            </div>
...
// ── INJAZ Team Checkpoint Control ─────────────────────────────
function CheckpointManager({ isInjazTeam = false }) {
  const [checkpoint, setCheckpoint] = useState(null);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ checkpoint: "foundation", status: "in_progress", note: "" });
  const { showSuccess, showError } = useAuth();

  useEffect(() => {
    const _t = setTimeout(() => setLoading(false), 7000);
    db.getProgramCheckpoint().then(d => {
      setCheckpoint(d);
      if (d) setForm({ checkpoint: d.value || "foundation", status: d.meta?.status || "in_progress", note: d.meta?.note || "" });
      setLoading(false);
  ...
  const currentId = checkpoint?.value || "foundation";
  const currentCp = INTERSECTION_CHECKPOINTS.find(c => c.id === currentId) || INTERSECTION_CHECKPOINTS[0];
  const lastUpdated = checkpoint?.meta?.updated_at ? new Date(checkpoint.meta.updated_at).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" }) : "Never updated";

  const statusOptions = [
    { id: "in_progress", label: "In Progress", color: "#2980B9" },
    { id: "completed", label: "Stage Complete", color: "#27AE60" },
    { id: "paused", label: "Paused", color: "#E67E22" },
  ];
  ...
  return (
    <div>
      <div style={{ marginBottom: 32 }}>
        ...
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 6 }}>Checkpoint Control</h1>
        <p style={{ color: "#8A9BB5", fontSize: 14 }}>Set the active program stage — all participants and trainers see this instantly</p>
      </div>
      ...
      {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 80 }}><Spinner size={36} /></div> : (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 20, alignItems: "start" }}>
          <div className="card fade-up" style={{ padding: 28 }}>
              ...
              <div style={{ display: "flex", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: 4, gap: 4 }}>
                {statusOptions.map(s => (
                  <button key={s.id} onClick={() => setForm(f => ({ ...f, status: s.id }))}
                    style={{ flex: 1, padding: "9px 8px", borderRadius: 7, border: "none", background: form.status === s.id ? `${s.color}33` : "transparent", color: form.status === s.id ? s.color : "#8A9BB5", fontSize: 12, fontWeight: form.status === s.id ? 700 : 500, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", transition: "all .2s", boxShadow: form.status === s.id ? `inset 0 0 0 1px ${s.color}55` : "none" }}>
                    {s.label}
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginBottom: 24 }}>
              <Label>Message to All Participants</Label>
              <textarea className="input-field" rows={3} style={{ resize: "vertical" }}
                placeholder="Optional message shown to all participants and trainers..."
                value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} />
            </div>

            <button className="btn-primary" style={{ width: "100%", justifyContent: "center", padding: "13px", fontSize: 15 }} onClick={handleSave} disabled={saving}>
              {saving ? <><Spinner size={16} color="#080F1E" /> Updating...</> : "Update Program Checkpoint"}
            </button>
          </div>

          <div>
            <div className="card fade-up-1" style={{ padding: 24, marginBottom: 16, border: `1px solid ${currentCp.color}33` }}>
              <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>Live Status</div>
              <div style={{ textAlign: "center", padding: "16px 0" }}>
                <div style={{ fontSize: 48, color: currentCp.color, marginBottom: 12 }}>{currentCp.icon}</div>
                <div style={{ fontSize: 11, color: currentCp.color, fontWeight: 600, letterSpacing: 1, marginBottom: 6 }}>{currentCp.period}</div>
                <div style={{ fontSize: 18, fontWeight: 700, color: "#F0EBE0", marginBottom: 8 }}>{currentCp.label}</div>
                <div style={{ fontSize: 12, color: "#4A5A72" }}>{lastUpdated}</div>
              </div>
              <div style={{ height: 1, background: "rgba(255,255,255,0.06)", margin: "16px 0" }} />
              <div style={{ fontSize: 12, color: "#8A9BB5", textAlign: "center" }}>Visible to all participants and trainers</div>
            </div>
            <div className="card fade-up-2" style={{ padding: 20 }}>
              <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>Timeline</div>
              {INTERSECTION_CHECKPOINTS.map((cp, i) => {
                const currentCheckpointIdx = INTERSECTION_CHECKPOINTS.findIndex(c => c.id === currentId);
                const done = i < currentCheckpointIdx;
                const active = i === currentCheckpointIdx;
                return (
                  <div key={cp.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0" }}>
                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: done || active ? cp.color : "rgba(255,255,255,0.1)", flexShrink: 0, boxShadow: active ? `0 0 8px ${cp.color}` : "none" }} />
                    <span style={{ fontSize: 12, color: done ? "#8A9BB5" : active ? cp.color : "#4A5A72", fontWeight: active ? 700 : 400 }}>{cp.label}</span>
                    {done && <span style={{ fontSize: 10, color: "#27AE60", marginLeft: "auto" }}>Done</span>}
                    {active && <span style={{ fontSize: 10, color: cp.color, marginLeft: "auto", fontWeight: 700 }}>NOW</span>}
                  </div>
                );
              })}
            </div>
...
// ══════════════════════════════════════════════════════════════
//  ATTENDANCE SHEET PAGES
// ══════════════════════════════════════════════════════════════

// ── Seeker Attendance Sheet ───────────────────────────────────
function MyAttendance() {
  ...
  const [attendanceTab, setAttendanceTab] = useState("sessions");
  const [reflections, setReflections] = useState([]);
  const [showReflection, setShowReflection] = useState(false);
  const [reflectionSession, setReflectionSession] = useState(null);
  const [newSession, setNewSession] = useState({
    session_name: "", type: "training", mentorship_format: "group",
    date: new Date().toISOString().split("T")[0], modality: "in_person", notes: ""
  });
  ...
  const loadSessions = async () => {
    ...
    const [data, refs] = await Promise.all([
      ...
      db.getMyReflections(profile.id).catch(() => []),
    ]);
    setSessions(data);
    setReflections(refs);
    setLoading(false);
  ...
  const handleAdd = async () => {
    if (!profile?.id) { showError("Profile not loaded yet. Please refresh."); return; }
    if (!newSession.session_name.trim() || !newSession.date) {
      showError("Please fill in session name and date.");
      return;
    }
    setAdding(true);
    try {
      const added = await db.addAttendanceSession({
        ...
        mentorship_format: newSession.type === "mentorship" ? newSession.mentorship_format : null,
        date: newSession.date,
        modality: newSession.modality,
        notes: newSession.notes,
        verified: false,
      });
      setNewSession({ session_name: "", type: "training", mentorship_format: "group", date: new Date().toISOString().split("T")[0], modality: "in_person", notes: "" });
      await loadSessions();
      showSuccess("Session added!");
      // Trigger reflection card for 1-on-1 mentorship
      if (newSession.type === "mentorship" && newSession.mentorship_format === "one_on_one" && added) {
        const oneOnOneSessions = sessions.filter(s => s.type === "mentorship" && s.mentorship_format === "one_on_one").length + 1;
  ...
  const training = sessions.filter(s => s.type === "training").length;
  const mentorship = sessions.filter(s => s.type === "mentorship").length;
  ...
  const getCategory = (pct) => {
    ...
    if (pct >= 75) return { label: "Good", color: "#2980B9" };
    if (pct >= 60) return { label: "Satisfactory", color: "#C9A84C" };
    if (pct >= 40) return { label: "Needs Improvement", color: "#E67E22" };
    return { label: "At Risk", color: "#C8392B" };
  };
  const category = getCategory(verifiedPct);

  const typeColors = { training: "#2980B9", mentorship: "#8E44AD" };
  const modalityColors = { in_person: "#27AE60", online: "#C9A84C" };

  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>◈ Intersection Program</div>
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 26, fontWeight: 700, color: "#F0EBE0", marginBottom: 6 }}>My Attendance</h1>
        <p style={{ color: "#8A9BB5", fontSize: 14 }}>Log your sessions and view your reflection cards</p>
      </div>
      <div className="tab-bar" style={{ marginBottom: 24 }}>
        {[["sessions", "◈ Sessions"], ["reflections", "◉ Reflection Cards"]].map(([t, l]) => (
          <button key={t} className={`tab ${attendanceTab === t ? "active" : ""}`} onClick={() => setAttendanceTab(t)}>{l}</button>
        ))}
      </div>
      ...
      <div className="grid-4" style={{ marginBottom: 24 }}>
        {[
          { label: "Total Sessions", value: total, color: "#C9A84C", icon: "◈" },
          { label: "Training", value: training, color: "#2980B9", icon: "◆" },
          { label: "Mentorship", value: mentorship, color: "#8E44AD", icon: "◉" },
          { label: "Verified", value: verified, color: "#27AE60", icon: "✓" },
        ].map(s => (
      ...
      {/* Attendance % card */}
      {total > 0 && (
        <div className="card fade-up" style={{ padding: 28, marginBottom: 20, border: `1px solid ${category.color}44` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <div>
              <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 6 }}>Attendance Rate</div>
              <div style={{ fontSize: 36, fontWeight: 700, color: category.color }}>{verifiedPct}%</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: category.color, marginBottom: 4 }}>{category.label}</div>
              <div style={{ fontSize: 12, color: "#4A5A72" }}>{verified} of {total} sessions verified</div>
              {pending > 0 && <div style={{ fontSize: 12, color: "#C9A84C", marginTop: 4 }}>{pending} pending verification</div>}
            </div>
          </div>
          <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 99, height: 10, overflow: "hidden" }}>
            <div style={{ width: `${verifiedPct}%`, height: "100%", background: `linear-gradient(90deg,${category.color},${category.color}CC)`, borderRadius: 99, transition: "width .8s ease", boxShadow: `0 0 10px ${category.color}44` }} />
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, fontSize: 11, color: "#4A5A72" }}>
            {[["0-39%", "#C8392B"], ["40-59%", "#E67E22"], ["60-74%", "#C9A84C"], ["75-89%", "#2980B9"], ["90-100%", "#27AE60"]].map(([range, color]) => (
      ...
      <div className="card fade-up-1" style={{ padding: 28, marginBottom: 20 }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>⊕ Add New Session</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
          <div style={{ gridColumn: "1 / -1" }}>
            <Label required>Session Name</Label>
            <input className="input-field" placeholder="e.g. Career Workshop #1, Mentor Meeting with Sara..." value={newSession.session_name} onChange={e => setNewSession(s => ({ ...s, session_name: e.target.value }))} />
          </div>

          <div>
            <Label required>Type</Label>
            <div style={{ display: "flex", gap: 8 }}>
              {[["training", "◆ Training"], ["mentorship", "◉ Mentorship"]].map(([v, l]) => (
                <button key={v} onClick={() => setNewSession(s => ({ ...s, type: v }))}
                  style={{ flex: 1, padding: "10px 8px", borderRadius: 10, border: `1px solid ${newSession.type === v ? typeColors[v] + "66" : "rgba(255,255,255,0.1)"}`, background: newSession.type === v ? `${typeColors[v]}22` : "rgba(255,255,255,0.03)", color: newSession.type === v ? typeColors[v] : "#8A9BB5", fontSize: 13, fontWeight: newSession.type === v ? 700 : 400, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", transition: "all .2s" }}>
                  {l}
                </button>
              ))}
            </div>
          ...
          {newSession.type === "mentorship" && (
            <div>
              <Label>Mentorship Format</Label>
              <div style={{ display: "flex", gap: 8 }}>
                {[["group", "Group"], ["one_on_one", "1-on-1"]].map(([v, l]) => (
                  <button key={v} onClick={() => setNewSession(s => ({ ...s, mentorship_format: v }))}
                    style={{ flex: 1, padding: "10px 8px", borderRadius: 10, border: `1px solid ${newSession.mentorship_format === v ? "#8E44AD66" : "rgba(255,255,255,0.1)"}`, background: newSession.mentorship_format === v ? "rgba(142,68,173,0.15)" : "rgba(255,255,255,0.03)", color: newSession.mentorship_format === v ? "#C39BD3" : "#8A9BB5", fontSize: 13, fontWeight: newSession.mentorship_format === v ? 700 : 400, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", transition: "all .2s" }}>
                    {l}
                  </button>
                ))}
              </div>
          ...
          <div>
            ...
            <input className="input-field" type="date" value={newSession.date} onChange={e => setNewSession(s => ({ ...s, date: e.target.value }))} />
          </div>

          <div>
            <Label>Modality</Label>
            <div style={{ display: "flex", gap: 8 }}>
              {[["in_person", "◆ In Person"], ["online", "◉ Online"]].map(([v, l]) => (
                <button key={v} onClick={() => setNewSession(s => ({ ...s, modality: v }))}
                  style={{ flex: 1, padding: "10px 8px", borderRadius: 10, border: `1px solid ${newSession.modality === v ? modalityColors[v] + "66" : "rgba(255,255,255,0.1)"}`, background: newSession.modality === v ? `${modalityColors[v]}22` : "rgba(255,255,255,0.03)", color: newSession.modality === v ? modalityColors[v] : "#8A9BB5", fontSize: 13, fontWeight: newSession.modality === v ? 700 : 400, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", transition: "all .2s" }}>
                  {l}
                </button>
              ))}
            </div>
          ...
          <div style={{ gridColumn: "1 / -1" }}>
            ...
            <input className="input-field" placeholder="Brief description of the session..." value={newSession.notes} onChange={e => setNewSession(s => ({ ...s, notes: e.target.value }))} />
          </div>
        </div>

        <button className="btn-primary" style={{ display: "flex", alignItems: "center", gap: 8, padding: "11px 24px", fontSize: 14 }} onClick={handleAdd} disabled={adding}>
          {adding ? <Spinner size={16} color="#080F1E" /> : "⊕"} Add Session
        </button>
      </div>

      {/* Sessions list */}
      <div className="card fade-up-2" style={{ padding: 0, overflow: "hidden" }}>
        <div style={{ padding: "20px 24px", borderBottom: "1px solid rgba(201,168,76,0.12)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase" }}>◈ Session Log ({total})</div>
          {pending > 0 && <span style={{ fontSize: 12, color: "#C9A84C", background: "rgba(201,168,76,0.1)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 20, padding: "3px 12px" }}>{pending} awaiting verification</span>}
        </div>

        {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 60 }}><Spinner size={32} /></div>
          : sessions.length === 0 ? (
            <div style={{ textAlign: "center", padding: 60, color: "#4A5A72" }}>
              <div style={{ fontSize: 36, marginBottom: 12, opacity: .3 }}>◈</div>
              <div style={{ marginBottom: 8 }}>No sessions logged yet</div>
              <div style={{ fontSize: 13 }}>Add your first session above</div>
            </div>
          ) : (
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: "1px solid rgba(201,168,76,0.12)", background: "rgba(201,168,76,0.04)" }}>
                    {["Session", "Type", "Format", "Date", "Modality", "Status", ""].map(h => (
                      <th key={h} style={{ padding: "12px 16px", textAlign: "left", color: "#C9A84C", fontWeight: 600, fontSize: 10, textTransform: "uppercase", letterSpacing: .8, whiteSpace: "nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {sessions.map(s => (
                    <tr key={s.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}
                      ...
                      onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                      <td style={{ padding: "12px 16px" }}>
                        <div style={{ fontWeight: 600, color: "#F0EBE0", marginBottom: 2 }}>{s.session_name}</div>
                        {s.notes && <div style={{ fontSize: 11, color: "#4A5A72" }}>{s.notes}</div>}
                      </td>
                      <td style={{ padding: "12px 16px" }}>
                        <span style={{ fontSize: 11, background: `${typeColors[s.type]}22`, color: typeColors[s.type], border: `1px solid ${typeColors[s.type]}44`, borderRadius: 20, padding: "3px 10px", fontWeight: 600, textTransform: "capitalize" }}>{s.type}</span>
                      </td>
                      <td style={{ padding: "12px 16px", color: "#8A9BB5", fontSize: 12 }}>
                        {s.type === "mentorship" ? (s.mentorship_format === "one_on_one" ? "1-on-1" : "Group") : "—"}
                      </td>
                      <td style={{ padding: "12px 16px", color: "#8A9BB5", whiteSpace: "nowrap" }}>
                        {s.date ? new Date(s.date).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" }) : "—"}
                      </td>
                      <td style={{ padding: "12px 16px" }}>
                        <span style={{ fontSize: 11, background: `${modalityColors[s.modality]}22`, color: modalityColors[s.modality], border: `1px solid ${modalityColors[s.modality]}44`, borderRadius: 20, padding: "3px 10px" }}>
                          {s.modality === "in_person" ? "In Person" : "Online"}
                        </span>
                      </td>
                      <td style={{ padding: "12px 16px" }}>
                        {s.verified
                          ? <span style={{ fontSize: 11, background: "rgba(39,174,96,0.12)", color: "#4AE08A", border: "1px solid rgba(39,174,96,0.3)", borderRadius: 20, padding: "3px 10px" }}>✓ Verified</span>
                          : <span style={{ fontSize: 11, background: "rgba(201,168,76,0.1)", color: "#C9A84C", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 20, padding: "3px 10px" }}>Pending</span>}
                      </td>
                      <td style={{ padding: "12px 16px" }}>
                        {!s.verified && (
                          <button onClick={() => handleDelete(s.id)} disabled={deleting === s.id}
                            style={{ background: "rgba(200,57,43,0.1)", border: "1px solid rgba(200,57,43,0.2)", color: "#FF8A80", borderRadius: 8, padding: "4px 12px", fontSize: 12, cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}>
                            {deleting === s.id ? <Spinner size={12} color="#FF8A80" /> : "Remove"}
                          </button>
                        )}
                      </td>
                    </tr>
      ...
      {/* Reflections tab */}
      {attendanceTab === "reflections" && (
        <div>
          {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 60 }}><Spinner size={32} /></div>
            : reflections.length === 0 ? (
              <div className="card" style={{ textAlign: "center", padding: 60, color: "#4A5A72" }}>
                <div style={{ fontSize: 36, marginBottom: 12, opacity: .3 }}>◉</div>
                <div style={{ marginBottom: 8, color: "#8A9BB5" }}>No reflection cards yet</div>
                <div style={{ fontSize: 13 }}>Fill a reflection card after your next 1-on-1 mentorship session</div>
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {reflections.map((r, idx) => (
                  <div key={r.id} className="card" style={{ padding: 28, border: "1px solid rgba(142,68,173,0.2)" }}>
                    {/* Card header */}
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, paddingBottom: 16, borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                      <div>
                        <div style={{ fontSize: 11, color: "#8E44AD", letterSpacing: 2, textTransform: "uppercase", marginBottom: 4 }}>◉ Reflection Card</div>
                        <div style={{ fontWeight: 700, fontSize: 16, color: "#F0EBE0" }}>Session #{r.session_number || idx + 1}</div>
                        {r.mentor_name && <div style={{ fontSize: 13, color: "#8A9BB5", marginTop: 2 }}>Mentor: {r.mentor_name}</div>}
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontSize: 13, color: "#8A9BB5" }}>{r.session_date ? new Date(r.session_date).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" }) : "—"}</div>
                        {r.rating && (
                          <div style={{ marginTop: 6, display: "flex", gap: 3, justifyContent: "flex-end" }}>
                            {[1,2,3,4,5].map(s => (
                              <span key={s} style={{ fontSize: 16, color: s <= r.rating ? "#C9A84C" : "rgba(255,255,255,0.15)" }}>★</span>
                            ))}
                          </div>
                        )}
                      </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                      ...
                      {r.takeaways?.length > 0 && (
                        <div>
                          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}>
                      ...
                      {r.action_steps?.length > 0 && (
                        <div>
                          <div style={{ fontSize: 11, color: "#2980B9", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    {r.personal_reflection && (
                      <div style={{ marginTop: 16, padding: 16, background: "rgba(142,68,173,0.08)", border: "1px solid rgba(142,68,173,0.15)", borderRadius: 10 }}>
                        <div style={{ fontSize: 11, color: "#8E44AD", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>
      ...
      {showReflection && reflectionSession && (
        <ReflectionCardModal
          session={reflectionSession}
          sessionNumber={sessions.filter(s => s.type === "mentorship" && s.mentorship_format === "one_on_one").length}
          onClose={() => { setShowReflection(false); setReflectionSession(null); }}
          onSave={() => loadSessions()}
        />
      )}
    </div>
...
// ── INJAZ Team / Trainer Attendance Verification ──────────────
function AttendanceVerification({ isInjazTeam = false }) {
  ...
  const loadData = async () => {
    const data = await Promise.race([db.getAllAttendance(), new Promise(r=>setTimeout(()=>r([]),7000))]).catch(()=>[]);
    setAllSessions(data);
    setLoading(false);
  };
  ...
  const filtered = allSessions.filter(s => {
    const matchStatus = filterStatus === "all" || (filterStatus === "pending" ? !s.verified : s.verified);
    const matchType = filterType === "all" || s.type === filterType;
    const matchSearch = !search || (s.job_seekers?.full_name || "").toLowerCase().includes(search.toLowerCase()) || (s.session_name || "").toLowerCase().includes(search.toLowerCase());
    return matchStatus && matchType && matchSearch;
  });

  // Summary stats
  const total = allSessions.length;
  const pending = allSessions.filter(s => !s.verified).length;
  const verified = allSessions.filter(s => s.verified).length;
  const training = allSessions.filter(s => s.type === "training").length;
  const mentorship = allSessions.filter(s => s.type === "mentorship").length;

  // Per-participant attendance summary
  const participantStats = {};
  allSessions.forEach(s => {
    const name = s.job_seekers?.full_name || "Unknown";
    if (!participantStats[name]) participantStats[name] = { total: 0, verified: 0, training: 0, mentorship: 0 };
    participantStats[name].total++;
    if (s.verified) participantStats[name].verified++;
    if (s.type === "training") participantStats[name].training++;
    if (s.type === "mentorship") participantStats[name].mentorship++;
  });
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>◈ Intersection Program</div>
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 6 }}>Attendance Verification</h1>
        <p style={{ color: "#8A9BB5", fontSize: 14 }}>Review and verify participant attendance sessions</p>
      </div>
      ...
      <div className="grid-4" style={{ marginBottom: 24 }}>
        {[
          { label: "Total Sessions", value: total, color: "#C9A84C", icon: "◈" },
          { label: "Pending Verification", value: pending, color: "#E67E22", icon: "◐" },
          { label: "Verified", value: verified, color: "#27AE60", icon: "✓" },
          { label: "Training / Mentorship", value: `${training}/${mentorship}`, color: "#2980B9", icon: "◆" },
        ].map(s => (
          <div key={s.label} className="card" style={{ padding: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
              <span style={{ fontSize: 18, color: s.color }}>{s.icon}</span>
            ...
            <div style={{ fontSize: 26, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>{s.value}</div>
            <div style={{ fontSize: 12, color: "#8A9BB5" }}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      {Object.keys(participantStats).length > 0 && (
        <div className="card fade-up" style={{ padding: 28, marginBottom: 20 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>✦ Attendance by Participant</div>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(201,168,76,0.12)" }}>
                  {["Participant", "Total Sessions", "Verified", "Training", "Mentorship", "Attendance %", "Category"].map(h => (
                    <th key={h} style={{ padding: "10px 14px", textAlign: "left", color: "#C9A84C", fontWeight: 600, fontSize: 10, textTransform: "uppercase", letterSpacing: .8, whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {Object.entries(participantStats).map(([name, stats]) => {
                  ...
                  return (
                    <tr key={name} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                      <td style={{ padding: "12px 14px", fontWeight: 600, color: "#F0EBE0" }}>{name}</td>
                      <td style={{ padding: "12px 14px", color: "#F0EBE0", fontWeight: 700 }}>{stats.total}</td>
                      <td style={{ padding: "12px 14px", color: "#4AE08A" }}>{stats.verified}</td>
                      <td style={{ padding: "12px 14px", color: "#74B9FF" }}>{stats.training}</td>
                      <td style={{ padding: "12px 14px", color: "#C39BD3" }}>{stats.mentorship}</td>
                      <td style={{ padding: "12px 14px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{ width: 60, background: "rgba(255,255,255,0.06)", borderRadius: 99, height: 5 }}>
                      ...
                      <td style={{ padding: "12px 14px" }}>
                        <span style={{ fontSize: 11, background: `${cat.color}22`, color: cat.color, border: `1px solid ${cat.color}44`, borderRadius: 20, padding: "3px 10px", fontWeight: 600, whiteSpace: "nowrap" }}>{cat.label}</span>
                      </td>
                    </tr>
                  );
                })}
      ...
      {/* Filters */}
      <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
        <input className="input-field" style={{ flex: 1, minWidth: 200 }} placeholder="Search participant or session..." value={search} onChange={e => setSearch(e.target.value)} />
        <div className="tab-bar">
          {[["pending", "Pending"], ["verified", "Verified"], ["all", "All"]].map(([f, l]) => (
            <button key={f} className={`tab ${filterStatus === f ? "active" : ""}`} onClick={() => setFilterStatus(f)}>{l}</button>
          ))}
        </div>
        <div className="tab-bar">
          {[["all", "All Types"], ["training", "Training"], ["mentorship", "Mentorship"]].map(([f, l]) => (
            <button key={f} className={`tab ${filterType === f ? "active" : ""}`} onClick={() => setFilterType(f)}>{l}</button>
          ))}
        </div>
      </div>
      ...
      {/* Sessions table */}
      <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        <div style={{ padding: "16px 20px", borderBottom: "1px solid rgba(201,168,76,0.12)", display: "flex", justifyContent: "space-between" }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase" }}>Sessions ({filtered.length})</div>
          {pending > 0 && filterStatus === "pending" && (
            <button className="btn-primary" style={{ fontSize: 12, padding: "6px 16px" }}
              onClick={async () => {
                const pendingSessions = filtered.filter(s => !s.verified);
                for (const s of pendingSessions) await handleVerify(s.id);
              }}>
        ...
        {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 60 }}><Spinner size={32} /></div>
          : filtered.length === 0 ? <div style={{ textAlign: "center", padding: 60, color: "#4A5A72" }}>No sessions found.</div>
          : <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid rgba(201,168,76,0.12)", background: "rgba(201,168,76,0.04)" }}>
                  {["Participant", "Session", "Type", "Format", "Date", "Modality", "Status", "Action"].map(h => (
                    <th key={h} style={{ padding: "12px 16px", textAlign: "left", color: "#C9A84C", fontWeight: 600, fontSize: 10, textTransform: "uppercase", letterSpacing: .8, whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map(s => (
                  <tr key={s.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}
                    onMouseEnter={e => e.currentTarget.style.background = "rgba(201,168,76,0.02)"}
                    onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                    <td style={{ padding: "12px 16px", fontWeight: 600, color: "#F0EBE0" }}>{s.job_seekers?.full_name || "—"}</td>
                    <td style={{ padding: "12px 16px" }}>
                      <div style={{ color: "#F0EBE0", marginBottom: 2 }}>{s.session_name}</div>
                      {s.notes && <div style={{ fontSize: 11, color: "#4A5A72" }}>{s.notes}</div>}
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      <span style={{ fontSize: 11, background: `${typeColors[s.type]}22`, color: typeColors[s.type], border: `1px solid ${typeColors[s.type]}44`, borderRadius: 20, padding: "3px 10px", textTransform: "capitalize" }}>{s.type}</span>
                    </td>
                    <td style={{ padding: "12px 16px", color: "#8A9BB5", fontSize: 12 }}>
                      {s.type === "mentorship" ? (s.mentorship_format === "one_on_one" ? "1-on-1" : "Group") : "—"}
                    </td>
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      {s.verified
                        ? <span style={{ fontSize: 11, background: "rgba(39,174,96,0.12)", color: "#4AE08A", border: "1px solid rgba(39,174,96,0.3)", borderRadius: 20, padding: "3px 10px" }}>✓ Verified</span>
                        : <span style={{ fontSize: 11, background: "rgba(230,126,34,0.12)", color: "#F0A500", border: "1px solid rgba(230,126,34,0.3)", borderRadius: 20, padding: "3px 10px" }}>Pending</span>}
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      {!s.verified && (
                        <button className="btn-primary" style={{ fontSize: 12, padding: "6px 14px" }} onClick={() => handleVerify(s.id)} disabled={verifying === s.id}>
                          {verifying === s.id ? <Spinner size={12} color="#080F1E" /> : "Verify ✓"}
                        </button>
                      )}
                      {s.verified && s.verified_at && (
                        <div style={{ fontSize: 11, color: "#4A5A72" }}>{new Date(s.verified_at).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</div>
                      )}
                    </td>
                  </tr>
                ))}
...
function ReflectionCardModal({ session, sessionNumber, onClose, onSave }) {
  ...
  const handleSave = async () => {
    ...
    try {
      await db.saveReflection({
        ...
        takeaways: [form.takeaway_1, form.takeaway_2].filter(Boolean),
        action_steps: [form.action_step_1, form.action_step_2, form.action_step_3].filter(Boolean),
        personal_reflection: form.personal_reflection,
        rating,
      });
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300, padding: 20, backdropFilter: "blur(10px)" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 580, maxHeight: "90vh", overflowY: "auto", background: "#0D1829", border: "1px solid rgba(201,168,76,0.3)", borderRadius: 20, padding: 0, boxShadow: "0 24px 80px rgba(0,0,0,0.6)" }}>
        
        {/* Header */}
        <div style={{ padding: "28px 32px 20px", borderBottom: "1px solid rgba(201,168,76,0.15)", background: "rgba(201,168,76,0.06)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 6 }}>◉ Intersection Mentorship</div>
              <h2 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 20, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>Session Reflection Card</h2>
              <div style={{ fontSize: 13, color: "#8A9BB5" }}>
                Session #{sessionNumber} · {session.date ? new Date(session.date).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" }) : "—"}
              </div>
            </div>
            <button onClick={onClose} style={{ background: "none", border: "none", color: "#4A5A72", fontSize: 24, cursor: "pointer", lineHeight: 1 }}>×</button>
          </div>
        </div>
        ...
        <div style={{ padding: "24px 32px", display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          {/* Mentor Name */}
          <div>
            <label className="lbl">Mentor Name</label>
            <input className="input-field" placeholder="Your mentor's name..." value={form.mentor_name} onChange={e => set("mentor_name", e.target.value)} />
          </div>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
              ...
              <div>
                <div style={{ fontWeight: 700, fontSize: 15, color: "#F0EBE0" }}>1. Key Takeaways</div>
                <div style={{ fontSize: 12, color: "#8A9BB5" }}>What stood out the most? What did I learn?</div>
              </div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {["takeaway_1", "takeaway_2"].map((key, i) => (
                <div key={key} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ color: "#C9A84C", fontWeight: 700, fontSize: 14, flexShrink: 0 }}>→</span>
                  <input className="input-field" placeholder={`Takeaway ${i + 1}...`} value={form[key]} onChange={e => set(key, e.target.value)} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
              <span style={{ fontSize: 16, color: "#2980B9" }}>◈</span>
              <div>
                <div style={{ fontWeight: 700, fontSize: 15, color: "#F0EBE0" }}>2. Action Steps</div>
                <div style={{ fontSize: 12, color: "#8A9BB5" }}>What will I do before the next session? Be specific.</div>
              </div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {["action_step_1", "action_step_2", "action_step_3"].map((key, i) => (
                <input key={key} className="input-field" placeholder={`Action step ${i + 1}...`} value={form[key]} onChange={e => set(key, e.target.value)} />
              ))}
            </div>
          </div>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
              <span style={{ fontSize: 16, color: "#8E44AD" }}>◐</span>
              <div>
                <div style={{ fontWeight: 700, fontSize: 15, color: "#F0EBE0" }}>3. Personal Reflection</div>
                <div style={{ fontSize: 12, color: "#8A9BB5" }}>How did this session challenge or shift my thinking?</div>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
              <span style={{ color: "#C9A84C", fontWeight: 700, fontSize: 14, marginTop: 10, flexShrink: 0 }}>→</span>
              <textarea className="input-field" rows={3} style={{ resize: "vertical" }} placeholder="How did this session challenge or shift my thinking..." value={form.personal_reflection} onChange={e => set("personal_reflection", e.target.value)} />
            </div>
          </div>

          {/* Quick Rating */}
          <div style={{ background: "rgba(201,168,76,0.06)", border: "1px solid rgba(201,168,76,0.15)", borderRadius: 14, padding: "20px 24px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
              <span style={{ fontSize: 18, color: "#C9A84C" }}>★</span>
              <div style={{ fontWeight: 700, fontSize: 15, color: "#F0EBE0" }}>Quick Rating</div>
            </div>
            <div style={{ fontSize: 13, color: "#8A9BB5", marginBottom: 14 }}>Value of this session:</div>
            <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
              {[1, 2, 3, 4, 5].map(star => (
                <button key={star}
                  ...
                  style={{ background: "none", border: "none", cursor: "pointer", fontSize: 32, color: star <= (hoveredRating || rating) ? "#C9A84C" : "rgba(255,255,255,0.15)", transition: "all .15s", transform: star <= (hoveredRating || rating) ? "scale(1.15)" : "scale(1)" }}>
                  ★
                </button>
              ))}
              {rating > 0 && (
                <span style={{ fontSize: 13, color: "#C9A84C", fontWeight: 600, marginLeft: 8 }}>
          ...
          <div style={{ display: "flex", gap: 10, paddingTop: 8 }}>
            <button className="btn-primary" style={{ flex: 1, justifyContent: "center", padding: "12px" }} onClick={handleSave} disabled={saving}>
              {saving ? <Spinner size={16} color="#080F1E" /> : "✦ Save Reflection Card"}
            </button>
...
// ── Shared components ─────────────────────────────────────────
function StarRating({ value, onChange, max = 5, labels = [] }) {
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 6 }}>
        {Array.from({ length: max }, (_, i) => i + 1).map(n => (
          <button key={n} onMouseEnter={() => setHovered(n)} onMouseLeave={() => setHovered(0)} onClick={() => onChange(n)}
            style={{ background: "none", border: "none", cursor: "pointer", fontSize: 28, color: n <= (hovered || value) ? "#C9A84C" : "rgba(255,255,255,0.15)", transition: "all .15s", transform: n <= (hovered || value) ? "scale(1.15)" : "scale(1)", padding: 0 }}>★</button>
        ))}
        {value > 0 && labels[value - 1] && <span style={{ fontSize: 13, color: "#C9A84C", fontWeight: 600, marginLeft: 8 }}>{labels[value - 1]}</span>}
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: "#4A5A72", maxWidth: max * 38 }}>
        {labels.length > 0 && <><span>{labels[0]}</span><span>{labels[labels.length - 1]}</span></>}
      </div>
    </div>
  );
}
...
function FormSection({ icon, title, sub, children }) {
  return (
    <div style={{ marginBottom: 28 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: sub ? 6 : 16 }}>
        <span style={{ fontSize: 18, color: "#C9A84C" }}>{icon}</span>
        <div style={{ fontWeight: 700, fontSize: 16, color: "#F0EBE0" }}>{title}</div>
      </div>
      {sub && <div style={{ fontSize: 13, color: "#8A9BB5", marginBottom: 16, paddingLeft: 28 }}>{sub}</div>}
      {children}
    </div>
  );
}
...
function FormField({ label, required, children }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <label style={{ fontSize: 11, fontWeight: 600, color: "#8A9BB5", letterSpacing: .8, textTransform: "uppercase", display: "block", marginBottom: 8 }}>
...
// ── Assessment Hub (seeker) ────────────────────────────────────
function SeekerAssessments() {
  const { profile } = useAuth();
  ...
  useEffect(() => {
    if (profile?.id) {
      db.getMyAssessments(profile.id, "seeker").then(setSubmitted).catch(() => {}).finally(() => setLoading(false));
    } else setLoading(false);
  }, [profile?.id]);
  ...
  if (activeForm === "mentee_mid_program") return <MenteeMidProgramForm onBack={() => setActiveForm(null)} onSubmit={() => { setActiveForm(null); db.getMyAssessments(profile.id, "seeker").then(setSubmitted); }} />;
  if (activeForm === "mentee_final") return <MenteeFinalForm onBack={() => setActiveForm(null)} onSubmit={() => { setActiveForm(null); db.getMyAssessments(profile.id, "seeker").then(setSubmitted); }} />;
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {forms.map(form => (
          <div key={form.id} className="card" style={{ padding: 28, border: hasSubmitted(form.id) ? "1px solid rgba(39,174,96,0.3)" : `1px solid ${form.color}22`, display: "flex", justifyContent: "space-between", alignItems: "center", transition: "all .3s" }}>
            <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
              <div style={{ width: 52, height: 52, borderRadius: "50%", background: `${form.color}22`, border: `1px solid ${form.color}44`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, color: form.color, flexShrink: 0 }}>{form.icon}</div>
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                  <div style={{ fontWeight: 700, fontSize: 16, color: "#F0EBE0" }}>{form.title}</div>
                  <span style={{ fontSize: 10, background: `${form.color}22`, color: form.color, border: `1px solid ${form.color}44`, borderRadius: 20, padding: "2px 8px" }}>{form.timing}</span>
                </div>
                <div style={{ fontSize: 13, color: "#8A9BB5" }}>{form.desc}</div>
              </div>
            </div>
...
// ── Assessment Helper Components ────────────────────────────
function ScaleQuestion({ label, sub, value, onChange, min = 1, max = 5, minLabel, maxLabel }) {
  const [hovered, setHovered] = useState(0);
  return (
    <div style={{ marginBottom: 24 }}>
      <div style={{ fontWeight: 600, fontSize: 14, color: "#F0EBE0", marginBottom: 4 }}>{label}</div>
      {sub && <div style={{ fontSize: 12, color: "#8A9BB5", marginBottom: 12 }}>{sub}</div>}
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        {minLabel && <span style={{ fontSize: 11, color: "#4A5A72" }}>{minLabel}</span>}
        <div style={{ display: "flex", gap: 8 }}>
          {Array.from({ length: max - min + 1 }, (_, i) => i + min).map(n => (
            <button key={n} onMouseEnter={() => setHovered(n)} onMouseLeave={() => setHovered(0)} onClick={() => onChange(n)}
              style={{ width: 40, height: 40, borderRadius: 10, border: `1px solid ${value === n ? "rgba(201,168,76,0.6)" : "rgba(255,255,255,0.1)"}`, background: value === n ? "rgba(201,168,76,0.2)" : n <= hovered ? "rgba(201,168,76,0.08)" : "rgba(255,255,255,0.03)", color: value === n ? "#C9A84C" : "#8A9BB5", fontSize: 14, fontWeight: value === n ? 700 : 400, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", transition: "all .15s" }}>
              {n}
            </button>
          ))}
        </div>
        {maxLabel && <span style={{ fontSize: 11, color: "#4A5A72" }}>{maxLabel}</span>}
      </div>
    </div>
  );
}

function RadioQuestion({ label, sub, options, value, onChange }) {
  return (
    <div style={{ marginBottom: 24 }}>
      <div style={{ fontWeight: 600, fontSize: 14, color: "#F0EBE0", marginBottom: 4 }}>{label}</div>
      {sub && <div style={{ fontSize: 12, color: "#8A9BB5", marginBottom: 12 }}>{sub}</div>}
...
function TextQuestion({ label, sub, value, onChange, multiline = false, placeholder = "Your answer..." }) {
  return (
    <div style={{ marginBottom: 24 }}>
      <div style={{ fontWeight: 600, fontSize: 14, color: "#F0EBE0", marginBottom: 4 }}>{label}</div>
      {sub && <div style={{ fontSize: 12, color: "#8A9BB5", marginBottom: 10 }}>{sub}</div>}
      {multiline
        ? <textarea className="input-field" rows={3} style={{ resize: "vertical" }} placeholder={placeholder} value={value || ""} onChange={e => onChange(e.target.value)} />
        : <input className="input-field" placeholder={placeholder} value={value || ""} onChange={e => onChange(e.target.value)} />}
    </div>
  );
}

function AssessmentShell({ title, subtitle, icon, color, children, onSubmit, saving, submitted, submittedMsg }) {
  if (submitted) return (
    <div className="card" style={{ padding: 60, textAlign: "center", border: `1px solid ${color}44` }}>
      <div style={{ fontSize: 48, color, marginBottom: 16 }}>{icon}</div>
      <div style={{ fontSize: 22, fontWeight: 700, color: "#F0EBE0", marginBottom: 10 }}>Submitted!</div>
      <div style={{ fontSize: 14, color: "#8A9BB5" }}>{submittedMsg || "Thank you for your feedback."}</div>
    </div>
  );
  return (
    <div>
      <div className="card" style={{ padding: "24px 28px", marginBottom: 16, border: `1px solid ${color}33`, background: `rgba(255,255,255,0.02)` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <span style={{ fontSize: 28, color }}>{icon}</span>
          <div>
            <div style={{ fontSize: 11, color, letterSpacing: 2, textTransform: "uppercase", marginBottom: 4 }}>Intersection Mentorship Program</div>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>{title}</h2>
      ...
      <div className="card" style={{ padding: 32 }}>
        ...
        <button className="btn-primary" style={{ padding: "13px 32px", fontSize: 15 }} onClick={onSubmit} disabled={saving}>
          {saving ? <><Spinner size={16} color="#080F1E" /> Submitting...</> : "✦ Submit Assessment"}
        </button>
      </div>
    </div>
  );
...
function MyAssessments() {
  ...
  const formList = [
    ...
    { id: "mentee_mid_program", title: "Mid-Program Assessment", sub: "How is your mentorship journey going?", icon: "◐", color: "#C9A84C", timing: "Mid-program (Aug)" },
    { id: "mentee_final", title: "Final Program Assessment", sub: "Your overall program experience", icon: "★", color: "#8E44AD", timing: "End of program (Dec)" },
  ];

  const renderForm = (id) => {
    const done = () => { reload(); setActiveForm(null); };
    if (id === "mentee_first_meeting") return <MenteeFirstMeetingForm onBack={() => setActiveForm(null)} onSubmit={done} existing={has(id)} />;
    if (id === "mentee_mid_program") return <MenteeMidProgramForm onBack={() => setActiveForm(null)} onSubmit={done} existing={has(id)} />;
  ...
  if (activeForm) return (
    <div>
      <button className="btn-ghost" style={{ marginBottom: 20 }} onClick={() => setActiveForm(null)}>← Back to Assessments</button>
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>◈ Intersection Program</div>
        <h1 style={{ fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 6 }}>My Assessments</h1>
        <p style={{ color: "#8A9BB5", fontSize: 14 }}>Program feedback forms — for 1-on-1 mentorship participants only</p>
      </div>
      {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 60 }}><Spinner size={32} /></div>
        : <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {formList.map(form => {
            ...
            return (
              <div key={form.id} className="card" style={{ padding: "22px 28px", display: "flex", alignItems: "center", gap: 20, border: done ? "1px solid rgba(39,174,96,0.2)" : `1px solid ${form.color}22`, cursor: done ? "default" : "pointer", transition: "all .2s" }}
                ...
                <div style={{ width: 48, height: 48, borderRadius: "50%", background: done ? "rgba(39,174,96,0.12)" : `${form.color}22`, border: `1px solid ${done ? "rgba(39,174,96,0.3)" : form.color + "44"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, color: done ? "#4AE08A" : form.color, flexShrink: 0 }}>
                  {done ? "✓" : form.icon}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, fontSize: 15, color: "#F0EBE0", marginBottom: 3 }}>{form.title}</div>
                  <div style={{ fontSize: 13, color: "#8A9BB5", marginBottom: 4 }}>{form.sub}</div>
...
function MenteeFirstMeetingForm({ onBack, onSubmit }) {
  ...
  const [f, setF] = useState({
    ...
    career_goals: "", areas_of_support: "", expectations: "",
  });
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
        <button className="btn-ghost" style={{ fontSize: 13 }} onClick={onBack}>← Back</button>
        <div>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 4 }}>Mentee Assessment</div>
          <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 22, fontWeight: 700, color: "#F0EBE0" }}>First Meeting Feedback</h1>
        </div>
      </div>
      <div style={{ background: "rgba(39,174,96,0.06)", border: "1px solid rgba(39,174,96,0.15)", borderRadius: 12, padding: "14px 18px", marginBottom: 24, fontSize: 13, color: "#8A9BB5", lineHeight: 1.7 }}>
        We hope you enjoyed your first meeting with your mentor. Your input is extremely valuable for us to evaluate and measure the success of your journey. Thank you for taking 5-10 minutes to complete this survey.
      </div>
        ...
        <FormSection icon="◆" title="Your Information">
          <div className="grid-2" style={{ gap: 14 }}>
            ...
            <FormField label="Phone Number" required>
              <input className="input-field" value={f.phone} onChange={e => set("phone", e.target.value)} placeholder="+961..." />
            </FormField>
            <FormField label="Field of Study" required>
              <input className="input-field" value={f.field_of_study} onChange={e => set("field_of_study", e.target.value)} placeholder="e.g. Computer Science" />
            </FormField>
            <FormField label="University" required>
              <input className="input-field" value={f.university} onChange={e => set("university", e.target.value)} placeholder="e.g. AUB, LAU" />
            </FormField>
            <FormField label="Current Employer (if applicable)">
              <input className="input-field" value={f.current_employer} onChange={e => set("current_employer", e.target.value)} placeholder="Company name" />
            </FormField>
            <FormField label="Current Job Title (if applicable)">
              <input className="input-field" value={f.job_title} onChange={e => set("job_title", e.target.value)} placeholder="Job title" />
            </FormField>
          </div>
        </FormSection>

        <div style={{ height: 1, background: "rgba(201,168,76,0.1)", margin: "24px 0" }} />

        <FormSection icon="◈" title="Have you had your first meeting with your mentor?" required>
          <RadioGroup options={[{ value: "yes", label: "Yes" }, { value: "no", label: "No" }]} value={f.had_first_meeting} onChange={v => set("had_first_meeting", v)} />
        </FormSection>

        {f.had_first_meeting === "yes" && <>
          <div style={{ height: 1, background: "rgba(201,168,76,0.1)", margin: "24px 0" }} />
          <FormSection icon="◉" title="Your First Meeting with Your Mentor">
            <FormField label="Date of First Meeting" required>
              <input className="input-field" type="date" value={f.first_meeting_date} onChange={e => set("first_meeting_date", e.target.value)} />
            </FormField>
            <FormField label="Did you find your mentor encouraging and engaged with you?" required>
              <RadioGroup options={["Yes", "Somewhat", "Not sure", "No"]} value={f.mentor_encouraging} onChange={v => set("mentor_encouraging", v)} />
            </FormField>
            <FormField label="How do you describe your first meeting?" required>
              <textarea className="input-field" rows={3} style={{ resize: "vertical" }} value={f.meeting_description} onChange={e => set("meeting_description", e.target.value)} placeholder="Describe how the meeting went..." />
            </FormField>
            <FormField label="Did you agree with your mentor on the frequency of meetings?" required>
              <RadioGroup options={[{ value: "yes", label: "Yes" }, { value: "no", label: "No" }]} value={f.agreed_on_frequency} onChange={v => set("agreed_on_frequency", v)} />
            </FormField>
          </FormSection>

          <div style={{ height: 1, background: "rgba(201,168,76,0.1)", margin: "24px 0" }} />
          <FormSection icon="★" title="Tell us a bit more about yourself">
            <FormField label="What do you consider are your greatest strengths?" required>
              <textarea className="input-field" rows={3} style={{ resize: "vertical" }} value={f.greatest_strengths} onChange={e => set("greatest_strengths", e.target.value)} placeholder="Your strengths..." />
            </FormField>
            <FormField label="What are your major challenges you are trying to overcome?" required>
              <textarea className="input-field" rows={3} style={{ resize: "vertical" }} value={f.major_challenges} onChange={e => set("major_challenges", e.target.value)} placeholder="Your challenges..." />
            </FormField>
            <FormField label="What are your career goals for this program?" required>
              <textarea className="input-field" rows={3} style={{ resize: "vertical" }} value={f.career_goals} onChange={e => set("career_goals", e.target.value)} placeholder="Your goals..." />
            </FormField>
            <FormField label="What areas do you need the most support in?" required>
              <textarea className="input-field" rows={2} style={{ resize: "vertical" }} value={f.areas_of_support} onChange={e => set("areas_of_support", e.target.value)} placeholder="Areas needing support..." />
            </FormField>
            <FormField label="What are your expectations from this mentorship?">
              <textarea className="input-field" rows={2} style={{ resize: "vertical" }} value={f.expectations} onChange={e => set("expectations", e.target.value)} placeholder="Your expectations..." />
            </FormField>
          </FormSection>
        </>}

        <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
          <button className="btn-primary" style={{ flex: 1, justifyContent: "center", padding: "13px" }} onClick={handleSubmit} disabled={saving}>
            {saving ? <Spinner size={16} color="#080F1E" /> : "✦ Submit Assessment"}
          </button>
...
function MenteeMidProgramForm({ onBack, onSubmit }) {
  ...
  const [f, setF] = useState({
    ...
    avg_meeting_duration: "", num_meetings: "",
    meeting_format: [], communication_quality: 0, goal_progress: 0,
    mentor_support_rating: 0, overall_satisfaction: 0,
    biggest_achievement: "", challenges_faced: "", group_sessions_rating: {},
    gathering_satisfaction: 0, gathering_best: "", gathering_least: "",
    anything_else: "",
  });
  const set = (k, v) => setF(p => ({ ...p, [k]: v }));
  const sat5 = ["Not satisfied", "Fair", "Good", "Very good", "Super satisfied!"];
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
        <button className="btn-ghost" style={{ fontSize: 13 }} onClick={onBack}>← Back</button>
        <div>
          <div style={{ fontSize: 11, color: "#2980B9", letterSpacing: 2, textTransform: "uppercase", marginBottom: 4 }}>Mentee Assessment</div>
          <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 22, fontWeight: 700, color: "#F0EBE0" }}>Mid-Program Assessment</h1>
        </div>
      </div>
      <div style={{ background: "rgba(41,128,185,0.06)", border: "1px solid rgba(41,128,185,0.15)", borderRadius: 12, padding: "14px 18px", marginBottom: 24, fontSize: 13, color: "#8A9BB5", lineHeight: 1.7 }}>
        We hope your mentorship journey is progressing smoothly! Help us better serve you in the remaining period. Deadline: Friday, August 22nd at 6 PM.
      </div>

      <div className="card" style={{ padding: 32 }}>
        <FormSection icon="◆" title="Updates (only if changed)">
          <div className="grid-2" style={{ gap: 14 }}>
            <FormField label="Field of Study (if updated)"><input className="input-field" value={f.field_of_study_update} onChange={e => set("field_of_study_update", e.target.value)} placeholder="Updated field..." /></FormField>
            <FormField label="University (if updated)"><input className="input-field" value={f.university_update} onChange={e => set("university_update", e.target.value)} placeholder="Updated university..." /></FormField>
            <FormField label="Current Employer (if updated)"><input className="input-field" value={f.employer_update} onChange={e => set("employer_update", e.target.value)} placeholder="Updated employer..." /></FormField>
            <FormField label="Current Job Title (if updated)"><input className="input-field" value={f.job_title_update} onChange={e => set("job_title_update", e.target.value)} placeholder="Updated title..." /></FormField>
          </div>
        </FormSection>

        <div style={{ height: 1, background: "rgba(201,168,76,0.1)", margin: "24px 0" }} />

        <FormSection icon="◈" title="1-on-1 Meetings">
          <FormField label="Average duration of meetings" required>
            <RadioGroup options={["30 minutes", "45 minutes", "1 hour", "1:15 hours", "1:30 hours", "2 hours", "Other"]} value={f.avg_meeting_duration} onChange={v => set("avg_meeting_duration", v)} />
          </FormField>
          <FormField label="How many meetings have you conducted with your mentor so far?" required>
            <input className="input-field" type="number" min="0" value={f.num_meetings} onChange={e => set("num_meetings", e.target.value)} placeholder="Number of meetings" style={{ maxWidth: 200 }} />
          </FormField>
        </FormSection>

        <div style={{ height: 1, background: "rgba(201,168,76,0.1)", margin: "24px 0" }} />

        <FormSection icon="◉" title="Mentorship Quality">
          <FormField label="Quality of communication with your mentor"><StarRating value={f.communication_quality} onChange={v => set("communication_quality", v)} labels={["Poor", "Fair", "Good", "Very good", "Excellent"]} /></FormField>
          <FormField label="Progress towards your goals"><StarRating value={f.goal_progress} onChange={v => set("goal_progress", v)} labels={["None", "Little", "Some", "Good", "Great"]} /></FormField>
          <FormField label="How supportive is your mentor?"><StarRating value={f.mentor_support_rating} onChange={v => set("mentor_support_rating", v)} labels={["Not supportive", "Fair", "Supportive", "Very supportive", "Excellent"]} /></FormField>
          <FormField label="Overall satisfaction with mentorship so far"><StarRating value={f.overall_satisfaction} onChange={v => set("overall_satisfaction", v)} labels={sat5} /></FormField>
        </FormSection>

        <div style={{ height: 1, background: "rgba(201,168,76,0.1)", margin: "24px 0" }} />

        <FormSection icon="★" title="Your Progress">
          <FormField label="What is your biggest achievement so far?" required>
            <textarea className="input-field" rows={3} style={{ resize: "vertical" }} value={f.biggest_achievement} onChange={e => set("biggest_achievement", e.target.value)} placeholder="Share your achievement..." />
          </FormField>
          <FormField label="What challenges have you faced?">
            <textarea className="input-field" rows={3} style={{ resize: "vertical" }} value={f.challenges_faced} onChange={e => set("challenges_faced", e.target.value)} placeholder="Describe challenges..." />
          </FormField>
        </FormSection>

        <div style={{ height: 1, background: "rgba(201,168,76,0.1)", margin: "24px 0" }} />

        <FormSection icon="◐" title="In-Person Gathering">
          <FormField label="How satisfied are you with the in-person gathering?"><StarRating value={f.gathering_satisfaction} onChange={v => set("gathering_satisfaction", v)} labels={sat5} /></FormField>
          <FormField label="What did you enjoy best during the gathering?">
            <textarea className="input-field" rows={2} style={{ resize: "vertical" }} value={f.gathering_best} onChange={e => set("gathering_best", e.target.value)} placeholder="What you enjoyed most..." />
          </FormField>
          <FormField label="What did you enjoy least during the gathering?">
            <textarea className="input-field" rows={2} style={{ resize: "vertical" }} value={f.gathering_least} onChange={e => set("gathering_least", e.target.value)} placeholder="What could be improved..." />
          </FormField>
          <FormField label="Anything else you want to share?">
            <textarea className="input-field" rows={3} style={{ resize: "vertical" }} value={f.anything_else} onChange={e => set("anything_else", e.target.value)} placeholder="Questions, recommendations, achievements, milestones..." />
          </FormField>
        </FormSection>

        <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
          <button className="btn-primary" style={{ flex: 1, justifyContent: "center", padding: "13px" }} onClick={handleSubmit} disabled={saving}>
            {saving ? <Spinner size={16} color="#080F1E" /> : "✦ Submit Mid-Program Assessment"}
          </button>
          <button className="btn-ghost" onClick={onBack}>Cancel</button>
        </div>
      </div>
...
function MenteeFinalForm({ onBack, onSubmit }) {
  ...
  const [f, setF] = useState({
    ...
    num_meetings: "", avg_duration: "", meeting_format: [],
    mentor_match_rating: 0, overall_satisfaction: 0, communication_rating: 0,
    program_duration_satisfaction: "", favorite_component: "", least_favorite: "",
    suggestions: "", would_recommend: "", testimonial: "", additional_comments: "",
  });
  const set = (k, v) => setF(p => ({ ...p, [k]: v }));
  const sat5 = ["Not satisfied", "Fair", "Good", "Very good", "Super satisfied!"];
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
        ...
        <div>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 4 }}>Mentee Assessment</div>
          <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 22, fontWeight: 700, color: "#F0EBE0" }}>Final Program Assessment</h1>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 32 }}>
        <FormSection icon="◆" title="About You">
          <div className="grid-2" style={{ gap: 14 }}>
            <FormField label="Are you enrolled in a graduate program?" required>
              <RadioGroup options={["No", "Yes, master's in Lebanon", "Yes, master's outside Lebanon", "Yes, PhD in Lebanon", "Yes, PhD outside Lebanon", "No, but planning to apply"]} value={f.grad_program} onChange={v => set("grad_program", v)} />
            </FormField>
            <div>
              <FormField label="Field of Study (if applicable)"><input className="input-field" value={f.field_of_study} onChange={e => set("field_of_study", e.target.value)} placeholder="Field of study..." /></FormField>
              <FormField label="University (if applicable)"><input className="input-field" value={f.university} onChange={e => set("university", e.target.value)} placeholder="University..." /></FormField>
              <FormField label="Current Job Position" required><input className="input-field" value={f.job_position} onChange={e => set("job_position", e.target.value)} placeholder="Write NA if not employed" /></FormField>
              <FormField label="Company and Type"><input className="input-field" value={f.company} onChange={e => set("company", e.target.value)} placeholder="e.g. INJAZ Lebanon, NGO" /></FormField>
            </div>
          </div>
        </FormSection>
        ...
        <FormSection icon="◈" title="Your Mentorship Journey">
          <div className="grid-2" style={{ gap: 14 }}>
            <FormField label="Total meetings conducted" required>
              <input className="input-field" type="number" min="0" value={f.num_meetings} onChange={e => set("num_meetings", e.target.value)} placeholder="Number of meetings" />
            </FormField>
            <FormField label="Average meeting duration" required>
              <RadioGroup options={["30 minutes", "45 minutes", "1 hour", "1:30 hours", "2 hours"]} value={f.avg_duration} onChange={v => set("avg_duration", v)} />
            </FormField>
          </div>
          <FormField label="How well matched were you with your mentor?"><StarRating value={f.mentor_match_rating} onChange={v => set("mentor_match_rating", v)} labels={["Poor match", "Fair", "Good", "Very good", "Perfect match!"]} /></FormField>
          <FormField label="Overall satisfaction with the mentorship program"><StarRating value={f.overall_satisfaction} onChange={v => set("overall_satisfaction", v)} labels={sat5} /></FormField>
          <FormField label="Effectiveness of communication and follow-up from INJAZ"><StarRating value={f.communication_rating} onChange={v => set("communication_rating", v)} labels={["Not effective", "Fair", "Effective", "Very effective", "Excellent"]} /></FormField>
        </FormSection>

        <div style={{ height: 1, background: "rgba(201,168,76,0.1)", margin: "24px 0" }} />

        <FormSection icon="◉" title="Program Feedback">
          <FormField label="How satisfied are you with the 9-month duration?" required>
            <RadioGroup options={["Very satisfied", "Satisfied", "I prefer a shorter duration (6 months)", "Other"]} value={f.program_duration_satisfaction} onChange={v => set("program_duration_satisfaction", v)} />
          </FormField>
          <FormField label="Most favorite component of the program" required>
            <input className="input-field" value={f.favorite_component} onChange={e => set("favorite_component", e.target.value)} placeholder="What you loved most..." />
          </FormField>
          <FormField label="Least favorite component" required>
            <textarea className="input-field" rows={2} style={{ resize: "vertical" }} value={f.least_favorite} onChange={e => set("least_favorite", e.target.value)} placeholder="What could be improved..." />
          </FormField>
          <FormField label="Suggestions for Intersection 5.0" required>
            <textarea className="input-field" rows={3} style={{ resize: "vertical" }} value={f.suggestions} onChange={e => set("suggestions", e.target.value)} placeholder="Your recommendations..." />
          </FormField>
        </FormSection>

        <div style={{ height: 1, background: "rgba(201,168,76,0.1)", margin: "24px 0" }} />

        <FormSection icon="★" title="Final Words">
          <FormField label="Would you recommend this program to others?" required>
            <RadioGroup options={["Yes! Sharing it with friends immediately", "Yes", "Maybe", "No"]} value={f.would_recommend} onChange={v => set("would_recommend", v)} />
          </FormField>
          <FormField label="Share a short testimonial (may be shared on social media)" required>
            <textarea className="input-field" rows={4} style={{ resize: "vertical" }} value={f.testimonial} onChange={e => set("testimonial", e.target.value)} placeholder="A couple of sentences about your experience..." />
          </FormField>
          <FormField label="Any additional comments or feedback?">
            <textarea className="input-field" rows={3} style={{ resize: "vertical" }} value={f.additional_comments} onChange={e => set("additional_comments", e.target.value)} placeholder="Anything else you want to share..." />
          </FormField>
        </FormSection>

        <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
          <button className="btn-primary" style={{ flex: 1, justifyContent: "center", padding: "13px" }} onClick={handleSubmit} disabled={saving}>
            {saving ? <Spinner size={16} color="#080F1E" /> : "✦ Submit Final Assessment"}
          </button>
          <button className="btn-ghost" onClick={onBack}>Cancel</button>
        </div>
      </div>
...
function TrainerAssessments() {
  ...
  useEffect(() => {
    if (profile?.id) {
      db.getMyAssessments(profile.id, "trainer").then(setSubmitted).catch(() => {}).finally(() => setLoading(false));
    } else setLoading(false);
  }, [profile?.id]);
  ...
  const forms = [
    { id: "mentor_first_meeting", title: "First Meeting Feedback", desc: "Share your experience after your first meeting with your mentee", icon: "◆", color: "#27AE60", timing: "After 1st session" },
    { id: "mentor_final", title: "Final Program Assessment", desc: "Share your overall experience and help shape Intersection 5.0", icon: "★", color: "#C9A84C", timing: "End of program" },
  ];

  if (activeForm === "mentor_first_meeting") return <MentorFirstMeetingForm onBack={() => setActiveForm(null)} onSubmit={() => { setActiveForm(null); db.getMyAssessments(profile.id, "trainer").then(setSubmitted); }} />;
  if (activeForm === "mentor_final") return <MentorFinalForm onBack={() => setActiveForm(null)} onSubmit={() => { setActiveForm(null); db.getMyAssessments(profile.id, "trainer").then(setSubmitted); }} />;
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>◈ Intersection Program</div>
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 26, fontWeight: 700, color: "#F0EBE0", marginBottom: 6 }}>My Assessments</h1>
        <p style={{ color: "#8A9BB5", fontSize: 14 }}>Program feedback forms for mentors — 1-on-1 mentorship only</p>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {forms.map(form => (
          <div key={form.id} className="card" style={{ padding: 28, border: hasSubmitted(form.id) ? "1px solid rgba(39,174,96,0.3)" : `1px solid ${form.color}22`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
              <div style={{ width: 52, height: 52, borderRadius: "50%", background: `${form.color}22`, border: `1px solid ${form.color}44`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, color: form.color, flexShrink: 0 }}>{form.icon}</div>
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
...
function MentorFirstMeetingForm({ onBack, onSubmit }) {
  ...
  const [f, setF] = useState({
    ...
    scheduling_issues: "", mentee_engaged: "", meeting_description: "",
    agreed_on_frequency: "", concerns: "",
    onboarding_helped: "", additional_training: "", mentee_prepared: "",
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
        <button className="btn-ghost" style={{ fontSize: 13 }} onClick={onBack}>← Back</button>
        <div>
          <div style={{ fontSize: 11, color: "#27AE60", letterSpacing: 2, textTransform: "uppercase", marginBottom: 4 }}>Mentor Assessment</div>
          <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 22, fontWeight: 700, color: "#F0EBE0" }}>First Meeting Feedback</h1>
        </div>
      </div>
      <div style={{ background: "rgba(39,174,96,0.06)", border: "1px solid rgba(39,174,96,0.15)", borderRadius: 12, padding: "14px 18px", marginBottom: 24, fontSize: 13, color: "#8A9BB5", lineHeight: 1.7 }}>
      ...
      <div className="card" style={{ padding: 32 }}>
        <FormSection icon="◆" title="Your Information">
          <FormField label="Name of Mentee" required>
            <input className="input-field" value={f.mentee_name} onChange={e => set("mentee_name", e.target.value)} placeholder="Your mentee's name" />
          </FormField>
        </FormSection>

        <div style={{ height: 1, background: "rgba(201,168,76,0.1)", margin: "24px 0" }} />

        <FormSection icon="◈" title="Have you had your first meeting with your mentee?" required>
          <RadioGroup options={[{ value: "yes", label: "Yes" }, { value: "no", label: "No" }]} value={f.had_first_meeting} onChange={v => set("had_first_meeting", v)} />
        </FormSection>

        {f.had_first_meeting === "yes" && <>
          <div style={{ height: 1, background: "rgba(201,168,76,0.1)", margin: "24px 0" }} />
          <FormSection icon="◉" title="Your First Meeting with Your Mentee">
            <FormField label="Did you face any issues in scheduling the meeting?" required>
              <RadioGroup options={[{ value: "yes", label: "Yes" }, { value: "no", label: "No" }]} value={f.scheduling_issues} onChange={v => set("scheduling_issues", v)} />
            </FormField>
            <FormField label="Did you find your mentee responsive and engaged?" required>
              <RadioGroup options={["Yes", "Somewhat", "Not sure", "No"]} value={f.mentee_engaged} onChange={v => set("mentee_engaged", v)} />
            </FormField>
            <FormField label="How would you describe your first meeting?" required>
              <textarea className="input-field" rows={3} style={{ resize: "vertical" }} value={f.meeting_description} onChange={e => set("meeting_description", e.target.value)} placeholder="Describe the meeting..." />
            </FormField>
            <FormField label="Did you agree with your mentee on the frequency of meetings?" required>
              <RadioGroup options={[{ value: "yes", label: "Yes" }, { value: "no", label: "No" }]} value={f.agreed_on_frequency} onChange={v => set("agreed_on_frequency", v)} />
            </FormField>
            <FormField label="Do you have any concerns about continuing the relationship?">
              <textarea className="input-field" rows={2} style={{ resize: "vertical" }} value={f.concerns} onChange={e => set("concerns", e.target.value)} placeholder="Describe any concerns or foreseen challenges..." />
            </FormField>
          </FormSection>

          <div style={{ height: 1, background: "rgba(201,168,76,0.1)", margin: "24px 0" }} />
          <FormSection icon="★" title="Your Feedback on the Program">
            <FormField label="Did the mentor onboarding session help you prepare?" required>
              <RadioGroup options={["Yes", "Somewhat", "Not sure", "No"]} value={f.onboarding_helped} onChange={v => set("onboarding_helped", v)} />
            </FormField>
            <FormField label="Would you have liked additional training for mentors?" required>
              <RadioGroup options={["Yes", "Maybe", "Probably not", "No"]} value={f.additional_training} onChange={v => set("additional_training", v)} />
            </FormField>
            <FormField label="Did you feel your mentee was prepared for the first meeting?" required>
              <RadioGroup options={["Yes", "Somewhat", "Not sure", "No"]} value={f.mentee_prepared} onChange={v => set("mentee_prepared", v)} />
            </FormField>
            <FormField label="Your overall first impression">
              <textarea className="input-field" rows={2} style={{ resize: "vertical" }} value={f.overall_first_impression} onChange={e => set("overall_first_impression", e.target.value)} placeholder="Any additional thoughts..." />
            </FormField>
          </FormSection>
        </>}
...
function MentorFinalForm({ onBack, onSubmit }) {
  ...
  const [f, setF] = useState({
    ...
    overall_satisfaction: 0, communication_rating: 0, mentor_mentee_match: 0, mentee_progress: 0,
    favorite_component: "", least_favorite: "",
    what_learned: "", recommendations: "",
    mentor_next_edition: "", deliver_session: "", refer_mentors: "",
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}>
        <button className="btn-ghost" style={{ fontSize: 13 }} onClick={onBack}>← Back</button>
        <div>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 4 }}>Mentor Assessment</div>
          <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 22, fontWeight: 700, color: "#F0EBE0" }}>Final Program Assessment</h1>
        </div>
      </div>
      <div style={{ background: "rgba(201,168,76,0.06)", border: "1px solid rgba(201,168,76,0.15)", borderRadius: 12, padding: "14px 18px", marginBottom: 24, fontSize: 13, color: "#8A9BB5", lineHeight: 1.7 }}>
      ...
      <div className="card" style={{ padding: 32 }}>
        <FormSection icon="◆" title="About You">
          <div className="grid-2" style={{ gap: 14 }}>
            <FormField label="Field of Work / Industry" required><input className="input-field" value={f.field_of_work} onChange={e => set("field_of_work", e.target.value)} placeholder="e.g. Technology, Finance" /></FormField>
            <FormField label="Job Title and Company" required><input className="input-field" value={f.job_title_company} onChange={e => set("job_title_company", e.target.value)} placeholder="Title at Company" /></FormField>
          </div>
        </FormSection>

        <div style={{ height: 1, background: "rgba(201,168,76,0.1)", margin: "24px 0" }} />

        <FormSection icon="◈" title="Program Satisfaction">
          <FormField label="Overall satisfaction with the mentorship program"><StarRating value={f.overall_satisfaction} onChange={v => set("overall_satisfaction", v)} labels={sat5} /></FormField>
          <FormField label="Effectiveness of communication and follow-up from INJAZ"><StarRating value={f.communication_rating} onChange={v => set("communication_rating", v)} labels={["Not effective", "Fair", "Effective", "Very effective", "Excellent"]} /></FormField>
          <FormField label="How well matched were you with your mentee?"><StarRating value={f.mentor_mentee_match} onChange={v => set("mentor_mentee_match", v)} labels={["Poor match", "Fair", "Good", "Very good", "Perfect match!"]} /></FormField>
          <FormField label="Satisfaction with your mentee's progress toward career goals"><StarRating value={f.mentee_progress} onChange={v => set("mentee_progress", v)} labels={["Not satisfied", "Fair", "Good", "Very good", "Super satisfied"]} /></FormField>
        </FormSection>

        <div style={{ height: 1, background: "rgba(201,168,76,0.1)", margin: "24px 0" }} />

        <FormSection icon="◉" title="Testimonials and Suggestions">
          <FormField label="Most favorite component of the program" required>
            <input className="input-field" value={f.favorite_component} onChange={e => set("favorite_component", e.target.value)} placeholder="What you loved most..." />
          </FormField>
          <FormField label="Least favorite component" required>
          ...
          <FormField label="What did you learn from your mentee or from the mentoring process?">
            <textarea className="input-field" rows={3} style={{ resize: "vertical" }} value={f.what_learned} onChange={e => set("what_learned", e.target.value)} placeholder="Lessons learned..." />
          </FormField>
          <FormField label="What are you looking forward to in Intersection 5.0?" required>
            <textarea className="input-field" rows={3} style={{ resize: "vertical" }} value={f.recommendations} onChange={e => set("recommendations", e.target.value)} placeholder="Your recommendations for the next edition..." />
          </FormField>
        </FormSection>

        <div style={{ height: 1, background: "rgba(201,168,76,0.1)", margin: "24px 0" }} />

        <FormSection icon="★" title="Looking Ahead">
          <FormField label="Would you like to be a mentor in Intersection 5.0?" required>
            <RadioGroup options={[{ value: "yes", label: "Yes" }, { value: "no", label: "No" }]} value={f.mentor_next_edition} onChange={v => set("mentor_next_edition", v)} />
          </FormField>
          <FormField label="Would you like to deliver a group session in Intersection 5.0?" required>
            <RadioGroup options={["Yes", "No", "Maybe"]} value={f.deliver_session} onChange={v => set("deliver_session", v)} />
          </FormField>
          <FormField label="Do you have professionals in your network interested in joining as mentors?" required>
            <RadioGroup options={[{ value: "yes", label: "Yes" }, { value: "no", label: "No" }]} value={f.refer_mentors} onChange={v => set("refer_mentors", v)} />
          </FormField>
          <FormField label="Share a short testimonial (may be shared on social media)" required>
            <textarea className="input-field" rows={4} style={{ resize: "vertical" }} value={f.testimonial} onChange={e => set("testimonial", e.target.value)} placeholder="A couple of sentences about your experience..." />
          </FormField>
...
function InjazAssessmentsView() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => setLoading(false), 6000); // Force stop loading after 6s
...
function ParticipantAnnouncements() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [anns, reads] = await Promise.all([
        db.getAnnouncements(),
        session?.user?.id ? db.getReadAnnouncements(session.user.id) : [],
      ]);
  ...
  const markRead = async (id) => {
    if (!session?.user?.id || readIds.includes(id)) return;
    await db.markAnnouncementRead(id, session.user.id).catch(() => {});
  ...
  const typeConfig = {
    general: { color: "#C9A84C", icon: "◆", label: "General" },
    urgent: { color: "#C8392B", icon: "◉", label: "Urgent" },
    reminder: { color: "#2980B9", icon: "◈", label: "Reminder" },
    milestone: { color: "#27AE60", icon: "★", label: "Milestone" },
    event: { color: "#8E44AD", icon: "◐", label: "Event" },
  };
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
        <div>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>◆ INJAZ Lebanon</div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>Announcements</h1>
          <p style={{ fontSize: 13, color: "#8A9BB5" }}>Updates from INJAZ team and your trainer</p>
        </div>
        {unreadCount > 0 && (
          <div style={{ background: "rgba(200,57,43,0.15)", border: "1px solid rgba(200,57,43,0.3)", borderRadius: 20, padding: "6px 16px", fontSize: 13, fontWeight: 700, color: "#FF8A80" }}>
            {unreadCount} unread
          </div>
        )}
      </div>
      ...
      {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 80 }}><Spinner size={32} /></div>
        : announcements.length === 0 ? (
          <div className="card" style={{ textAlign: "center", padding: 80, color: "#4A5A72" }}>
            <div style={{ fontSize: 48, marginBottom: 16, opacity: .3 }}>◆</div>
            <div style={{ color: "#8A9BB5", marginBottom: 8 }}>No announcements yet</div>
            <div style={{ fontSize: 13 }}>Check back soon for updates from your INJAZ team</div>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {announcements.map(ann => {
              ...
              return (
                <div key={ann.id} className="card" style={{ padding: 0, overflow: "hidden", border: !isRead ? `1px solid ${cfg.color}44` : "1px solid rgba(201,168,76,0.08)", transition: "all .2s" }}>
                  {/* Unread indicator */}
                  {!isRead && <div style={{ height: 3, background: `linear-gradient(90deg,${cfg.color},${cfg.color}88)` }} />}
                  <div style={{ padding: "18px 24px", cursor: "pointer" }}
                    onClick={() => { setExpanded(isExpanded ? null : ann.id); markRead(ann.id); }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                      {/* Icon */}
                      <div style={{ width: 42, height: 42, borderRadius: "50%", background: `${cfg.color}22`, border: `1px solid ${cfg.color}44`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, color: cfg.color, flexShrink: 0 }}>
                        {cfg.icon}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                          <span style={{ fontWeight: 700, fontSize: 15, color: "#F0EBE0" }}>{ann.title}</span>
                          <span style={{ fontSize: 10, background: `${cfg.color}22`, color: cfg.color, border: `1px solid ${cfg.color}44`, borderRadius: 20, padding: "2px 8px", fontWeight: 600 }}>{cfg.label}</span>
                          {!isRead && <span style={{ fontSize: 10, background: "rgba(200,57,43,0.15)", color: "#FF8A80", border: "1px solid rgba(200,57,43,0.3)", borderRadius: 20, padding: "2px 8px", fontWeight: 700 }}>NEW</span>}
                        </div>
                        <div style={{ fontSize: 12, color: "#8A9BB5", display: "flex", gap: 12, flexWrap: "wrap" }}>
                          <span>By {ann.sender_name || "INJAZ Team"}</span>
                          <span>·</span>
                          <span>{new Date(ann.created_at).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</span>
                          {ann.target_role && ann.target_role !== "all" && <span>· For: {ann.target_role === "seeker" ? "Participants" : ann.target_role === "trainer" ? "Trainers" : "All"}</span>}
                        </div>
                      </div>
                      <span style={{ color: "#4A5A72", fontSize: 14, flexShrink: 0 }}>{isExpanded ? "▲" : "▼"}</span>
                    </div>
                    {/* Preview */}
                    {!isExpanded && (
                      <div style={{ marginTop: 10, marginLeft: 58, fontSize: 13, color: "#4A5A72", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                        {ann.body}
                      </div>
                    )}
                  </div>
                  {/* Expanded body */}
                  {isExpanded && (
                    <div style={{ padding: "0 24px 20px 24px", marginLeft: 58 }}>
                      <div style={{ height: 1, background: "rgba(255,255,255,0.06)", marginBottom: 16 }} />
                      <div style={{ fontSize: 14, color: "#8A9BB5", lineHeight: 1.8, whiteSpace: "pre-wrap" }}>{ann.body}</div>
                      {ann.link && (
                        <a href={ann.link} target="_blank" rel="noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: 6, marginTop: 14, color: "#C9A84C", fontSize: 13, fontWeight: 600, textDecoration: "none" }}>
                          → View Link
                        </a>
                      )}
                    </div>
...
function ComposeAnnouncements({ senderRole = "injaz" }) {
  ...
  const [deleting, setDeleting] = useState(null);
  const [form, setForm] = useState({ title: "", body: "", type: "general", target_role: "seeker", link: "" });
  const [posting, setPosting] = useState(false);
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));

  const load = async () => {
    const data = await db.getAnnouncements().catch(() => []);
    setAnnouncements(data);
    setLoading(false);
  };

  useEffect(() => {
    const _t = setTimeout(() => setLoading(false), 7000); load(); }, []);
  ...
  const handlePost = async () => {
    ...
    try {
      await db.createAnnouncement({
        ...
        sender_name: profile?.full_name || (senderRole === "injaz" ? "INJAZ Team" : "Trainer"),
        sender_role: senderRole,
      });
      setForm({ title: "", body: "", type: "general", target_role: "seeker", link: "" });
      setComposing(false);
      await load();
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
        <div>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>◆ Communications</div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>Announcements</h1>
          <p style={{ fontSize: 13, color: "#8A9BB5" }}>Send updates and notifications to participants and trainers</p>
        </div>
        <button className="btn-primary" onClick={() => setComposing(true)} style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ⊕ New Announcement
        </button>
      </div>
      ...
      {/* Compose modal */}
      {composing && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 20, backdropFilter: "blur(8px)" }} onClick={() => setComposing(false)}>
          <div className="card" style={{ maxWidth: 560, width: "100%", padding: 36, border: "1px solid rgba(201,168,76,0.3)" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
              <h2 style={{ fontSize: 20, fontWeight: 700, color: "#F0EBE0" }}>New Announcement</h2>
              <button onClick={() => setComposing(false)} style={{ background: "none", border: "none", color: "#4A5A72", fontSize: 24, cursor: "pointer" }}>×</button>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {Object.entries(typeConfig).map(([k, cfg]) => (
                      <button key={k} onClick={() => set("type", k)}
                        style={{ padding: "7px 14px", borderRadius: 20, border: `1px solid ${form.type === k ? cfg.color + "66" : "rgba(255,255,255,0.1)"}`, background: form.type === k ? `${cfg.color}22` : "transparent", color: form.type === k ? cfg.color : "#8A9BB5", fontSize: 12, fontWeight: form.type === k ? 700 : 400, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", transition: "all .15s" }}>
                        {cfg.icon} {cfg.label}
                      </button>
                    ))}
                  </div>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {[["all", "Everyone"], ["seeker", "Participants Only"], ["trainer", "Trainers Only"]].map(([v, l]) => (
                    <button key={v} onClick={() => set("target_role", v)}
                      style={{ flex: 1, padding: "9px 8px", borderRadius: 10, border: `1px solid ${form.target_role === v ? "rgba(201,168,76,0.5)" : "rgba(255,255,255,0.1)"}`, background: form.target_role === v ? "rgba(201,168,76,0.12)" : "rgba(255,255,255,0.03)", color: form.target_role === v ? "#C9A84C" : "#8A9BB5", fontSize: 12, fontWeight: form.target_role === v ? 700 : 400, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", transition: "all .2s" }}>
                      {l}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <textarea className="input-field" rows={5} style={{ resize: "vertical" }} placeholder="Write your announcement here..." value={form.body} onChange={e => set("body", e.target.value)} />
              </div>

              <div>
                <label className="lbl">Link (optional)</label>
                <input className="input-field" placeholder="https://..." value={form.link} onChange={e => set("link", e.target.value)} />
              </div>

              <div style={{ display: "flex", gap: 10 }}>
                <button className="btn-primary" style={{ flex: 1, justifyContent: "center" }} onClick={handlePost} disabled={posting}>
                  {posting ? <><Spinner size={16} color="#080F1E" /> Posting...</> : "✦ Post Announcement"}
                </button>
      ...
      {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 60 }}><Spinner size={32} /></div>
        : announcements.length === 0 ? (
          <div className="card" style={{ textAlign: "center", padding: 80, color: "#4A5A72" }}>
            <div style={{ fontSize: 48, marginBottom: 16, opacity: .3 }}>◆</div>
            <div style={{ color: "#8A9BB5", marginBottom: 20 }}>No announcements yet</div>
            <button className="btn-primary" onClick={() => setComposing(true)}>⊕ Create First Announcement</button>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {announcements.map(ann => {
              const cfg = typeConfig[ann.type] || typeConfig.general;
              return (
                <div key={ann.id} className="card" style={{ padding: "18px 24px", border: `1px solid ${cfg.color}22` }}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
                    <div style={{ width: 40, height: 40, borderRadius: "50%", background: `${cfg.color}22`, border: `1px solid ${cfg.color}44`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, color: cfg.color, flexShrink: 0 }}>{cfg.icon}</div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                        <span style={{ fontWeight: 700, fontSize: 15, color: "#F0EBE0" }}>{ann.title}</span>
                        <span style={{ fontSize: 10, background: `${cfg.color}22`, color: cfg.color, border: `1px solid ${cfg.color}44`, borderRadius: 20, padding: "2px 8px" }}>{cfg.label}</span>
                        <span style={{ fontSize: 10, background: "rgba(255,255,255,0.05)", color: "#8A9BB5", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: "2px 8px" }}>
                          → {ann.target_role === "seeker" ? "Participants" : ann.target_role === "trainer" ? "Trainers" : "Everyone"}
                        </span>
                      </div>
                      <div style={{ fontSize: 13, color: "#8A9BB5", lineHeight: 1.7, marginBottom: 8 }}>{ann.body}</div>
                      <div style={{ fontSize: 11, color: "#4A5A72" }}>
                        {new Date(ann.created_at).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}
                        {ann.link && <a href={ann.link} target="_blank" rel="noreferrer" style={{ marginLeft: 12, color: "#C9A84C", textDecoration: "none" }}>→ View Link</a>}
                      </div>
                    </div>
                    <button onClick={() => handleDelete(ann.id)} disabled={deleting === ann.id}
                      style={{ background: "rgba(200,57,43,0.1)", border: "1px solid rgba(200,57,43,0.2)", color: "#FF8A80", borderRadius: 8, padding: "6px 12px", fontSize: 12, cursor: "pointer", flexShrink: 0, fontFamily: "'DM Sans',sans-serif" }}>
                      {deleting === ann.id ? <Spinner size={12} color="#FF8A80" /> : "Delete"}
                    </button>
                  </div>
                </div>
              );
...
function InviteManagement() {
  ...
  const load = () => db.getInvites().then(setInvites).catch(() => {}).finally(() => setLoading(false));
  useEffect(() => {
    const _t = setTimeout(() => setLoading(false), 7000); load(); }, []);
  ...
  const handleInvite = async () => {
    ...
    try {
      ...
      // Also create their Supabase auth account via admin invite
      await supabase.auth.admin?.inviteUserByEmail?.(form.email.trim(), {
        data: { role: form.role, full_name: form.full_name }
      }).catch(() => {}); // fails silently if not admin
  ...
  const roleColors = {
    seeker: { color: "#2980B9", label: "Participant" },
    trainer: { color: "#8E44AD", label: "Trainer/Mentor" },
    injaz_team: { color: "#C9A84C", label: "INJAZ Team" },
    employer: { color: "#27AE60", label: "Employer" },
  };
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>✦ Access Control</div>
        <h1 style={{ fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>User Invitations</h1>
        <p style={{ fontSize: 13, color: "#8A9BB5" }}>Control who can access the platform — invite-only system</p>
      </div>

      {/* Stats */}
      <div className="grid-3" style={{ marginBottom: 24 }}>
        {[
          { label: "Total Invites", value: invites.length, color: "#C9A84C", icon: "◈" },
          { label: "Pending (not joined)", value: pending, color: "#E67E22", icon: "◐" },
          { label: "Joined", value: used, color: "#27AE60", icon: "✓" },
        ].map(s => (
          <div key={s.label} className="card" style={{ padding: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
              <span style={{ fontSize: 18, color: s.color }}>{s.icon}</span>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "340px 1fr", gap: 20, alignItems: "start" }}>
        {/* Create invite */}
        <div className="card fade-up" style={{ padding: 28 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 20 }}>⊕ Add New User</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
            ...
            <div>
              ...
              <input className="input-field" placeholder="Their full name" value={form.full_name} onChange={e => setForm(f => ({ ...f, full_name: e.target.value }))} />
            </div>
            <div>
              <label className="lbl">Role</label>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {Object.entries(roleColors).map(([r, cfg]) => (
                  <button key={r} onClick={() => setForm(f => ({ ...f, role: r }))}
                    style={{ padding: "10px 14px", borderRadius: 10, border: `1px solid ${form.role === r ? cfg.color + "55" : "rgba(255,255,255,0.08)"}`, background: form.role === r ? `${cfg.color}18` : "rgba(255,255,255,0.02)", color: form.role === r ? cfg.color : "#8A9BB5", fontSize: 13, fontWeight: form.role === r ? 700 : 400, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", transition: "all .15s", textAlign: "left" }}>
                    {cfg.label}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="lbl">Note (internal)</label>
              <input className="input-field" placeholder="Optional note..." value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} />
            </div>
            <button className="btn-primary" style={{ justifyContent: "center", padding: "12px" }} onClick={handleInvite} disabled={sending}>
              {sending ? <><Spinner size={16} color="#080F1E" /> Adding...</> : "⊕ Add User"}
            </button>
            <div style={{ padding: 12, background: "rgba(41,128,185,0.08)", border: "1px solid rgba(41,128,185,0.2)", borderRadius: 10, fontSize: 12, color: "#74B9FF", lineHeight: 1.6 }}>
              ◈ After adding, share the platform URL with the user. They sign in with their email — you set their password via Supabase or send a reset link.
            </div>
          </div>
        </div>
        ...
        <div className="card fade-up-1" style={{ padding: 0, overflow: "hidden" }}>
          <div style={{ padding: "16px 24px", borderBottom: "1px solid rgba(201,168,76,0.12)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase" }}>All Users ({invites.length})</div>
          </div>
          {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 60 }}><Spinner size={32} /></div>
            : invites.length === 0 ? (
              <div style={{ textAlign: "center", padding: 60, color: "#4A5A72" }}>
                <div style={{ fontSize: 36, marginBottom: 12, opacity: .3 }}>◈</div>
                No users added yet. Add your first user on the left.
              </div>
            ) : (
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead>
                    <tr style={{ borderBottom: "1px solid rgba(201,168,76,0.1)", background: "rgba(201,168,76,0.03)" }}>
                      {["Name / Email", "Role", "Note", "Status", "Added", ""].map(h => (
                        <th key={h} style={{ padding: "10px 16px", textAlign: "left", color: "#C9A84C", fontWeight: 600, fontSize: 10, textTransform: "uppercase", letterSpacing: .8, whiteSpace: "nowrap" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {invites.map(inv => {
                      const cfg = roleColors[inv.role] || roleColors.seeker;
                      return (
                        <tr key={inv.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}
                          onMouseEnter={e => e.currentTarget.style.background = "rgba(201,168,76,0.02)"}
                          onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                          <td style={{ padding: "12px 16px" }}>
                            {inv.full_name && <div style={{ fontWeight: 600, color: "#F0EBE0", marginBottom: 2 }}>{inv.full_name}</div>}
                            <div style={{ fontSize: 12, color: "#8A9BB5" }}>{inv.email}</div>
                          </td>
                          <td style={{ padding: "12px 16px" }}>
                            <span style={{ fontSize: 11, background: `${cfg.color}22`, color: cfg.color, border: `1px solid ${cfg.color}44`, borderRadius: 20, padding: "3px 10px", fontWeight: 600 }}>{cfg.label}</span>
                          </td>
                          <td style={{ padding: "12px 16px", color: "#4A5A72", fontSize: 12 }}>{inv.note || "—"}</td>
                          <td style={{ padding: "12px 16px" }}>
                            {inv.used
                              ? <span style={{ fontSize: 11, background: "rgba(39,174,96,0.12)", color: "#4AE08A", border: "1px solid rgba(39,174,96,0.3)", borderRadius: 20, padding: "3px 10px" }}>✓ Joined</span>
                              : <span style={{ fontSize: 11, background: "rgba(230,126,34,0.12)", color: "#F0A500", border: "1px solid rgba(230,126,34,0.3)", borderRadius: 20, padding: "3px 10px" }}>Pending</span>}
                          </td>
                          <td style={{ padding: "12px 16px", color: "#4A5A72", fontSize: 11 }}>
                          ...
                          <td style={{ padding: "12px 16px" }}>
                            <button onClick={() => handleDelete(inv.id)} disabled={deleting === inv.id}
                              style={{ background: "rgba(200,57,43,0.1)", border: "1px solid rgba(200,57,43,0.2)", color: "#FF8A80", borderRadius: 8, padding: "5px 12px", fontSize: 12, cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}>
                              {deleting === inv.id ? <Spinner size={12} color="#FF8A80" /> : "Remove"}
                            </button>
                          </td>
                        </tr>
                      );
    ...
    if (this.state.hasError) {
      return (
        <div style={{ padding: 40, textAlign: "center" }}>
          <div style={{ fontSize: 36, color: "#C9A84C", marginBottom: 16 }}>◈</div>
          <div style={{ fontSize: 16, fontWeight: 700, color: "#F0EBE0", marginBottom: 8 }}>Something went wrong</div>
          <div style={{ fontSize: 13, color: "#8A9BB5", marginBottom: 20 }}>{this.state.error?.message}</div>
...
// ── Safe DB call with timeout ─────────────────────────────────
async function safeCall(promise, fallback = null, timeoutMs = 8000) {
  try {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    Promise.race([
      ...
      setSession(null); // Show login on timeout
    });

    // Listen for auth changes (login, logout, token refresh)
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, sess) => {
      if (event === "SIGNED_OUT") {
        setSession(null); setProfile(null); setRole(null); setIsAdmin(false);
        return;
      }
    ...
    // Keepalive using Web Worker (not throttled when tab is hidden)
    let keepaliveWorker = null;
    try {
      const workerCode = `
        let interval = setInterval(() => { postMessage('ping'); }, 20000);
        onmessage = (e) => { if (e.data === 'stop') { clearInterval(interval); } };
      `;
      const blob = new Blob([workerCode], { type: 'application/javascript' });
      keepaliveWorker = new Worker(URL.createObjectURL(blob));
      keepaliveWorker.onmessage = async () => {
        try {
    ...
    } catch(e) { console.warn('Worker not available'); }
    ...
    // Refresh on window focus - full page reload to prevent freeze
    const handleFocus = async () => {
      try {
        ...
        console.warn('Session check failed on focus:', e);
      }
    };
    window.addEventListener('focus', handleFocus);

    // Reload page when tab comes back if hidden > 2 minutes
    let hiddenAt = null;
    const handleVisibilityForReload = () => {
      if (document.visibilityState === 'hidden') {
        ...
        hiddenAt = null;
        if (hiddenFor > 2 * 60 * 1000) {
          window.location.reload();
        } else {
          // Short absence - just refresh auth
          supabase.auth.getSession().then(({ data }) => {
            if (data?.session) setSession(data.session);
            else { setSession(null); setProfile(null); setRole(null); }
          }).catch(() => {});
        }
      }
    };
    ...
    return () => {
      ...
      if (keepaliveWorker) { keepaliveWorker.postMessage('stop'); keepaliveWorker.terminate(); }

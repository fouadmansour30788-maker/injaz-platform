...
// Show clear error if env vars are missing
if (!supabase) {
  console.error("❌ INJAZ: Missing Supabase environment variables. Check VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in Vercel settings.");
}
...
// ── Gemini AI Client (free tier, no server needed) ─────────
...
const ai = {
  async generateCoverLetter(jobTitle, orgName, requiredSkills, candidateName, candidateSkills, tone, language) {
    const toneGuide = tone === "formal" ? "professional and formal" : tone === "youth" ? "enthusiastic and youth-friendly" : "NGO-focused and mission-driven";
    const prompt = `Write a cover letter in ${language} for ${candidateName} applying for ${jobTitle} at ${orgName}.
Tone: ${toneGuide}
Candidate skills: ${candidateSkills.join(", ")}
Required skills: ${requiredSkills.join(", ")}
Write a complete, compelling cover letter of 3-4 paragraphs. Return only the letter text.`;
    return await callGemini(prompt);
  },

  async getInterviewFeedback(question, answer, questionType, jobTitle) {
    const prompt = `You are an interview coach. Evaluate this interview answer and respond ONLY with valid JSON, no markdown.
...
{"clarity_score":85,"relevance_score":80,"confidence_score":75,"overall_score":80,"tip":"specific improvement tip here","strength":"what they did well here"}`;
  ...
  async reviewCV(cvText, sector) {
    const prompt = `You are an ATS and CV expert. Review this CV for the ${sector} sector and respond ONLY with valid JSON, no markdown.

CV text (first 3000 chars):
${cvText.slice(0, 3000)}

Return exactly this JSON structure:
{"ats_score":75,"overall_verdict":"Good CV with some improvements needed","top_suggestions":["suggestion 1","suggestion 2","suggestion 3"],"missing_sections":["section1"],"strongest_section":"Work Experience"}`;
    const text = await callGemini(prompt);
    const clean = text.replace(/```json|```/g, "").trim();
    return JSON.parse(clean);
  },
...
export default function App() {
  ...
  if (!supabase) {
    return (
      <div style={{ minHeight:"100vh", background:"#07111F", display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
        <style>{globalStyles}</style>
        <div style={{ maxWidth:520, background:"rgba(231,76,60,.1)", border:"1px solid rgba(231,76,60,.4)", borderRadius:16, padding:36, textAlign:"center" }}>
          <div style={{ fontSize:48, marginBottom:16 }}>⚠️</div>
          <div style={{ fontFamily:"serif", fontSize:24, color:"#E74C3C", marginBottom:12 }}>Setup Not Complete</div>
          <div style={{ color:"#CBD5E0", fontSize:14, lineHeight:1.7, marginBottom:20 }}>
            Go to Vercel → Your Project → Settings → Environment Variables and add these two keys from Supabase → Settings → API Keys:
          </div>
          <div style={{ background:"#0A1628", borderRadius:10, padding:16, textAlign:"left", fontFamily:"monospace", fontSize:13, color:"#58D68D", marginBottom:20, lineHeight:2 }}>
            <div>VITE_SUPABASE_URL</div>
            <div>VITE_SUPABASE_ANON_KEY</div>
          </div>
          <div style={{ color:"#6B7A9A", fontSize:13 }}>Then go to Deployments and click Redeploy.</div>
        </div>
      </div>
    );
  }
  ...
  const loadProfile = async (user) => {
    ...
    try {
      if (userRole === "seeker") {
        ...
        if (!p) {
          // Profile missing — create it now (handles email verification race condition)
          await supabase.from("job_seekers").insert({
            user_id: user.id,
            full_name: meta.full_name || "",
            governorate: meta.governorate || null,
            sector: meta.sector || null,
            nationality: meta.nationality || "lebanese",
            profile_score: 20,
          }).single();
          p = await db.getSeekerProfile(user.id);
        }
        setProfile(p);
        ...
        if (!p) {
          ...
          await supabase.from("employers").insert({
            user_id: user.id,
            org_name: meta.org_name || meta.full_name || "My Organization",
            contact_person: meta.full_name || "",
            governorate: meta.governorate || null,
            sector: meta.sector || null,
          }).single();
          p = await db.getEmployerProfile(user.id);
        }
        setProfile(p);
      }
    } catch (e) {

              ...
              <div style={{ display: "flex", gap: 8 }}>
                {statusOptions.map(s => (
                  <button key={s.id} onClick={() => setForm(f => ({ ...f, status: s.id }))}
                    style={{ flex: 1, padding: "10px 8px", borderRadius: 10, border: `1px solid ${form.status === s.id ? s.color + "66" : "rgba(255,255,255,0.08)"}`, background: form.status === s.id ? `${s.color}22` : "rgba(255,255,255,0.03)", color: form.status === s.id ? s.color : "#8A9BB5", fontSize: 12, fontWeight: form.status === s.id ? 700 : 400, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", transition: "all .2s" }}>
                    {s.label}
                  </button>
                ))}
              </div>

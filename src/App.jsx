      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 40 }}>
        <div style={{ width: "100%", maxWidth: 420 }}>
          ...
          {mode === "register" && step === 1 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 24, background: "rgba(255,255,255,0.05)", padding: 4, borderRadius: C.radiusSm }}>
              {[["seeker", "◈ Job Seeker"], ["employer", "◆ Employer"], ["trainer", "◉ Trainer / Mentor"], ["injaz_team", "✦ INJAZ Team"]].map(([r, label]) => (
            ...
            {mode === "register" && step === 2 && <>
              ...
              {role === "injaz_team" && <>
                <div><Label>Your Role at INJAZ</Label>
                  <select className="select-field" value={form.injaz_role || "program_officer"} onChange={e => set("injaz_role", e.target.value)}>
  ...
  return (
    <div>
      ...
      <div className="grid-4" style={{ marginBottom: 20 }}>
        {[
          { label: "Job Seekers", value: stats.seekers, icon: "◈", color: "#2980B9", sub: `${seekers.filter(s=>(s.profile_score||20)>=80).length} complete`, page: "seekers" },
...
function InjazSeekers() {
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
        <div>
          <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>Job Seekers</h1>
...
function ScaleQuestion({ label, sub, value, onChange, min = 1, max = 5, minLabel, maxLabel }) {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        {minLabel && <span style={{ fontSize: 11, color: "#4A5A72", whiteSpace: "nowrap" }}>{minLabel}</span>}
        <div style={{ display: "flex", gap: 8 }}>
        ...
        {maxLabel && <span style={{ fontSize: 11, color: "#4A5A72", whiteSpace: "nowrap" }}>{maxLabel}</span>}
      </div>
    </div>
  );
}
...
function AssessmentShell({ title, subtitle, icon, color, children, onSubmit, saving, submitted, submittedMsg }) {
  if (submitted) return (
    <div className="card" style={{ padding: 60, textAlign: "center", border: `1px solid ${color}44` }}>
      ...
      <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 22, fontWeight: 700, color: "#F0EBE0", marginBottom: 10 }}>Submitted!</div>
      <div style={{ fontSize: 14, color: "#8A9BB5" }}>{submittedMsg || "Thank you for your feedback."}</div>
    </div>
  );
  return (
    <div>
      <div className="card" style={{ padding: "28px 32px", marginBottom: 20, border: `1px solid ${color}33`, background: `rgba(${color === "#C9A84C" ? "201,168,76" : color === "#27AE60" ? "39,174,96" : color === "#2980B9" ? "41,128,185" : "142,68,173"},0.06)` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            ...
            <h2 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 20, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>{title}</h2>
...
// ── 1. Mentee First Meeting Feedback ──────────────────────────
function MenteeFirstMeetingForm({ onDone, existing }) {
  const { profile } = useAuth();
  const { showSuccess, showError } = useToast();
  const [saving, setSaving] = useState(false);
  const [f, setF] = useState({
    mentor_name: "", date_of_meeting: "", encouraging: "", description: "",
    agreed_frequency: "", strengths: "", challenges: "", goals: "",
    mentor_match: "", additional: ""
  });
  const set = (k, v) => setF(p => ({ ...p, [k]: v }));

  const handleSubmit = async () => {
    if (!f.mentor_name || !f.encouraging) { showError("Please fill required fields."); return; }
    setSaving(true);
    try {
      await db.saveAssessment({ user_id: profile.user_id, role: "seeker", type: "mentee_first_meeting", data: f, seeker_id: profile.id });
      showSuccess("Assessment submitted!"); onDone();
    } catch (e) { showError(e.message); }
    finally { setSaving(false); }
  };

  if (existing) return (
    <AssessmentShell title="First Meeting Feedback" subtitle="Already submitted" icon="◆" color="#27AE60" submitted submittedMsg="You have already submitted this assessment." />
  );

  return (
    <AssessmentShell title="Mentee — First Meeting Feedback" subtitle="Share your experience from your first meeting with your mentor" icon="◆" color="#27AE60" onSubmit={handleSubmit} saving={saving}>
      <TextQuestion label="Name of your Mentor *" value={f.mentor_name} onChange={v => set("mentor_name", v)} placeholder="Mentor's full name" />
      <TextQuestion label="Date of First Meeting *" value={f.date_of_meeting} onChange={v => set("date_of_meeting", v)} placeholder="e.g. April 10, 2025" />
      <RadioQuestion label="Did you find your mentor encouraging and engaged with you? *" value={f.encouraging} onChange={v => set("encouraging", v)} options={["Yes", "Somewhat", "Not sure", "No"]} />
      <TextQuestion label="How do you describe your first meeting? *" value={f.description} onChange={v => set("description", v)} multiline placeholder="Describe the atmosphere, topics discussed, your impression..." />
      <RadioQuestion label="Did you agree with your mentor on the frequency of meetings? *" value={f.agreed_frequency} onChange={v => set("agreed_frequency", v)} options={["Yes", "No"]} />
      <TextQuestion label="What do you consider are your greatest strengths? *" value={f.strengths} onChange={v => set("strengths", v)} multiline />
      <TextQuestion label="What are the major challenges you are trying to overcome? *" value={f.challenges} onChange={v => set("challenges", v)} multiline />
      <TextQuestion label="What are your main goals for this mentorship? *" value={f.goals} onChange={v => set("goals", v)} multiline />
      <RadioQuestion label="How would you rate your mentor match so far?" value={f.mentor_match} onChange={v => set("mentor_match", v)} options={["Excellent match", "Good match", "Neutral", "Not sure yet"]} />
      <TextQuestion label="Any additional comments?" value={f.additional} onChange={v => set("additional", v)} multiline />
    </AssessmentShell>
  );
}

// ── 2. Mentor First Meeting Feedback ─────────────────────────
function MentorFirstMeetingForm({ onDone, existing }) {
  const { profile } = useAuth();
  const { showSuccess, showError } = useToast();
  const [saving, setSaving] = useState(false);
  const [f, setF] = useState({
    mentee_name: "", scheduling_issues: "", mentee_engaged: "",
    description: "", agreed_frequency: "", concerns: "",
    onboarding_helped: "", additional_training: "", mentee_prepared: "",
    additional: ""
  });
  const set = (k, v) => setF(p => ({ ...p, [k]: v }));
  ...
  const handleSubmit = async () => {
    if (!f.mentee_name || !f.mentee_engaged) { showError("Please fill required fields."); return; }
    setSaving(true);
    try {
      await db.saveAssessment({ user_id: profile.user_id, role: "trainer", type: "mentor_first_meeting", data: f, trainer_id: profile.id });
      showSuccess("Assessment submitted!"); onDone();
    } catch (e) { showError(e.message); }
    finally { setSaving(false); }
  };
  ...
  return (
    <AssessmentShell title="Mentor — First Meeting Feedback" subtitle="Share your experience from your first meeting with your mentee" icon="◈" color="#2980B9" onSubmit={handleSubmit} saving={saving}>
      <TextQuestion label="Name of your Mentee *" value={f.mentee_name} onChange={v => set("mentee_name", v)} placeholder="Mentee's full name" />
      <RadioQuestion label="Did you face any issues scheduling the meeting? *" value={f.scheduling_issues} onChange={v => set("scheduling_issues", v)} options={["Yes", "No"]} />
      <RadioQuestion label="Did you find your mentee responsive and engaged? *" value={f.mentee_engaged} onChange={v => set("mentee_engaged", v)} options={["Yes", "Somewhat", "Not sure", "No"]} />
      <TextQuestion label="How would you describe your first meeting? *" value={f.description} onChange={v => set("description", v)} multiline placeholder="Describe the atmosphere, topics discussed, your impression..." />
      <RadioQuestion label="Did you agree with your mentee on the frequency of meetings? *" value={f.agreed_frequency} onChange={v => set("agreed_frequency", v)} options={["Yes", "No"]} />
      <TextQuestion label="Do you have any concerns about continuing the relationship? Describe any foreseen challenges." value={f.concerns} onChange={v => set("concerns", v)} multiline />
      <RadioQuestion label="Did the mentor onboarding session help you prepare for this experience? *" value={f.onboarding_helped} onChange={v => set("onboarding_helped", v)} options={["Yes", "Somewhat", "Not sure", "No"]} />
      <RadioQuestion label="Would you have liked additional training for mentors?" value={f.additional_training} onChange={v => set("additional_training", v)} options={["Yes", "Maybe", "Probably not", "No"]} />
      <RadioQuestion label="Did you feel your mentee was prepared for the first meeting? *" value={f.mentee_prepared} onChange={v => set("mentee_prepared", v)} options={["Yes", "Somewhat", "Not sure", "No"]} />
      <TextQuestion label="Any additional comments?" value={f.additional} onChange={v => set("additional", v)} multiline />
    </AssessmentShell>
  );
}

// ── 3. Mentee Mid-Program Assessment ─────────────────────────
function MenteeMidProgramForm({ onDone, existing }) {
  const { profile } = useAuth();
  const { showSuccess, showError } = useToast();
  const [saving, setSaving] = useState(false);
  const [f, setF] = useState({
    meeting_duration: "", meetings_count: "", meeting_quality: 0,
    mentor_availability: 0, goals_progress: 0, mentor_support: 0,
    program_satisfaction: 0, communication_rating: 0,
    gathering_satisfaction: 0, gathering_best: "", gathering_least: "",
    topics_for_future: "", anything_else: ""
  });
  const set = (k, v) => setF(p => ({ ...p, [k]: v }));

  const handleSubmit = async () => {
    if (!f.meeting_duration || !f.meeting_quality) { showError("Please fill required fields."); return; }
    setSaving(true);
    try {
      await db.saveAssessment({ user_id: profile.user_id, role: "seeker", type: "mentee_mid_program", data: f, seeker_id: profile.id });
      showSuccess("Mid-program assessment submitted!"); onDone();
    } catch (e) { showError(e.message); }
    finally { setSaving(false); }
  };

  if (existing) return (
    <AssessmentShell title="Mid-Program Assessment" subtitle="Already submitted" icon="◐" color="#C9A84C" submitted submittedMsg="You have already submitted this assessment. Thank you!" />
  );

  return (
    <AssessmentShell title="Mentee — Mid-Program Assessment" subtitle="Help us serve you better in the remaining program period" icon="◐" color="#C9A84C" onSubmit={handleSubmit} saving={saving}>
      <RadioQuestion label="Average duration of your meetings with your mentor? *" value={f.meeting_duration} onChange={v => set("meeting_duration", v)}
        options={["30 minutes", "45 minutes", "1 hour", "1:15 hours", "1:30 hours", "2 hours"]} />
      <TextQuestion label="How many meetings have you had with your mentor so far? *" value={f.meetings_count} onChange={v => set("meetings_count", v)} placeholder="e.g. 3" />
      <ScaleQuestion label="Overall quality of your 1-on-1 meetings *" value={f.meeting_quality} onChange={v => set("meeting_quality", v)} minLabel="Poor" maxLabel="Excellent" />
      <ScaleQuestion label="Mentor availability and responsiveness *" value={f.mentor_availability} onChange={v => set("mentor_availability", v)} minLabel="Not available" maxLabel="Very available" />
      <ScaleQuestion label="Progress toward your personal goals *" value={f.goals_progress} onChange={v => set("goals_progress", v)} minLabel="No progress" maxLabel="Great progress" />
      <ScaleQuestion label="How much has your mentor supported your growth? *" value={f.mentor_support} onChange={v => set("mentor_support", v)} minLabel="Not much" maxLabel="Greatly" />
      <ScaleQuestion label="Overall program satisfaction so far *" value={f.program_satisfaction} onChange={v => set("program_satisfaction", v)} minLabel="Not satisfied" maxLabel="Super satisfied" />
      <ScaleQuestion label="Effectiveness of communication from INJAZ team *" value={f.communication_rating} onChange={v => set("communication_rating", v)} minLabel="Not effective" maxLabel="Very effective" />
      <ScaleQuestion label="Satisfaction with the in-person gathering *" value={f.gathering_satisfaction} onChange={v => set("gathering_satisfaction", v)} minLabel="Not satisfied" maxLabel="Super satisfied" />
      <TextQuestion label="What did you enjoy best during the gathering? *" value={f.gathering_best} onChange={v => set("gathering_best", v)} multiline />
      <TextQuestion label="What did you enjoy least during the gathering? *" value={f.gathering_least} onChange={v => set("gathering_least", v)} multiline />
      <TextQuestion label="Are there specific topics you would like to see addressed in future sessions?" value={f.topics_for_future} onChange={v => set("topics_for_future", v)} multiline />
      <TextQuestion label="Anything else you want to share?" value={f.anything_else} onChange={v => set("anything_else", v)} multiline placeholder="Questions, recommendations, achievements, milestones..." />
    </AssessmentShell>
  );
}

// ── 4. Mentee Final Assessment ────────────────────────────────
function MenteeFinalForm({ onDone, existing }) {
  const { profile } = useAuth();
  const { showSuccess, showError } = useToast();
  const [saving, setSaving] = useState(false);
  const [f, setF] = useState({
    grad_program: "", field_of_study: "", university: "",
    job_position: "", company: "", program_satisfaction: 0,
    favorite_component: "", least_favorite: "", communication_rating: 0,
    duration_satisfaction: "", improvements: "", would_recommend: "",
    testimonial: "", additional: ""
  });
  const set = (k, v) => setF(p => ({ ...p, [k]: v }));

  const handleSubmit = async () => {
    if (!f.program_satisfaction || !f.would_recommend) { showError("Please fill required fields."); return; }
    setSaving(true);
    try {
      await db.saveAssessment({ user_id: profile.user_id, role: "seeker", type: "mentee_final", data: f, seeker_id: profile.id });
      showSuccess("Final assessment submitted! Thank you!"); onDone();
    } catch (e) { showError(e.message); }
    finally { setSaving(false); }
  };

  if (existing) return (
    <AssessmentShell title="Final Assessment" subtitle="Already submitted" icon="★" color="#C9A84C" submitted submittedMsg="Thank you for completing the final program assessment!" />
  );

  return (
    <AssessmentShell title="Mentee — Final Program Assessment" subtitle="As we conclude Intersection 4.0 — share your journey" icon="★" color="#C9A84C" onSubmit={handleSubmit} saving={saving}>
      <RadioQuestion label="Are you currently enrolled in a graduate program? *" value={f.grad_program} onChange={v => set("grad_program", v)}
        options={["No", "Yes, master's in Lebanon", "Yes, master's outside Lebanon", "Yes, PhD in Lebanon", "Yes, PhD outside Lebanon", "Planning to apply in Lebanon", "Planning to apply outside Lebanon"]} />
      <TextQuestion label="Current Field of Study (if applicable)" value={f.field_of_study} onChange={v => set("field_of_study", v)} />
      <TextQuestion label="Current University (if applicable)" value={f.university} onChange={v => set("university", v)} />
      <TextQuestion label="Current job position (write NA if not employed) *" value={f.job_position} onChange={v => set("job_position", v)} />
      <TextQuestion label="Current company and type (write NA if not employed)" value={f.company} onChange={v => set("company", v)} placeholder="e.g. INJAZ Lebanon, NGO" />
      <ScaleQuestion label="Overall satisfaction with the mentorship program *" value={f.program_satisfaction} onChange={v => set("program_satisfaction", v)} minLabel="Not satisfied" maxLabel="Super satisfied" />
      <TextQuestion label="What was your most favorite component of the program? *" value={f.favorite_component} onChange={v => set("favorite_component", v)} multiline />
      <TextQuestion label="What was your least favorite component? *" value={f.least_favorite} onChange={v => set("least_favorite", v)} multiline />
      <ScaleQuestion label="Effectiveness of communication from INJAZ team *" value={f.communication_rating} onChange={v => set("communication_rating", v)} minLabel="Not effective" maxLabel="Very effective" />
      <RadioQuestion label="How satisfied are you with the program duration (9 months)? *" value={f.duration_satisfaction} onChange={v => set("duration_satisfaction", v)}
        options={["Very satisfied", "Satisfied", "I prefer a shorter duration (6 months)"]} />
      <TextQuestion label="Do you suggest any improvements for Intersection 5.0? *" value={f.improvements} onChange={v => set("improvements", v)} multiline />
      <RadioQuestion label="Would you recommend this program to others? *" value={f.would_recommend} onChange={v => set("would_recommend", v)}
        options={["Yes! Waiting to share it with friends", "Yes", "Maybe", "No"]} />
      <TextQuestion label="Share a short testimonial about your experience (may be shared on social media) *" value={f.testimonial} onChange={v => set("testimonial", v)} multiline placeholder="A couple sentences about your Intersection journey..." />
      <TextQuestion label="Any additional comments or feedback?" value={f.additional} onChange={v => set("additional", v)} multiline />
    </AssessmentShell>
  );
}

// ── 5. Mentor Final Assessment ────────────────────────────────
function MentorFinalForm({ onDone, existing }) {
  const { profile } = useAuth();
  const { showSuccess, showError } = useToast();
  const [saving, setSaving] = useState(false);
  const [f, setF] = useState({
    field_of_work: "", job_title: "", program_satisfaction: 0,
    communication_rating: 0, mentor_match: 0, mentee_progress: 0,
    favorite_component: "", least_favorite: "", learned: "",
    recommendations: "", return_as_mentor: "", deliver_session: "",
    refer_mentors: "", testimonial: "", additional: ""
  });
  const set = (k, v) => setF(p => ({ ...p, [k]: v }));

  const handleSubmit = async () => {
    if (!f.program_satisfaction || !f.return_as_mentor) { showError("Please fill required fields."); return; }
    setSaving(true);
    try {
      await db.saveAssessment({ user_id: profile.user_id, role: "trainer", type: "mentor_final", data: f, trainer_id: profile.id });
      showSuccess("Final assessment submitted! Thank you!"); onDone();
    } catch (e) { showError(e.message); }
    finally { setSaving(false); }
  };
  ...
  if (existing) return (
    <AssessmentShell title="Final Assessment" subtitle="Already submitted" icon="★" color="#2980B9" submitted submittedMsg="Thank you for your dedication to Intersection!" />
  );

  return (
    <AssessmentShell title="Mentor — Final Program Assessment" subtitle="As we conclude Intersection 4.0 — share your mentoring journey" icon="★" color="#2980B9" onSubmit={handleSubmit} saving={saving}>
      <TextQuestion label="Field of work / Industry *" value={f.field_of_work} onChange={v => set("field_of_work", v)} />
      <TextQuestion label="Job title and company *" value={f.job_title} onChange={v => set("job_title", v)} placeholder="e.g. Marketing Manager, INJAZ Lebanon" />
      <ScaleQuestion label="Overall satisfaction with the mentorship program *" value={f.program_satisfaction} onChange={v => set("program_satisfaction", v)} minLabel="Not satisfied" maxLabel="Super satisfied" />
      <ScaleQuestion label="Effectiveness of communication and follow-up from INJAZ team *" value={f.communication_rating} onChange={v => set("communication_rating", v)} minLabel="Not effective" maxLabel="Very effective" />
      <ScaleQuestion label="How well were you matched with your mentee? *" value={f.mentor_match} onChange={v => set("mentor_match", v)} minLabel="Not a good match" maxLabel="A match made in heaven" />
      <ScaleQuestion label="Satisfaction with your mentee's progress toward career objectives *" value={f.mentee_progress} onChange={v => set("mentee_progress", v)} minLabel="Not satisfied" maxLabel="Super satisfied" />
      <TextQuestion label="What was your most favorite component of the program? *" value={f.favorite_component} onChange={v => set("favorite_component", v)} multiline />
      <TextQuestion label="What was your least favorite component? *" value={f.least_favorite} onChange={v => set("least_favorite", v)} multiline />
      <TextQuestion label="What did you learn from your mentee or from the mentoring process?" value={f.learned} onChange={v => set("learned", v)} multiline />
      <TextQuestion label="What are you looking forward to in Intersection 5.0? Any recommendations? *" value={f.recommendations} onChange={v => set("recommendations", v)} multiline />
      <RadioQuestion label="Would you like to be a mentor in Intersection 5.0? *" value={f.return_as_mentor} onChange={v => set("return_as_mentor", v)} options={["Yes", "No"]} />
      <RadioQuestion label="Would you like to deliver a group session during Intersection 5.0? *" value={f.deliver_session} onChange={v => set("deliver_session", v)} options={["Yes", "No", "Maybe"]} />
      <RadioQuestion label="Any professionals in your network interested in joining as mentors? *" value={f.refer_mentors} onChange={v => set("refer_mentors", v)} options={["Yes", "No"]} />
      <TextQuestion label="Share a short testimonial (may be shared on social media) *" value={f.testimonial} onChange={v => set("testimonial", v)} multiline placeholder="A couple sentences about your Intersection mentoring experience..." />
      <TextQuestion label="Any additional comments or feedback?" value={f.additional} onChange={v => set("additional", v)} multiline />
    </AssessmentShell>
  );
}

// ── Assessments Hub (Seeker) ──────────────────────────────────
function MyAssessments() {
  ...
  const forms = [
    {
      id: "mentee_first_meeting", title: "First Meeting Feedback", sub: "After your 1st session with your mentor",
      icon: "◆", color: "#27AE60", timing: "After 1st 1-on-1 session",
      component: <MenteeFirstMeetingForm onDone={() => { reload(); setActiveForm(null); }} existing={has("mentee_first_meeting")} />
    },
    {
      id: "mentee_mid_program", title: "Mid-Program Assessment", sub: "How is your mentorship journey going?",
      icon: "◐", color: "#C9A84C", timing: "Mid-program (Aug deadline)",
      component: <MenteeMidProgramForm onDone={() => { reload(); setActiveForm(null); }} existing={has("mentee_mid_program")} />
    },
    {
      id: "mentee_final", title: "Final Program Assessment", sub: "Your overall program experience",
      icon: "★", color: "#8E44AD", timing: "End of program (Dec)",
      component: <MenteeFinalForm onDone={() => { reload(); setActiveForm(null); }} existing={has("mentee_final")} />
    },
  ];
          ...
          {forms.map(form => {
            ...
            return (
              <div key={form.id} className="card" style={{ padding: "22px 28px", display: "flex", alignItems: "center", gap: 20, border: done ? "1px solid rgba(39,174,96,0.2)" : `1px solid ${form.color}22`, cursor: done ? "default" : "pointer", transition: "all .2s" }}
                ...
                onMouseEnter={e => { if (!done) e.currentTarget.style.borderColor = form.color + "55"; }}
                onMouseLeave={e => { if (!done) e.currentTarget.style.borderColor = form.color + "22"; }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, color: "#4A5A72" }}>📅 {form.timing}</div>
                </div>
                <div>
                  {done
...
// ── Assessments Hub (Trainer/Mentor) ──────────────────────────
function TrainerAssessments() {
  const { profile } = useAuth();
  const [assessments, setAssessments] = useState([]);
  const [loading, setLoading] = useState(true);
  ...
  const forms = [
    {
      id: "mentor_first_meeting", title: "First Meeting Feedback", sub: "After your 1st session with your mentee",
      icon: "◆", color: "#2980B9", timing: "After 1st 1-on-1 session",
      component: <MentorFirstMeetingForm onDone={() => { reload(); setActiveForm(null); }} existing={has("mentor_first_meeting")} />
    },
    {
      id: "mentor_final", title: "Final Program Assessment", sub: "Your overall mentoring experience",
      icon: "★", color: "#C9A84C", timing: "End of program (Dec)",
      component: <MentorFinalForm onDone={() => { reload(); setActiveForm(null); }} existing={has("mentor_final")} />
    },
  ];
  ...
  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>◈ Intersection Program</div>
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 6 }}>My Assessments</h1>
        <p style={{ color: "#8A9BB5", fontSize: 14 }}>Program feedback forms — complete after your mentoring sessions</p>
      </div>
      {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 60 }}><Spinner size={32} /></div>
        : <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {forms.map(form => {
...
function InjazAssessmentsView() {
  ...
  const typeLabels = {
    mentee_first_meeting: "Mentee — First Meeting",
    mentor_first_meeting: "Mentor — First Meeting",
    mentee_mid_program: "Mentee — Mid Program",
    mentee_final: "Mentee — Final",
    mentor_final: "Mentor — Final",
  };

  const typeColors = {
    mentee_first_meeting: "#27AE60", mentor_first_meeting: "#2980B9",
    mentee_mid_program: "#C9A84C", mentee_final: "#8E44AD", mentor_final: "#C9A84C",
  };

  const filtered = assessments.filter(a => filter === "all" || a.type === filter);

  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 24, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>Program Assessments</h1>
        <p style={{ fontSize: 13, color: "#8A9BB5" }}>All submitted assessments from mentees and mentors</p>
      </div>
      ...
      <div className="grid-4" style={{ marginBottom: 20 }}>
        {Object.entries(typeLabels).map(([type, label]) => {
          ...
          return (
            <div key={type} className="card" style={{ padding: 18, cursor: "pointer", border: filter === type ? `1px solid ${color}44` : "1px solid rgba(201,168,76,0.08)" }}
              onClick={() => setFilter(filter === type ? "all" : type)}>
              <div style={{ fontSize: 22, fontWeight: 700, color, marginBottom: 4 }}>{count}</div>
              <div style={{ fontSize: 11, color: "#8A9BB5", lineHeight: 1.4 }}>{label}</div>
            </div>
          );
        })}
      </div>
      ...
      <div className="tab-bar" style={{ marginBottom: 20 }}>
        {[["all", "All"], ...Object.entries(typeLabels).map(([k, v]) => [k, v.split(" — ")[1]])].map(([f, l]) => (
          <button key={f} className={`tab ${filter === f ? "active" : ""}`} onClick={() => setFilter(f)}>{l}</button>
        ))}
      </div>

      {loading ? <div style={{ display: "flex", justifyContent: "center", padding: 60 }}><Spinner size={32} /></div>
        : filtered.length === 0 ? <div className="card" style={{ textAlign: "center", padding: 60, color: "#4A5A72" }}>No assessments submitted yet.</div>
        : <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {filtered.map(a => {
            const color = typeColors[a.type] || "#C9A84C";
            const name = a.job_seekers?.full_name || a.trainers?.full_name || "Unknown";
            return (
              <div key={a.id} className="card" style={{ padding: "16px 24px", display: "flex", alignItems: "center", gap: 16, cursor: "pointer", transition: "all .2s" }}
                onClick={() => setSelected(selected?.id === a.id ? null : a)}
                onMouseEnter={e => e.currentTarget.style.borderColor = color + "44"}
                onMouseLeave={e => e.currentTarget.style.borderColor = "rgba(201,168,76,0.12)"}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: `${color}22`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, color, fontWeight: 700, flexShrink: 0 }}>
                  {name[0]?.toUpperCase()}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 600, color: "#F0EBE0", fontSize: 14 }}>{name}</div>
                  <div style={{ fontSize: 12, color: "#8A9BB5" }}>{typeLabels[a.type]} · {new Date(a.created_at).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</div>
                </div>
                <span style={{ fontSize: 11, background: `${color}22`, color, border: `1px solid ${color}44`, borderRadius: 20, padding: "3px 12px" }}>{typeLabels[a.type]}</span>
                <span style={{ color: "#4A5A72", fontSize: 16 }}>{selected?.id === a.id ? "▲" : "▼"}</span>
              </div>
            );
          })}
          {selected && (
            <div className="card" style={{ padding: 28, border: "1px solid rgba(201,168,76,0.2)" }}>
              <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>Assessment Details</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                {Object.entries(selected.data || {}).map(([key, val]) => val && (
                  <div key={key} style={{ padding: 14, background: "rgba(255,255,255,0.03)", borderRadius: 10 }}>
                    <div style={{ fontSize: 10, color: "#C9A84C", letterSpacing: 1, textTransform: "uppercase", marginBottom: 6 }}>{key.replace(/_/g, " ")}</div>
                    <div style={{ fontSize: 13, color: "#F0EBE0" }}>{String(val)}</div>
                  </div>
                ))}
              </div>
            </div>
...
export default function App() {
  ...
  if (role === "seeker") {
    const navItems = [
      ...
      { id: "assessments", icon: "◈", label: "My Assessments" },
  ...
  if (role === "injaz_team") {
    const injazNav = [
      { id: "dashboard", icon: "⊞", label: "Dashboard" },
      { id: "seekers", icon: "◈", label: "Job Seekers" },

...
const globalStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  @keyframes notifBell { 0%,100%{transform:rotate(0)} 20%{transform:rotate(-18deg)} 40%{transform:rotate(18deg)} 60%{transform:rotate(-12deg)} 80%{transform:rotate(8deg)} }
  .ann-popup {
    position:fixed; bottom:80px; right:24px; width:340px; max-width:calc(100vw - 48px);
    background:rgba(10,21,37,0.97); border:1px solid rgba(201,168,76,0.4);
    border-radius:14px; box-shadow:0 20px 60px rgba(0,0,0,0.7), 0 0 0 1px rgba(201,168,76,0.08);
    backdrop-filter:blur(24px); z-index:9999; overflow:hidden;
    animation:slideInRight .4s cubic-bezier(.22,1,.36,1) both;
  }
  .ann-popup.hiding { animation:slideOutRight .3s ease forwards; }
  .ann-popup-bar { height:3px; background:linear-gradient(90deg,#C9A84C,#E8C96A); transform-origin:left; animation:progressFill var(--dur,7s) linear forwards; --w:100%; }
  .ann-popup-bell { animation:notifBell .7s ease .3s; display:inline-block; }
  .nav-notif-badge {
    min-width:18px; height:18px; border-radius:9px; background:#C8392B;
    color:#fff; font-size:10px; font-weight:800; display:flex; align-items:center;
    justify-content:center; padding:0 4px; animation:pulse 2s infinite;
    box-shadow:0 0 0 2px #0A1525;
  }
`;
...
function AnnouncementPopup({ ann, onClose, onView }) {
  ...
  return (
    <div className={`ann-popup${hiding ? " hiding" : ""}`}>
      <div className="ann-popup-bar" style={{ "--dur": `${DURATION}ms` }} />
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(201,168,76,0.15)", border: "1px solid rgba(201,168,76,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>
              <span className="ann-popup-bell">◆</span>
            </div>
            <div>
          ...
          <button onClick={dismiss} style={{ background: "none", border: "none", color: "#4A5A72", cursor: "pointer", fontSize: 18, lineHeight: 1, padding: "2px 4px", flexShrink: 0 }}>×</button>
        </div>
        <p style={{ fontSize: 13, color: "#8A9BB5", margin: "0 0 14px 46px", lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
          {ann.body}
        </p>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginLeft: 46 }}>
          <span style={{ fontSize: 11, color: "#4A5A72" }}>
            By {ann.sender_name || "INJAZ Team"} · {ann.target_role === "all" ? "Everyone" : ann.target_role === "seeker" ? "Participants" : "Trainers"}
          </span>
          <button onClick={() => { onView(); dismiss(); }}
            style={{ fontSize: 12, fontWeight: 600, color: "#C9A84C", background: "rgba(201,168,76,0.1)", border: "1px solid rgba(201,168,76,0.25)", borderRadius: 8, padding: "5px 12px", cursor: "pointer" }}>
            View →
          </button>
        </div>
      </div>
...
function AnnouncementPopupManager({ session, role, onNavigateToAnnouncements }) {
  ...
  return (
    <AnnouncementPopup
      key={current._key}
      ann={current}
      onClose={() => remove(current._key)}
      onView={onNavigateToAnnouncements}
    />
  );
}
        ...
        <div style={{ width: "100%", maxWidth: 420 }}>
          ...
          {mode === "register" && step === 1 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 24, background: "rgba(255,255,255,0.05)", padding: 4, borderRadius: C.radiusSm }}>
              {[["seeker", "◈ Participant"], ["employer", "◆ Employer"], ["trainer", "◉ Trainer / Mentor"], ["injaz_team", "✦ INJAZ Team"]].map(([r, label]) => (
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#080F1E" }}>
        ...
        <nav style={{ flex: 1, display: "flex", flexDirection: "column", gap: 2 }}>
          {navItems.map(item => (
            <button key={item.id} className={`nav-item ${activePage === item.id ? "active" : ""}`}
              ...
              {item.badge && (item.badgeType === "notif" ? <span className="nav-notif-badge">{item.badge}</span> : <span style={{ fontSize: 11, background: "rgba(201,168,76,0.2)", color: "#C9A84C", borderRadius: 10, padding: "1px 7px", fontWeight: 700, border: "1px solid rgba(201,168,76,0.3)" }}>{item.badge}</span>)}
            </button>
          ))}
        </nav>
      ...
      <AnnouncementPopupManager
        session={session}
        role={role}
        onNavigateToAnnouncements={() => { setActivePage("announcements"); setSidebarOpen(false); }}
      />
    </div>
  );
}
...
function LebanonMap({ data }) {
  ...
  // Real Lebanon governorate SVG paths (accurate geographic shapes)
  const govPaths = {
    "Akkar": {
      d: "M 148,18 L 210,10 L 230,28 L 220,55 L 195,60 L 160,52 L 145,35 Z",
      labelX: 185, labelY: 38
    },
    "North Lebanon": {
      d: "M 110,52 L 145,35 L 160,52 L 195,60 L 200,75 L 185,90 L 155,95 L 125,80 L 108,68 Z",
      labelX: 155, labelY: 70
    },
    "Baalbek-Hermel": {
      d: "M 195,60 L 220,55 L 260,62 L 280,80 L 275,130 L 255,145 L 225,140 L 200,120 L 185,90 L 200,75 Z",
      labelX: 235, labelY: 100
    },
    "Bekaa": {
      d: "M 185,90 L 200,120 L 225,140 L 220,175 L 205,195 L 185,205 L 165,195 L 155,165 L 148,130 L 155,95 Z",
      labelX: 188, labelY: 150
    },
    "Mount Lebanon": {
      d: "M 108,68 L 125,80 L 155,95 L 148,130 L 135,148 L 118,152 L 95,140 L 82,120 L 88,95 L 100,78 Z",
      labelX: 120, labelY: 112
    },
    "Beirut": {
      d: "M 88,140 L 102,136 L 118,152 L 112,162 L 96,162 L 85,153 Z",
      labelX: 101, labelY: 151
    },
    "Nabatieh": {
      d: "M 118,152 L 135,148 L 155,165 L 148,188 L 130,200 L 110,198 L 100,182 L 105,165 Z",
      labelX: 128, labelY: 175
    },
    "South Lebanon": {
      d: "M 85,153 L 96,162 L 112,162 L 105,165 L 100,182 L 110,198 L 95,215 L 72,218 L 58,205 L 60,182 L 72,165 L 78,153 Z",
      labelX: 86, labelY: 185
    },
  };
  ...
  const getColor = (gov) => {
    ...
    return { fill: `rgba(201,168,76,${0.08 + intensity * 0.72})`, stroke: hovered === gov ? "#C9A84C" : `rgba(201,168,76,${0.25 + intensity * 0.4})` };
  };

  return (
    <div style={{ position: "relative" }}>
      <svg viewBox="40 5 250 220" style={{ width: "100%", height: 260, display: "block" }}>
        {/* Sea / background */}
        <rect x="40" y="5" width="250" height="220" fill="rgba(41,128,185,0.06)" rx="8" />
        ...
        {Object.entries(govPaths).map(([gov, { d, labelX, labelY }]) => {
          const val = data[gov] || 0;
          const { fill, stroke } = getColor(gov);
          const isHov = hovered === gov;
          return (
            <g key={gov} style={{ cursor: "pointer" }}
              onMouseEnter={() => setHovered(gov)}
              onMouseLeave={() => setHovered(null)}>
              <path d={d} fill={fill} stroke={stroke} strokeWidth={isHov ? 1.5 : 0.8}
                style={{ transition: "all .2s", filter: isHov ? "url(#glow)" : "none" }} />
              {/* Region label */}
              <text x={labelX} y={labelY - 5} textAnchor="middle"
                fill={isHov ? "#F0EBE0" : "rgba(240,235,224,0.75)"}
                fontSize={gov === "Beirut" ? "5.5" : "6.5"} fontWeight="600"
                style={{ pointerEvents: "none", transition: "fill .2s" }}>
                {gov === "North Lebanon" ? "North" : gov === "South Lebanon" ? "South" : gov === "Mount Lebanon" ? "Mt.Lebanon" : gov === "Baalbek-Hermel" ? "Baalbek" : gov}
              </text>
              {val > 0 && (
                <text x={labelX} y={labelY + 7} textAnchor="middle"
                  fill="#C9A84C" fontSize="7" fontWeight="700"
        ...
        {/* Mediterranean label */}
        <text x="65" y="130" fill="rgba(41,128,185,0.4)" fontSize="7" fontStyle="italic">Mediterranean</text>
        <text x="62" y="140" fill="rgba(41,128,185,0.4)" fontSize="7" fontStyle="italic">Sea</text>
      </svg>

      {/* Hover tooltip */}
      {hovered && (
        <div style={{ position: "absolute", bottom: 8, left: "50%", transform: "translateX(-50%)", textAlign: "center", padding: "8px 16px", background: "rgba(10,21,37,0.95)", borderRadius: 10, border: "1px solid rgba(201,168,76,0.35)", backdropFilter: "blur(12px)", whiteSpace: "nowrap" }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: "#C9A84C" }}>{hovered}</span>
          <span style={{ fontSize: 13, color: "#8A9BB5", marginLeft: 8 }}>{data[hovered] || 0} participant{(data[hovered] || 0) !== 1 ? "s" : ""}</span>
        </div>
      )}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 4 }}>
        ...
        <div style={{ flex: 1, height: 5, borderRadius: 99, background: "linear-gradient(90deg, rgba(201,168,76,0.08), rgba(201,168,76,0.8))" }} />
...
function SparkLine({ data = [], color = "#C9A84C", height = 50 }) {
  if (!data.length) return null;
  const max = Math.max(...data, 1);
...
// ── Trend line chart with labels ──────────────────────────────
function TrendChart({ points = [], labels = [], color = "#C9A84C", height = 100 }) {
  if (!points.length) return <div style={{ height, display: "flex", alignItems: "center", justifyContent: "center", color: "#4A5A72", fontSize: 12 }}>No data yet</div>;
  const max = Math.max(...points, 1);
  const W = 500, H = height;
  const padX = 20, padY = 8;
  const coords = points.map((v, i) => ({
    x: padX + (i / (points.length - 1 || 1)) * (W - padX * 2),
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height }}>
        <defs>
          <linearGradient id={`tg-${color.replace("#","")}`} x1="0" x2="0" y1="0" y2="1">
        ...
        {[0.25, 0.5, 0.75, 1].map(f => (
          <line key={f} x1={padX} x2={W - padX} y1={padY + (1-f) * (H - padY*2)} y2={padY + (1-f) * (H - padY*2)}
            stroke="rgba(255,255,255,0.04)" strokeWidth="1" />
        ))}
        <path d={areaPath} fill={`url(#tg-${color.replace("#","")})`} />
        <path d={linePath} fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
        {coords.map((p, i) => (
          <circle key={i} cx={p.x} cy={p.y} r="4" fill={color} stroke="rgba(8,15,30,0.8)" strokeWidth="2" />
        ))}
      </svg>
      {labels.length > 0 && (
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
          {labels.map((l, i) => (
            <span key={i} style={{ fontSize: 10, color: "#4A5A72", textAlign: "center", flex: 1 }}>{l}</span>
          ))}
        </div>
      )}
    </div>
...
function InsightCard({ icon, color, title, body, action, onAction }) {
  return (
    <div style={{ background: `${color}0D`, border: `1px solid ${color}30`, borderRadius: 12, padding: "14px 16px", display: "flex", gap: 14, alignItems: "flex-start" }}>
      <div style={{ width: 34, height: 34, borderRadius: 10, background: `${color}20`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>{icon}</div>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: "#F0EBE0", marginBottom: 3 }}>{title}</div>
        <div style={{ fontSize: 12, color: "#8A9BB5", lineHeight: 1.5 }}>{body}</div>
        {action && <button onClick={onAction} style={{ marginTop: 8, fontSize: 11, fontWeight: 600, color, background: `${color}15`, border: `1px solid ${color}30`, borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}>{action} →</button>}
      </div>
    </div>
  );
}
...
function InjazDashboard({ setActivePage }) {
  ...
  const topSectors = Object.entries(sectors).sort((a,b) => b[1]-a[1]).slice(0,7).map(([label,value]) => ({ label: label.length > 10 ? label.slice(0,9)+"…" : label, value }));
  ...
  const verifyRate = totalSessions > 0 ? Math.round(verifiedSessions / totalSessions * 100) : 0;
  const placedCount = stageCounts["placed"] || 0;
  const placementRate = seekers.length > 0 ? Math.round(placedCount / seekers.length * 100) : 0;

  const expData = [
    { label: "No exp", value: seekers.filter(s => (s.years_experience||0) === 0).length },
  ...
  // Simulated monthly trend (last 6 months, based on created_at where available)
  const now = new Date();
  ...
  const sessionTrend = months.map(m =>
    attendance.filter(s => {
  ...
  // Insights engine
  const insights = [];
  const profileComplete = seekers.filter(s => (s.profile_score||20) >= 80).length;
  const profileRate = seekers.length > 0 ? Math.round(profileComplete/seekers.length*100) : 0;
  if (profileRate < 50 && seekers.length > 0)
    insights.push({ icon: "◈", color: "#C9A84C", title: "Low Profile Completion", body: `Only ${profileRate}% of participants have completed profiles (80%+). Encourage them to fill in education, skills, and experience.`, action: "View Participants", page: "seekers" });
  if (verifyRate < 70 && totalSessions > 0)
    insights.push({ icon: "◉", color: "#E67E22", title: "Attendance Verification Lagging", body: `${verifyRate}% of sessions are verified. Remind trainers to verify attendance promptly after each session.`, action: "Attendance", page: "attendance" });
  if (placementRate >= 30)
    insights.push({ icon: "★", color: "#27AE60", title: "Strong Placement Rate", body: `${placementRate}% of participants have been placed — great progress! Share success stories to motivate others.`, action: "Journey", page: "journey" });
  if (seekers.length === 0)
    insights.push({ icon: "◆", color: "#2980B9", title: "No Participants Yet", body: "Add your first participants to start tracking journey progress, attendance, and outcomes.", action: "Add Participants", page: "seekers" });
  if (avgRating > 0 && avgRating < 3.5)
    insights.push({ icon: "⚠", color: "#C8392B", title: "Session Quality Alert", body: `Average session rating is ${avgRating.toFixed(1)}/5. Review trainer feedback and consider additional training support.`, action: "Reports", page: "reports" });
  if (insights.length === 0)
    insights.push({ icon: "✦", color: "#27AE60", title: "Platform Running Well", body: `${seekers.length} participants tracked, ${verifyRate}% verification rate, ${avgRating > 0 ? avgRating.toFixed(1)+"/5 rating" : "no ratings yet"}. Keep it up!`, action: null });
  ...
  return (
    <div>
      ...
      <div className="fade-up" style={{ marginBottom: 24, position: "relative" }}>
        <div style={{ position: "absolute", top: -10, right: 0, width: 70, height: 70, border: "1px solid rgba(201,168,76,0.15)", borderRadius: "50%", animation: "rotateSlow 20s linear infinite" }} />
        ...
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 26, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>
          Good {greeting}, <span style={{ color: "#C9A84C" }}>{name}</span>
        </h1>
        <p style={{ color: "#8A9BB5", fontSize: 13 }}>
          {profile?.injaz_role?.replace(/_/g," ")} · {new Date().toLocaleDateString("en-GB",{ weekday:"long", day:"numeric", month:"long", year:"numeric" })}
        </p>
      </div>
      ...
      <div className="grid-4" style={{ marginBottom: 18 }}>
        {[
          { label: "Participants", value: stats.seekers, icon: "◈", color: "#2980B9", sub: `${profileComplete} profiles complete`, page: "seekers" },
          { label: "Employers", value: stats.employers, icon: "◆", color: "#27AE60", sub: `${stats.postings} active postings`, page: "employers" },
          { label: "Sessions", value: totalSessions, icon: "◉", color: "#C9A84C", sub: `${verifyRate}% verified`, page: "attendance" },
          { label: "Placed ★", value: placedCount, icon: "★", color: "#8E44AD", sub: placementRate > 0 ? `${placementRate}% placement rate` : "0% so far", page: "journey" },
        ].map((s, i) => (
          <div key={s.label} className="card stat-hover fade-up" style={{ padding: 20, cursor: "pointer", animationDelay: `${i*.06}s` }} onClick={() => setActivePage(s.page)}>
            ...
            <div style={{ fontSize: 30, fontWeight: 700, color: "#F0EBE0", marginBottom: 2 }}>{loading ? "—" : s.value}</div>
      ...
      {/* Row 2: Lebanon Map + Trend Charts */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 16, marginBottom: 16 }}>
        {/* Real Lebanon Map */}
        <div className="card fade-up-1" style={{ padding: 22 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
            <div>
              <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 3 }}>◈ Geographic Distribution</div>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#F0EBE0" }}>Participants by Governorate</div>
            </div>
          ...
          {Object.keys(regionalData).length > 0 && (
            <div style={{ marginTop: 12, display: "flex", flexWrap: "wrap", gap: 6 }}>
              {Object.entries(regionalData).sort((a,b) => b[1]-a[1]).slice(0,4).map(([gov, count]) => (
                <span key={gov} style={{ fontSize: 11, background: "rgba(201,168,76,0.1)", border: "1px solid rgba(201,168,76,0.2)", borderRadius: 20, padding: "3px 9px", color: "#C9A84C" }}>
                  {gov.split(" ")[0]}: {count}
                </span>
              ))}
            </div>
        ...
        <div className="card fade-up-1" style={{ padding: 22 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase" }}>◉ 6-Month Trends</div>
            <div style={{ display: "flex", gap: 6 }}>
              {["registrations","sessions"].map(t => (
                <button key={t} onClick={() => setInsightTab(t)}
                  style={{ fontSize: 11, padding: "4px 10px", borderRadius: 8, border: `1px solid ${insightTab===t?"rgba(201,168,76,0.4)":"rgba(255,255,255,0.08)"}`, background: insightTab===t?"rgba(201,168,76,0.1)":"transparent", color: insightTab===t?"#C9A84C":"#8A9BB5", cursor: "pointer", fontFamily: "'DM Sans',sans-serif", textTransform: "capitalize" }}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <TrendChart
            points={insightTab === "sessions" ? sessionTrend : registrationTrend}
            labels={monthLabels}
            color={insightTab === "sessions" ? "#2980B9" : "#C9A84C"}
            height={100}
          />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
            {[
              { label: "Avg Rating", value: avgRating > 0 ? avgRating.toFixed(1)+"★" : "—", color: "#8E44AD", data: [0,1,2,3,4,5].map(i => reflections.filter(r=>r.rating===i).length) },
              { label: "Verification", value: verifyRate+"%", color: "#27AE60", data: [0,20,35,50,verifyRate,verifyRate] },
              { label: "Placements", value: placedCount, color: "#C9A84C", data: [0,0,1,1,placedCount-1>0?placedCount-1:0,placedCount] },
            ].map(({ label, value, color, data }) => (
              <div key={label} style={{ background: "rgba(255,255,255,0.02)", borderRadius: 10, padding: "10px 12px", border: "1px solid rgba(255,255,255,0.04)" }}>
                <div style={{ fontSize: 16, fontWeight: 700, color, marginBottom: 2 }}>{value}</div>
                <div style={{ fontSize: 10, color: "#4A5A72", marginBottom: 6 }}>{label}</div>
      ...
      {/* Row 3: Journey Funnel + Sector bars */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        {/* Journey Funnel */}
        <div className="card fade-up-2" style={{ padding: 22 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>✦ Journey Funnel</div>
          {JOURNEY_STAGES.map((stage, i) => {
            const count = stageCounts[stage.id] || 0;
            const pct = seekers.length > 0 ? Math.round((count / seekers.length) * 100) : 0;
            const barWidth = 100 - i * 8;
            return (
              <div key={stage.id} style={{ marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  <span style={{ fontSize: 12, color: "#F0EBE0", display: "flex", alignItems: "center", gap: 6 }}>
                    <span style={{ fontSize: 10, color: stage.color }}>{stage.icon}</span> {stage.label}
                  </span>
                  <span style={{ fontSize: 12, fontWeight: 700, color: stage.color }}>
                    {count} <span style={{ color: "#4A5A72", fontWeight: 400 }}>({pct}%)</span>
                  </span>
                </div>
                <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 99, height: 7, position: "relative", overflow: "hidden" }}>
                  <div style={{ width: `${pct}%`, height: "100%", background: `linear-gradient(90deg,${stage.color},${stage.color}AA)`, borderRadius: 99, boxShadow: `0 0 6px ${stage.color}44`, transition: "width .8s ease" }} />
                </div>
              </div>
            );
          })}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
            <span style={{ color: "#8A9BB5" }}>Placement rate</span>
            <span style={{ color: "#27AE60", fontWeight: 700 }}>{placementRate}%</span>
          </div>
        </div>

        {/* Sector distribution — horizontal bars */}
        <div className="card fade-up-2" style={{ padding: 22 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>◆ Top Sectors</div>
          {topSectors.length === 0 ? (
            <div style={{ textAlign: "center", padding: 32, color: "#4A5A72", fontSize: 12 }}>No sector data yet</div>
          ) : (() => {
            const maxV = Math.max(...topSectors.map(s => s.value), 1);
            const colors = ["#2980B9","#C9A84C","#27AE60","#8E44AD","#E67E22","#1ABC9C","#C8392B"];
            return topSectors.map((s, i) => (
              <div key={s.label} style={{ marginBottom: 11 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  <span style={{ fontSize: 12, color: "#F0EBE0" }}>{s.label}</span>
                ...
                <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 99, height: 7 }}>
                  <div style={{ width: `${(s.value / maxV) * 100}%`, height: "100%", background: `linear-gradient(90deg,${colors[i%colors.length]},${colors[i%colors.length]}88)`, borderRadius: 99, transition: "width .8s ease" }} />
                </div>
              </div>
            ));
          })()}
      ...
      {/* Row 4: Profile completion donut + Education + Experience */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
        {/* Profile completion */}
        <div className="card fade-up-3" style={{ padding: 22 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>◈ Profile Completion</div>
          <DonutChart segments={scoreRanges} size={120} />
        </div>
        {/* Education */}
        <div className="card fade-up-3" style={{ padding: 22 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>◈ Education Level</div>
          <DonutChart segments={eduSegments.length > 0 ? eduSegments : [{ label: "No data", value: 1, color: "#4A5A72" }]} size={120} />
        </div>
        {/* Experience bars */}
        <div className="card fade-up-3" style={{ padding: 22 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>◉ Experience</div>
          <BarChart data={expData} color="#2980B9" height={120} />
        </div>
      </div>

      {/* Row 5: Attendance insights + Ratings + Employment status */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
        {/* Attendance */}
        <div className="card fade-up-4" style={{ padding: 22 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 14 }}>≡ Attendance</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {[
              { label: "Total", value: totalSessions, color: "#C9A84C" },
              ...
              <div key={s.label} style={{ background: `${s.color}0A`, borderRadius: 10, padding: "10px 12px", border: `1px solid ${s.color}20` }}>
          ...
          <div style={{ marginTop: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 5 }}>
              <span style={{ color: "#8A9BB5" }}>Verify rate</span>
              <span style={{ color: "#27AE60", fontWeight: 700 }}>{verifyRate}%</span>
            </div>
            <ProgressBar value={verifyRate} max={100} color="#27AE60" height={5} />
          </div>
        </div>

        {/* Mentorship quality */}
        <div className="card fade-up-4" style={{ padding: 22 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 14 }}>★ Session Quality</div>
          {reflections.length === 0 ? (
            <div style={{ textAlign: "center", padding: 24, color: "#4A5A72" }}>
              <div style={{ fontSize: 28, marginBottom: 8, opacity: .3 }}>★</div>
              <div style={{ fontSize: 12 }}>No reflections submitted yet</div>
            </div>
          ) : (
            <>
              <div style={{ textAlign: "center", marginBottom: 14 }}>
                <div style={{ fontSize: 36, fontWeight: 700, color: "#C9A84C" }}>{avgRating.toFixed(1)}</div>
                <div style={{ display: "flex", justifyContent: "center", gap: 3, marginBottom: 4 }}>
                  {[1,2,3,4,5].map(s => <span key={s} style={{ fontSize: 14, color: s <= Math.round(avgRating) ? "#C9A84C" : "rgba(255,255,255,0.1)" }}>★</span>)}
                </div>
                <div style={{ fontSize: 11, color: "#8A9BB5" }}>{reflections.length} reflections</div>
              </div>
              {[5,4,3,2,1].map(star => (
                <div key={star} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 7 }}>
                  <span style={{ fontSize: 10, color: "#C9A84C", width: 18 }}>{star}★</span>
                  <div style={{ flex: 1, background: "rgba(255,255,255,0.06)", borderRadius: 99, height: 5 }}>
                    <div style={{ width: reflections.length > 0 ? `${(reflections.filter(r=>r.rating===star).length/reflections.length)*100}%` : "0%", height: "100%", background: "#C9A84C", borderRadius: 99 }} />
                  </div>
                  <span style={{ fontSize: 10, color: "#4A5A72", width: 16, textAlign: "right" }}>{reflections.filter(r=>r.rating===star).length}</span>
                </div>
              ))}
            </>
          )}
        ...
        {/* Employment status */}
        <div className="card fade-up-4" style={{ padding: 22 }}>
          <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 14 }}>◐ Employment Status</div>
          <DonutChart segments={[
      ...
      <div className="card fade-up" style={{ padding: 22, marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 3 }}>✦ Platform Insights & Recommendations</div>
            <div style={{ fontSize: 13, color: "#8A9BB5" }}>Based on current data</div>
          </div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {insights.map((ins, i) => (
            <InsightCard key={i} {...ins} onAction={() => ins.page && setActivePage(ins.page)} />
          ))}
        </div>
      </div>
      ...
      <div className="card fade-up" style={{ padding: 18 }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>✦ Quick Access</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(8,1fr)", gap: 8 }}>
          {[["◈","Seekers","seekers"],["◆","Employers","employers"],["◐","Journey","journey"],["◉","Checkpoints","checkpoints"],["≡","Attendance","attendance"],["◈","Trainers","trainers-overview"],["★","Reports","reports"],["⊕","Assessments","assessments"]].map(([icon,label,page])=>(
            <button key={page} onClick={() => setActivePage(page)}
              style={{ padding:"10px 6px", borderRadius:10, border:"1px solid rgba(201,168,76,0.12)", background:"rgba(255,255,255,0.02)", cursor:"pointer", textAlign:"center", transition:"all .2s", fontFamily:"'DM Sans',sans-serif" }}
              ...
              <div style={{fontSize:16,color:"#C9A84C",marginBottom:5}}>{icon}</div>
              <div style={{fontSize:10,color:"#F0EBE0"}}>{label}</div>
            </button>
          ))}
        </div>
      </div>
...
function useUnreadAnnouncements(session) {
  ...
  useEffect(() => {
    ...
    // Re-fetch count on any INSERT into announcements
    const channel = supabase

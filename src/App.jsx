...
function MyCheckpoints() {
  ...
  useEffect(() => {
    ...
    ]).then(setCheckpoint).catch(() => setCheckpoint(null)).finally(() => setLoading(false));
  }, []);
...
function SeekerAssessments() {
  ...
  useEffect(() => {
    if (profile?.id) {
      Promise.race([
        db.getMyAssessments(profile.id, "seeker"),
        new Promise((_, rej) => setTimeout(() => rej(new Error("timeout")), 10000))
      ]).then(setSubmitted).catch(() => setSubmitted([])).finally(() => setLoading(false));
    } else setLoading(false);
  }, [profile?.id]);
...
function ParticipantAnnouncements() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [anns, reads] = await Promise.race([
          Promise.all([
            db.getAnnouncements(),
            session?.user?.id ? db.getReadAnnouncements(session.user.id) : Promise.resolve([]),
          ]),
          new Promise((_, rej) => setTimeout(() => rej(new Error("timeout")), 10000))
        ]);
      ...
      if (userRole === "seeker") {
        ...
        if (!p) {
          ...
          await supabase.from("job_seekers").insert({
            user_id: user.id, full_name: meta.full_name || email.split("@")[0] || "",
            governorate: meta.governorate || null, sector: meta.sector || null,
            nationality: "Lebanese", profile_score: 20, journey_stage: "registered",
          }).single();
          p = await db.getSeekerProfile(user.id).catch(() => null);
        }
        // If still null, try by email as fallback
        if (!p) {
          const { data } = await supabase.from("job_seekers").select("*").eq("user_id", user.id).maybeSingle();
          p = data;
        }
        setProfile(p);

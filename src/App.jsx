...
function InjazDashboard({ setActivePage }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [s, u] = await Promise.all([db.getPlatformStats(), db.getAllUsers()]);
  ...
  const name = profile?.full_name?.split(" ")[0] || "there";
  const injazRole = profile?.injaz_role?.replace(/_/g, " ") || "INJAZ Team";
  const region = profile?.governorate || "National";

  // Journey stage counts
  const stageCounts = JOURNEY_STAGES.reduce((acc, s) => {
    acc[s.id] = users.seekers.filter(u => (u.journey_stage || "registered") === s.id).length;
    return acc;
  }, {});

  // Profile score distribution
  const scoreRanges = {
    high: users.seekers.filter(s => (s.profile_score || 20) >= 80).length,
    mid: users.seekers.filter(s => (s.profile_score || 20) >= 50 && (s.profile_score || 20) < 80).length,
    low: users.seekers.filter(s => (s.profile_score || 20) < 50).length,
  };

  return (
    <div>
      {/* Hero */}
      <div className="fade-up" style={{ marginBottom: 32, position: "relative" }}>
        <div style={{ position: "absolute", top: -10, right: 0, width: 100, height: 100, border: "1px solid rgba(201,168,76,0.15)", borderRadius: "50%", animation: "rotateSlow 20s linear infinite" }} />
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>✦ INJAZ Lebanon Platform</div>
        <h1 style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 28, fontWeight: 700, color: "#F0EBE0", marginBottom: 4 }}>
          Welcome, <span style={{ color: "#C9A84C" }}>{name}</span>
        </h1>
        <p style={{ color: "#8A9BB5", fontSize: 14 }}>{injazRole} · {region}</p>
      </div>

      {/* Key Stats */}
      <div className="grid-4" style={{ marginBottom: 24 }}>
        {[
          { label: "Job Seekers", value: stats.seekers, icon: "◈", color: "#2980B9", page: "seekers" },
          { label: "Employers", value: stats.employers, icon: "◆", color: "#27AE60", page: "employers" },
          { label: "Active Postings", value: stats.postings, icon: "◉", color: "#C9A84C", page: "postings" },
          { label: "Applications", value: stats.applications, icon: "◐", color: "#9B59B6", page: "reports" },
        ].map((s, i) => (
          <div key={s.label} className="card fade-up stat-hover" style={{ animationDelay: `${i*.07}s`, cursor: "pointer" }} onClick={() => setActivePage(s.page)}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 14 }}>
              <span style={{ fontSize: 20, color: s.color }}>{s.icon}</span>
              <div style={{ width: 6, height: 6, borderRadius: "50%", background: s.color, boxShadow: `0 0 8px ${s.color}` }} />
            </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
          ...
          {JOURNEY_STAGES.map((stage, i) => {
            ...
            return (
              <div key={stage.id} style={{ marginBottom: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  <span style={{ fontSize: 13, color: "#F0EBE0" }}>{stage.icon} {stage.label}</span>
                  <span style={{ fontSize: 13, fontWeight: 700, color: "#C9A84C" }}>{count} <span style={{ color: "#4A5A72", fontWeight: 400 }}>({pct}%)</span></span>
                </div>
                <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 99, height: 5 }}>
                  <div style={{ width: `${pct}%`, height: "100%", background: `linear-gradient(90deg, #C9A84C, #E8C96A)`, borderRadius: 99, transition: "width .8s ease", boxShadow: "0 0 6px rgba(201,168,76,0.4)" }} />
                </div>
              </div>
            );
          })}
        ...
        <div className="card fade-up-3" style={{ padding: 28 }}>
          ...
          {[
            { label: "Strong (80%+)", count: scoreRanges.high, color: "#27AE60" },
            { label: "Moderate (50–79%)", count: scoreRanges.mid, color: "#C9A84C" },
            { label: "Incomplete (<50%)", count: scoreRanges.low, color: "#E74C3C" },
          ].map(r => {
            const pct = stats.seekers > 0 ? Math.round((r.count / stats.seekers) * 100) : 0;
            return (
              <div key={r.label} style={{ marginBottom: 20 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                  <span style={{ fontSize: 13, color: "#F0EBE0" }}>{r.label}</span>
                  <span style={{ fontSize: 13, fontWeight: 700, color: r.color }}>{r.count} ({pct}%)</span>
                </div>
                <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 99, height: 8 }}>
                  <div style={{ width: `${pct}%`, height: "100%", background: r.color, borderRadius: 99, transition: "width .8s ease", boxShadow: `0 0 8px ${r.color}44` }} />
                </div>
              </div>
            );
          })}
          ...
          <div style={{ marginTop: 24 }}>
            <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12 }}>By Region</div>
            {["Beirut", "Mount Lebanon", "North Lebanon", "South Lebanon", "Bekaa"].map(gov => {
              const count = users.seekers.filter(s => s.governorate === gov).length;
              const pct = stats.seekers > 0 ? Math.round((count / stats.seekers) * 100) : 0;
              return (
                <div key={gov} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                  <span style={{ fontSize: 12, color: "#8A9BB5", width: 120, flexShrink: 0 }}>{gov}</span>
                  <div style={{ flex: 1, background: "rgba(255,255,255,0.06)", borderRadius: 99, height: 4 }}>
                    <div style={{ width: `${pct}%`, height: "100%", background: "#2980B9", borderRadius: 99 }} />
                  </div>
                  <span style={{ fontSize: 12, color: "#C9A84C", width: 30, textAlign: "right" }}>{count}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="card fade-up-4" style={{ padding: 24 }}>
        <div style={{ fontSize: 11, color: "#C9A84C", letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>✦ Platform Management</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 10 }}>
          {[
            ["◈", "Seekers", "seekers"], ["◆", "Employers", "employers"],
            ["◉", "Job Postings", "postings"], ["◐", "Journey Tracking", "journey"],
            ["✦", "Trainers & Mentors", "trainers-overview"], ["≡", "Reports & Export", "reports"],
            ["⊞", "My Profile", "profile"], ["⊕", "Program Settings", "settings"],
          ].map(([icon, label, page]) => (
            <button key={page} onClick={() => setActivePage(page)}
              style={{ padding: "16px 12px", borderRadius: 12, border: "1px solid rgba(201,168,76,0.12)", background: "rgba(255,255,255,0.03)", cursor: "pointer", textAlign: "center", transition: "all .2s", fontFamily: "'DM Sans',sans-serif" }}
              ...
              <div style={{ fontSize: 20, color: "#C9A84C", marginBottom: 8 }}>{icon}</div>
              <div style={{ fontSize: 12, color: "#F0EBE0", fontWeight: 500 }}>{label}</div>
            </button>
          ))}
        </div>
      </div>

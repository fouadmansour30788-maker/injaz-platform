...
function InjazSeekers() {
  ...
  useEffect(() => {
    db.getAllUsers().then(u => { setSeekers(u.seekers); setLoading(false); });
  }, []);
...
function AttendanceVerification({ isInjazTeam = false }) {
  ...
  const loadData = async () => {
    const data = await db.getAllAttendance().catch(() => []);
    setAllSessions(data);
    setLoading(false);
  };
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Freeze detector: if the page was hidden for more than 5 minutes, reload
    let hiddenAt = null;
    ...
    const handleShow = async () => {
      if (document.visibilityState === 'visible' && hiddenAt) {
        ...
        if (hiddenFor > 5 * 60 * 1000) {
          // Hidden for more than 5 minutes - do a soft reload
